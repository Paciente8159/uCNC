 ...
 * This allows to create a custom probing inputs (like other inputs, software
 * conditions, etc..) Note that $6 will have no effect on the custom probing
 * logic
 */
 ...
 * Enable this option if you want to disable the limits for rotation axis to
 * work allowing them to rotatle freely
 *
 */
 ...
 * If commented ABC will be treated like individual linear axis for speed
 ...
 * Enable this option if you want the rotation axis to work in relative distance
 * mode only This will mean that no matter if the machine is working in absolute
 * (G90) or relative (G91) coordinates the rotational axis will always calculate
 * the motion in relative distance mode
 *
 */
 ...
 * NOTE: If Laser PPI is enabled one of the stepper drivers position will be
 * used by the laser controller Usually that is STEPPER<AXIS_COUNT> so if
 * AXIS_COUNT=3, STEPPER3 will be used by laser PPI
 */
 ...
 * LINACT are the linear actuators that drive the machine motion. You can think
 * of linear actuator as a combination of linear guide + motor
 *
 ...
 * etc..
 *
 * In some machines the coorelation between AXIS and LINACT is direct. For
 * example cartesian machines the AXIS X motion is the result of motions of the
 * LINACT0, AXIS Y of LINACT1, AXIS Z of LINACT2, etc. On Core XY kinematics the
 * same logic as the cartesian is applied (regarding AXIS and LINACT logic)
 * although, AXIS X and Y motions are a combination of motions of both LINACT0
 * and LINAXCT1. On Delta type machines the same logic as the cartesian is
 * applied (regarding AXIS and LINACT logic) although, AXIS X, Y and Z are all a
 * combination of motions of LINACT0, 1 and 2.
 *
 * As stated earlier LINACT is a combination of linear guide + motor. Usually a
 * LINACT is defined as a single stepper motor. But it can also be composed of
 * multiple stepper motors (2, 3, 4, etc...)
 *
 * To enable this just define the LINACTx_IO_MASK as a combination of
 * STEPx_IO_MASK's
 * **/
...
// there is no limit to the ammount of STEP IO that can be combined into a
// LINACT. For example it's possible to assign 4 independent STEP IO to a single
// LINACT #define LINACT2_IO_MASK (STEP2_IO_MASK | STEP5_IO_MASK | STEP6_IO_MASK
// | STEP7_IO_MASK)

/**
 * SELF SQUARING/AUTOLEVEL AXIS
 * Limits switches have a mask value that is equivalent to the STEPx that it
 * controls.
 *
 * By default these mask values match the corresponding AXIS, LINACT, STEP,
 * etc...
 *
 * STEPx			7		6		5 4 3 2 1
 * 0 STEPx_IO_MASK	128		64		32		16
 * 8		4		2		1 AXISx			-
 * -		C		B		C		Z Y X LINACTx
 * -		-		5		4		3 2 1 0 LIMITx
 * -		-		C		B		A
 * Z&Z2	Y&Y2	X&X2
 *
 * LINACT with multiple STEP IO pulse all those IO in sync, but when homing it
 * can stop independently as it hits the correspondent limit until all motors
 * reach the desired home position. To achieve that each each LIMITx_IO_MASK
 * should be set to the corresponding STEP IO MASK that it controls
 *
 * For example to use STEP0 and STEP6 to drive the AXIS_X/LINACT0 you need to
 * configure the correct LINACT0_IO_MASK and then to make LIMIT_X stop STEP0 and
 * LIMIT_X2 stop STEP6 you need to reassign LIMIT_X2 to STEP6 IO MASK do it like
 * this
 *
...
/**
 * Advanced Multi-axis
 * The advantage of using this mask scheme is that it's possible to defined
 * advanced custom self squaring/planning rigs and complete reassing of unused
 * axis limits to perform the task
 *
 * Let's assume a custom cartesian machine that has 3 axis with 5 motors and 5
 * limit switches with the following configuration
 *
 * Axis X - 1 motor/limit
 * Axis Y - 2 motors/limits (self squaring)
 * Axis Z - 3 motors/limits (self leveling - 3 point plane)
 *
 * By default, with the machine configured for a 3 axis machine 3 linear
 * actuators are configured with one motor each. By default AXIS_X -> LINACT0 ->
 * STEP0, AXIS_Y-> LINACT1 -> STEP1, etc... One possibility would be to map the
 * connections (from STEP0 to STEP5) as X,Y,Z,Y2,Z2,Z3.
 *
 * Let's assume the user also wants the connections to be made in the order
 * X,Y,Y2,Z,Z2,Z3
 *
 *
 * Axis X does not require any special configuration. Uses the default settings
 * for a single linear actuator with one motor and a limit switch Axis Y
 * requires a second STEPx output. Because we also want to reorder the
 * connection we must define our custom LINACTx for the remaining axis like this
 *
 ...
 *
 * Limits for AXIS X does not require any special configuration. Uses the
 * default settings for a single linear actuator with one motor and a limit
 * switch (uses LIMIT_X) Limits for AXIS Y only require Limit Y2 to be reasigned
 * to match STEP2 so in this case
 *
 * #define LIMIT_Y2_IO_MASK STEP2_IO_MASK
 *
 * And finally limits for AXIS Z will new to be reassigned to the matching
 * STEPx. We will also need a 3rd limit. We will use LIMIT_A since AXIS_A is not
 * used.
 *
 ...
 *
 * There is still a final step that involve reassign LINACT2 limits to include
 * the extra limit (LIMIT A) like this
 *
...
/*
        Tool definition
        For any given tool the respective macro TOOLx (x from 1 to 16) must be
   created Tools must be assigned in sequential order That is TOOL1, TOOL2
   etc... You can't skip tool numbers (for example define TOOL1 and TOOL3
   without having a TOOL2)
*/
 ...
 *
 * Tools can be any of the built in tools available in /src/hal/tools/tools/ or
 * you can use your own custom tool.
 *
 * **/
 ...
 * If defined and the expression evaluates to true the ISR will be unable to
 * generate steps until condition cleared
 *
 * In the example bellow if input pin DIN19 is active step ISR will stop
 * generating steps Can be uses for example in sewing machines to prevent motion
 * on needle down detection and avoid damadge to the needle
 *
 * If RT_STEP_PREVENT_CONDITION is not defined it will default to a hookable
 * callback named itp_rt_step_prevent_cb
 *
 * **/
 ...
 *
 * To know more about encoders please read the document under
 * /modules/encoder.md
 ...
 * Some MCU like ESP32 require this option to be enabled because the IO by some
 * SDK function calls without realizing
 */
