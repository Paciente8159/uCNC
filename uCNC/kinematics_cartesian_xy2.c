...
#if(MACHINE_KINEMATICS==MACHINE_CARTESIAN_XY2)
#include <stdio.h>
#include <math.h>
#include "mcu.h"
#include "settings.h"
...
void kinematics_apply_inverse(float* axis, uint32_t* steps)
{
    ...
    steps[1] = steps[2] = (uint32_t)lroundf(g_settings.step_per_mm[1] * axis[AXIS_Y]);
}
...
void kinematics_lock_step(uint8_t limits_mask)
{
    ...
    if (CHECKFLAG(limits_mask, LIMIT_Y_MASK))
    {
        if(limits_mask & LIMITS_LIMIT0_MASK) //LIMIT_Y hit
        {
            itp_lock_stepper(1);
        }
        else if(limits_mask & LIMITS_LIMIT1_MASK) //LIMIT_Y2 hit
        {
            itp_lock_stepper(2);
        }
    }
