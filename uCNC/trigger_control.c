...
{
	g_cnc_state.hard_limits = limits;
...
bool tc_check_boundaries(float* axis)
{
		...
		{
			g_cnc_state.soft_limits |= LIMIT_X_MASK;
			return false;
		}
	#endif
	#ifdef AXIS_Y
		...
		{
			g_cnc_state.soft_limits |= LIMIT_Y_MASK;
			return false;
		}
	#endif
	#ifdef AXIS_Z
		...
		{
			g_cnc_state.soft_limits |= LIMIT_Z_MASK;
			return false;
		}
	#endif
	#ifdef AXIS_A
		...
		{
			g_cnc_state.soft_limits |= LIMIT_A_MASK;
			return false;
		}
	#endif
	#ifdef AXIS_B
		...
		{
			g_cnc_state.soft_limits |= LIMIT_B_MASK;
			return false;
		}
	#endif
	#ifdef AXIS_C
		...
		{
			g_cnc_state.soft_limits |= LIMIT_C_MASK;
			return false;
		}
	#endif
