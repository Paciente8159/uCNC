...
#define __combusreg__(X) (__helper__(RCC->, X, ENR))
#define __drreg__(X) (__helper__(X, ->DR, ))
...
#define LIMIT_X_EXTIVAL EXTINT(LIMIT_X_PORT)
...
#define COM_TX_GPIOREG __gpio__(TX_PORT)

#if (TX_PIN < 8)
#define COM_TX_CTRLREG CRL
#else
#define COM_TX_CTRLREG CRH
#endif

#define COM_BUS (__helper__(RCC->, X, ENR))
...
#define mcu_config_output(diopin)                                                                             \
	{                                                                                                         \
		...
		__indirect__(diopin, GPIO)->__indirect__(diopin, CR) &= ~(0xF << (__indirect__(diopin, CROFF) << 2)); \
		__indirect__(diopin, GPIO)->__indirect__(diopin, CR) |= (0x7 << (__indirect__(diopin, CROFF) << 2));  \
	}
#define mcu_config_input(diopin)                                                                              \
	{                                                                                                         \
		RCC->APB2ENR = __indirect__(diopin, APB2EN);                                                          \
		__indirect__(diopin, GPIO)->__indirect__(diopin, CR) &= ~(0xF << (__indirect__(diopin, CROFF) << 2)); \
		__indirect__(diopin, GPIO)->__indirect__(diopin, CR) |= (0x4 << (__indirect__(diopin, CROFF) << 2));  \
	}
#define mcu_config_pullup(diopin)                                                                             \
	{                                                                                                         \
		__indirect__(diopin, GPIO)->__indirect__(diopin, CR) &= ~(0x4 << (__indirect__(diopin, CROFF) << 2)); \
		__indirect__(diopin, GPIO)->__indirect__(diopin, CR) |= ~(0x8 << (__indirect__(diopin, CROFF) << 2)); \
...
#define mcu_config_input_isr(diopin)                                                                                \
	{                                                                                                               \
		...
		EXTI->RTSR = __indirect__(diopin, BIT);                                                                     \
		EXTI->FTSR = __indirect__(diopin, BIT);                                                                     \
		EXTI->IMR = __indirect__(diopin, BIT);                                                                      \
