/*
        ...
        All entry points for extending µCNC core functionalities are declared in
   this module.
...
#define LISTENER_HWSPI_LOCK                                                    \
  0x02 // prevent multiple accesses to HWSPI by different modules
#define LISTENER_HWSPI2_LOCK                                                   \
  0x04 // prevent multiple accesses to HWSPI by different modules
#define LISTENER_HWI2C_LOCK 0x08 // for future use
#define LISTENER_SWSPI_LOCK                                                    \
  0x20 // prevent multiple accesses to any software emulated SPI by different
       // modules
#define LISTENER_SWI2C_LOCK                                                    \
  0x40 // prevent multiple accesses to any software emulated I2C by different
       // modules
...
#define MODULE_LOCK_ENABLE(X)                                                  \
  SETFLAG(g_module_lockguard, (X & ~LISTENER_RUNNING_LOCK))
#define MODULE_LOCK_DISABLE(X)                                                 \
  CLEARFLAG(g_module_lockguard, (X & ~LISTENER_RUNNING_LOCK))
...
#define CREATE_EVENT_LISTENER(name, handler)                                   \
  __attribute__((used)) name##_delegate_event_t name##_delegate_##handler = {  \
      &handler, LISTENER_NO_LOCK, NULL}
#define CREATE_EVENT_LISTENER_WITHLOCK(name, handler, lock_flags)              \
  __attribute__((used)) name##_delegate_event_t name##_delegate_##handler = {  \
      &handler, (lock_flags & (~LISTENER_RUNNING_LOCK)), NULL}
...
// definitions to create overridable default handlers for functions with a
// declaration like uint8_t (*function)(void *, bool *); the resulting handles
// is named event_<event name>_handler and can be placed inside any point in the
// core code for example DECL_EVENT_HANDLER(<event name>) will create a function
// declaration equivalent to uint8_t event_<event name>_handler(void* args)
// event_<event name>_handler can then be placed inside the core code to run the
// hook code
...
// this comes with a small performance penalty but also makes code about 1k~2k
// smaller by using a generic function
...
#define DEFAULT_EVENT_HANDLER(name)                                            \
  {                                                                            \
    ...
    while (ptr != NULL) {                                                      \
      start = ptr->next;                                                       \
      if (ptr->fptr != NULL &&                                                 \
          !CHECKFLAG(ptr->fplock,                                              \
                     (g_module_lockguard | LISTENER_RUNNING_LOCK))) {          \
...
bool mod_event_default_handler(mod_delegate_event_t **event,
                               mod_delegate_event_t **last, void **args);

#define DEFAULT_EVENT_HANDLER(name)                                            \
  {                                                                            \
    static name##_delegate_event_t *last = NULL;                               \
    return mod_event_default_handler((mod_delegate_event_t **)(&name##_event), \
                                     (mod_delegate_event_t **)(&last),         \
                                     (void **)&args);                          \
  }
#endif
void mod_init(void);
...
#define HOOK_INVOKE_ISR(name, ...)                                             \
  if (name##_cb) {                                                             \
    mcu_isr_context_enter(); /*this make the event aware that is running       \
                                inside and ISR*/                               \
    name##_cb(__VA_ARGS__);                                                    \
  }
