...
#define DECL_GRBL_STREAM(name, getc_cb, available_cb, clear_cb, putc_cb,       \
                         flush_cb)                                             \
  grbl_stream_t name = {getc_cb, available_cb, clear_cb,                       \
                        putc_cb, flush_cb,     NULL}
...
#define DBGMSGIF(condition, fmt, ...)                                          \
  do {                                                                         \
    if (DEBUG_STREAM && DEBUG_STREAM->registered && (condition)) {             \
      debug_printf(__romstr__(DEBUG_PRELUDE fmt "\n"), ##__VA_ARGS__);         \
      debug_flush();                                                           \
    }                                                                          \
  } while (0)
