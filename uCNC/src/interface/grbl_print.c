    ...
    if (c == '%') {
      ...
      case 'u':
        ...
        do {
          ...
          case 'X':
            maxlen =
                prt_byte(out, maxlen, (const uint8_t *)pt, (hexflags | lcount));
            break;
          case 'I':
      ...
      case 'F':
        ...
        do {
          ...
          case 'a':
          case 'A':
            maxlen = prt_byte(out, maxlen, (const uint8_t *)f_ptr++,
                              (hexflags | VAR_DWORD));
            break;
          default:
...
#define atof_peek(cb, buffer)                                                  \
  ((!buffer) ? ((prt_getc_cb)cb)(true)                                         \
             : ((cb) ? ((prt_read_rom_byte)cb)(*buffer) : **buffer))
#define atof_get(cb, buffer)                                                   \
  ((!buffer) ? ((prt_getc_cb)cb)(false) : ({                                   \
    *buffer += 1;                                                              \
    0;                                                                         \
  }))
