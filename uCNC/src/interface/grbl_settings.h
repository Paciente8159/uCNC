/*
                Name: settings.h
                Description: µCNC runtime settings. These functions store
   settings and other parameters in non-volatile memory.
...
// step delay not used
  ...
  // value must be set between -1.0 and 1.0 If set to 0.0 is the same as exact
  // path mode (G61) and -1.0 is the same as exact stop mode (G61.1)
...
#define SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET                              \
  (SETTINGS_ADDRESS_OFFSET + sizeof(settings_t) + 1)
#endif
...
#endif
#define PARSER_PARAM_SIZE                                                      \
  (sizeof(float) * AXIS_COUNT) // parser parameters array size
#define PARSER_PARAM_ADDR_OFFSET                                               \
  (PARSER_PARAM_SIZE + 1)         // parser parameters array size + 1 crc byte
...
#define PARSER_CORDSYS_ADDRESS                                                 \
  (SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET) // 1st coordinate system offset
                                              // eeprom address (G54)
#define G28ADDRESS                                                             \
  (SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +                                 \
   (PARSER_PARAM_ADDR_OFFSET *                                                 \
    G28HOME)) // G28 coordinate offset eeprom address
#define G30ADDRESS                                                             \
  (SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +                                 \
   (PARSER_PARAM_ADDR_OFFSET *                                                 \
    G30HOME)) // G28 coordinate offset eeprom address
#ifdef G92_STORE_NONVOLATILE
#define G92ADDRESS                                                             \
  (SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +                                 \
   (PARSER_PARAM_ADDR_OFFSET *                                                 \
    G92OFFSET)) // G92 coordinate offset eeprom address
#endif
...
#ifdef G92_STORE_NONVOLATILE
#define STARTUP_BLOCK0_ADDRESS_OFFSET                                          \
  (SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +                                 \
   (PARSER_PARAM_ADDR_OFFSET * (TOTAL_COORDINATE_SYSTEMS + 1)))
#else
#define STARTUP_BLOCK0_ADDRESS_OFFSET                                          \
  (SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +                                 \
   (PARSER_PARAM_ADDR_OFFSET * TOTAL_COORDINATE_SYSTEMS))
#endif
#endif
#define STARTUP_BLOCK_ADDRESS_OFFSET(NBLOCK)                                   \
  (STARTUP_BLOCK0_ADDRESS_OFFSET + (NBLOCK * RX_BUFFER_SIZE))

#ifndef MODULES_SETTINGS_ADDRESS_OFFSET
#define MODULES_SETTINGS_ADDRESS_OFFSET                                        \
  STARTUP_BLOCK_ADDRESS_OFFSET(STARTUP_BLOCKS_COUNT)
#endif
...
#define __DECL_EXTENDED_SETTING__(ID, var, type, count, print_cb)              \
  ...
  bool set##ID##_settings_load(void *args) {                                   \
    settings_load(set##ID##_settings_address, (uint8_t *)var,                  \
                  sizeof(type) * count);                                       \
    return EVENT_CONTINUE;                                                     \
  }                                                                            \
  bool set##ID##_settings_save(void *args) {                                   \
    settings_save(set##ID##_settings_address, (uint8_t *)var,                  \
                  sizeof(type) * count);                                       \
    return EVENT_CONTINUE;                                                     \
  }                                                                            \
...
#define DECL_EXTENDED_SETTING(ID, var, type, count, print_cb)                  \
  __DECL_EXTENDED_SETTING__(ID, var, type, count, print_cb)
...
#define __DECL_EXTENDED_STRING_SETTING__(ID, var, count)                       \
  ...
  bool set##ID##_settings_save(void *args) { return EVENT_CONTINUE; }          \
  bool set##ID##_settings_change(void *args) {                                 \
    setting_args_t *set = (setting_args_t *)args;                              \
    if (set->id == ID) {                                                       \
      settings_load(set##ID##_settings_address, (uint8_t *)var,                \
...
#define DECL_EXTENDED_STRING_SETTING(ID, var, count)                           \
  __DECL_EXTENDED_STRING_SETTING__(ID, var, count)
...
#define __EXTENDED_SETTING_INIT__(ID, var)                                     \
  ...
  if (!set##ID##_init) {                                                       \
    set##ID##_settings_address =                                               \
        settings_register_external_setting(sizeof(var));                       \
    ...
    set##ID##_settings_load(NULL); /* do the initial settings load because     \
                                      event may have fired already */          \
    set##ID##_init = true;                                                     \
  }
