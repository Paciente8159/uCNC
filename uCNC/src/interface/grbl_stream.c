...
DECL_GRBL_STREAM(uart_grbl_stream, mcu_uart_getc, mcu_uart_available,
                 mcu_uart_clear, mcu_uart_putc, mcu_uart_flush);
#endif
#if defined(MCU_HAS_UART2) && !defined(DETACH_UART2_FROM_MAIN_PROTOCOL)
DECL_GRBL_STREAM(uart2_grbl_stream, mcu_uart2_getc, mcu_uart2_available,
                 mcu_uart2_clear, mcu_uart2_putc, mcu_uart2_flush);
#endif
#if defined(MCU_HAS_USB) && !defined(DETACH_USB_FROM_MAIN_PROTOCOL)
DECL_GRBL_STREAM(usb_grbl_stream, mcu_usb_getc, mcu_usb_available,
                 mcu_usb_clear, mcu_usb_putc, mcu_usb_flush);
#endif
#if defined(ENABLE_SOCKETS) && !defined(DETACH_TELNET_FROM_MAIN_PROTOCOL)
DECL_GRBL_STREAM(telnet_grbl_stream, mcu_telnet_getc, mcu_telnet_available,
                 mcu_telnet_clear, mcu_telnet_putc, mcu_telnet_flush);
#endif
#if defined(MCU_HAS_BLUETOOTH) && !defined(DETACH_BLUETOOTH_FROM_MAIN_PROTOCOL)
DECL_GRBL_STREAM(bt_grbl_stream, mcu_bt_getc, mcu_bt_available, mcu_bt_clear,
                 mcu_bt_putc, mcu_bt_flush);
#endif
#endif
...
static void FORCEINLINE debug_putc(char c) {
  if (!BUFFER_TRY_ENQUEUE(debug_tx, &c)) {
    ...
    rom_strcpy((char *)debug_tx_bufferdata,
               __romstr__("Debug buffer overflow!!\n\0"));
    debug_tx_lines = 1;
...
#ifndef DISABLE_MULTISTREAM_SERIAL
  grbl_stream_t *cleanup __attribute__((__cleanup__(stream_stdin))) =
      default_stream;
...
grbl_stream_t *grbl_stream_readonly(grbl_stream_getc_cb getc_cb,
                                    grbl_stream_available_cb available_cb,
                                    grbl_stream_clear_cb clear_cb) {
#ifdef ENABLE_MULTISTREAM_GUARD
  if (grbl_stream_rx_busy) {
    return NULL;
  }
...
uint8_t grbl_stream_busy(void) { return grbl_stream_tx_count; }

void grbl_stream_print_cb(void *arg, char c) { grbl_stream_putc(c); }
