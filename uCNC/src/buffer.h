...
#error                                                                         \
    "You need to manually define buf_index_byteoffset. This should be a value equal to log2(8 * sizeof(buffer_index_t))"
#endif
...
#define DECL_BUFFER(type, name, size)                                          \
  ...
  static buffer_index_t name##_bufferflags[((size + buf_index_bitoffset) >>    \
                                            buf_index_byteoffset)];            \
  ring_buffer_t name = {                                                       \
      0, 0, name##_bufferflags, name##_bufferdata, size, sizeof(type)}
...
#define BUFFER_WRITE(buffer, ptr, len, written)                                \
  buffer_write(&buffer, ptr, len, &written)
#define BUFFER_READ(buffer, ptr, len, read)                                    \
  buffer_read(&buffer, ptr, len, &read)
...
#define BUFFER_TRY_DEQUEUE(buffer, ptr)                                        \
  ((!BUFFER_EMPTY(buffer)) ? ({                                                \
    uint8_t tail;                                                              \
    ATOMIC_CODEBLOCK { tail = buffer.tail; }                                   \
...
#define BUFFER_TRY_ENQUEUE(buffer, ptr)                                        \
  ((!BUFFER_FULL(buffer)) ? ({                                                 \
    uint8_t head;                                                              \
    ATOMIC_CODEBLOCK { head = buffer.head; }                                   \
  ...
  ({                                                                           \
    ...
    if (count) {                                                               \
      ...
      if (avail < count && avail) {                                            \
        memcpy(&buffer##_bufferdata[head], ptr,                                \
               avail * sizeof(buffer##_bufferdata[0]));                        \
        (written) = avail;                                                     \
        count -= avail;                                                        \
        head = 0;                                                              \
      } else {                                                                 \
      ...
      if (count) {                                                             \
        memcpy(&buffer##_bufferdata[head], &ptr[avail],                        \
               count * sizeof(buffer##_bufferdata[0]));                        \
        (written) += count;                                                    \
        ATOMIC_CODEBLOCK {                                                     \
          head += count;                                                       \
          if (head == buffer##_size) {                                         \
...
#define BUFFER_READ(buffer, ptr, len, read)                                    \
  ({                                                                           \
    ...
    if (count) {                                                               \
      ...
      if (avail < count && avail) {                                            \
        memcpy(ptr, &buffer##_bufferdata[tail],                                \
               avail * sizeof(buffer##_bufferdata[0]));                        \
        (read) = avail;                                                        \
        count -= avail;                                                        \
        tail = 0;                                                              \
      ...
      if (count) {                                                             \
        memcpy(&ptr[avail], &buffer##_bufferdata[tail],                        \
               count * sizeof(buffer##_bufferdata[0]));                        \
        (read) += count;                                                       \
        ATOMIC_CODEBLOCK {                                                     \
          tail += count;                                                       \
...
#define BUFFER_CLEAR(buffer)                                                   \
  {ATOMIC_CODEBLOCK{buffer##_bufferdata[0] = 0;                                \
  buffer.tail = 0;                                                             \
  buffer.head = 0;                                                             \
  buffer.count = 0;                                                            \
  }                                                                            \
