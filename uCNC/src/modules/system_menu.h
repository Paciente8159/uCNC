...
#define SYSTEM_MENU_ID_HOMING 3
#define SYSTEM_MENU_ID_STEPPER_SETTINGS 4
#define SYSTEM_MENU_ID_KINEMATIC_SETTINGS 5
#define SYSTEM_MENU_ID_IO_SETTIGNS 6
#define SYSTEM_MENU_ID_JOG 7
#define SYSTEM_MENU_ID_OVERRIDES 8
#define SYSTEM_MENU_ID_MISC_SETTINGS 9
...
// Characters given to this macro should be in range of printable ASCII
// characters (32-127)
...
#define DECL_MENU_ENTRY(menu_id, name, strvalue, arg_ptr, display_cb,          \
                        display_cb_arg, action_cb, action_cb_arg)              \
  static const system_menu_item_t name##_item __rom__ = {                      \
      .label = strvalue,                                                       \
      .argptr = arg_ptr,                                                       \
      .item_render = display_cb,                                               \
      .render_arg = display_cb_arg,                                            \
      .item_action = action_cb,                                                \
      .action_arg = action_cb_arg};                                            \
 ...
 * Helper macros
 * **/
#define DECL_MENU_LABEL(menu_id, name, strvalue)                               \
  DECL_MENU_ENTRY(menu_id, name, strvalue, NULL, NULL, NULL, NULL, NULL)
#define DECL_MENU_GOTO(menu_id, name, strvalue, menu)                          \
  DECL_MENU_ENTRY(menu_id, name, strvalue, NULL, NULL, "->",                   \
                  system_menu_action_goto, menu)
#define DECL_MENU_VAR(menu_id, name, strvalue, varptr, vartype)                \
  DECL_MENU_ENTRY(menu_id, name, strvalue, varptr,                             \
                  system_menu_item_render_var_arg, CONST_VARG(vartype),        \
                  system_menu_action_edit, CONST_VARG(vartype))
#define DECL_MENU_VAR_SIMPLE(menu_id, name, strvalue, varptr, vartype)         \
  DECL_MENU_ENTRY(menu_id, name, strvalue, varptr,                             \
                  system_menu_item_render_var_arg, CONST_VARG(vartype),        \
                  system_menu_action_edit_simple, CONST_VARG(vartype))
#define DECL_MENU_VAR_CUSTOM_EDIT(menu_id, name, strvalue, varptr, vartype,    \
                                  actioncb, actioncb_arg)                      \
  DECL_MENU_ENTRY(menu_id, name, strvalue, varptr,                             \
                  system_menu_item_render_var_arg, CONST_VARG(vartype),        \
                  actioncb, actioncb_arg)
#define DECL_MENU_ACTION(menu_id, name, strvalue, action_cb, action_cb_arg)    \
  DECL_MENU_ENTRY(menu_id, name, strvalue, NULL, NULL, NULL, action_cb,        \
                  action_cb_arg)

#define DECL_MENU(id, parentid, label)                                         \
  static const char m##id##_label[] __rom__ = label;                           \
  static system_menu_page_t m##id = {.menu_id = id,                            \
                                     .parent_id = parentid,                    \
                                     .page_label = m##id##_label,              \
                                     .page_render = NULL,                      \
                                     .page_action = NULL,                      \
                                     .items_index = NULL,                      \
                                     .extended = NULL};                        \
...
#define DECL_DYNAMIC_MENU(id, parentid, render_cb, action_cb)                  \
  static system_menu_page_t m##id = {.menu_id = id,                            \
                                     .parent_id = parentid,                    \
                                     .page_label = NULL,                       \
                                     .page_render = render_cb,                 \
                                     .page_action = action_cb,                 \
                                     .items_index = NULL,                      \
                                     .extended = NULL};                        \
  system_menu_append(&m##id)
#define MENU(id) (&m##id)

#define MENU_LOOP(page, item)                                                  \
  for (system_menu_page_t *item = page; item != NULL; item = item->extended)
...
/**
 * Overridable functions to be implemented for the display to render the system
 * menu
 * **/
void system_menu_render_header(const char *__s);
bool system_menu_render_menu_item_filter(uint8_t item_index);
void system_menu_render_menu_item(uint8_t render_flags,
...
#define system_menu_int_to_str(buf_ptr, var)                                   \
  ({                                                                           \
    char *_ptr = (buf_ptr);                                                    \
    char **ptr_ptr = &_ptr;                                                    \
    prt_int(ptr_ptr, PRINT_MAX, (uint32_t)var, 0);                             \
  })
#define system_menu_flt_to_str(buf_ptr, var)                                   \
  ({                                                                           \
    char *_ptr = (buf_ptr);                                                    \
    char **ptr_ptr = &_ptr;                                                    \
    prt_flt(ptr_ptr, PRINT_MAX, (float)var,                                    \
            ((!g_settings.report_inches) ? 3 : 5));                            \
  })
