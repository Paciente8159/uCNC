...
static uint8_t softi2c_clock_stretch(softi2c_port_t *port,
                                     uint32_t ms_timeout) {
  // releases the clock then monitors the clock line to see if it's available or
  // not if not released under 50ms issues a timeout error
  port->scl(true);
...
static uint8_t softi2c_write(softi2c_port_t *port, uint8_t c, bool send_start,
                             bool send_stop, uint32_t ms_timeout) {
  uint8_t ack = 0;
  TASK_YIELD();
  if (send_start) {
    // init
    ...
    __TIMEOUT_ASSERT__(timeout) { return I2C_NOTOK; }
...
uint8_t softi2c_send(softi2c_port_t *port, uint8_t address, uint8_t *data,
                     uint8_t len, bool release, uint32_t ms_timeout) {
  if (!port) {
#ifdef MCU_HAS_I2C
  ...
  if (len) {
    len--;
    if (softi2c_write(port, address << 1, true, false, ms_timeout) ==
...
uint8_t softi2c_receive(softi2c_port_t *port, uint8_t address, uint8_t *data,
                        uint8_t len, uint32_t ms_timeout) {
  if (!port) {
#ifdef MCU_HAS_I2C
  ...
  if (len) {
    len--;
    if (softi2c_write(port, (address << 1) | 0x01, true, false, ms_timeout) ==
        I2C_OK) // start, send address, write
    {
      for (uint8_t i = 0; i < len; i++) {
