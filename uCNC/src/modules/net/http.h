 ...
 * Delegates execute from socket_server_dotasks(), never from an ISR/backend
 * task.
 */
 ...
 * and data are copied into bounded static client state. Calls after headers
 * have started, invalid input, overflow, or a full header table return false.
 */
...
static inline int http_send_str(int client_idx, int code,
                                const char *content_type, const char *data) {
  return http_send(client_idx, code, content_type, data,
                   data ? strlen(data) : 0U);
}
...
/*
 * Copies the current upload base name and NUL terminates when maxlen is
 * nonzero. Invalid clients produce an empty string. filename may not be NULL.
 */
