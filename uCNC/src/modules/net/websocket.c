...
static void ws_notify_close(websocket_protocol_t *ws, uint8_t client_idx,
                            ws_client_state_t *client, uint16_t code) {
...
static int ws_send_handshake(websocket_protocol_t *ws, uint8_t client_idx,
...
static size_t ws_frame_header(uint8_t *header, uint8_t opcode,
                              size_t payload_len) {
  if (payload_len <= 125U) {
 ...
 */
static int ws_send_frame(websocket_protocol_t *ws, uint8_t client_idx,
                         uint8_t opcode, const uint8_t *payload,
                         size_t payload_len, bool mark_close) {
  ws_client_state_t *client;
  uint8_t header[10];
  size_t header_len;
  int sent;
...
static int ws_parse_header(ws_client_state_t *client) {
  ...
  if ((b0 & 0x70U) != 0U || (b1 & 0x80U) == 0U)
    return -1;
  if (!(opcode == 0x0U || opcode == 0x1U || opcode == 0x2U || opcode == 0x8U ||
        opcode == 0x9U || opcode == 0xAU))
    return -1;
...
static void ws_dispatch_payload(websocket_protocol_t *ws,
                                ws_client_state_t *client, uint8_t client_idx,
  ...
  if (!ws || !client || !ws->ws_onrecv_cb)
    return;
  effective_opcode = client->state.frame.frame_opcode == 0U
                         ? client->state.frame.message_opcode
...
static void ws_handle_control_complete(websocket_protocol_t *ws,
  ...
  if (client->state.frame.frame_opcode == 0x9U) {
    queued =
        ws_send_frame(ws, client_idx, 0xAU, client->state.frame.control_buf,
                      client->state.frame.control_len, false);
    if (queued < 0)
      (void)socket_close(ws->ws_socket, client_idx);
    ...
    client->close_code = code;
    queued =
        ws_send_frame(ws, client_idx, 0x8U, client->state.frame.control_buf,
                      client->state.frame.control_len, true);
    if (queued < 0) {
      client->status = WS_S_CLOSING;
      (void)socket_close(ws->ws_socket, client_idx);
    }
...
static void ws_handle_handshake(websocket_protocol_t *ws,
  ...
  /* The request parser advances the pointer but must treat pointed bytes as
   * read-only. */
      ...
      if (client->state.hs.header.status == REQ_HEAD_FINISHED) {
        if (client->state.hs.header.name[0] == '\0') {
          if (client->state.hs.handshake.hs_got_upgrade &&
            ...
            sha1_init(&sha1);
            sha1_update(&sha1,
                        (const uint8_t *)client->state.hs.handshake.hs_key,
            ...
            if (length <= 0 || (size_t)length >= sizeof(response) ||
                ws_send_handshake(ws, client_idx, response, (size_t)length,
                                  true) < 0)
              (void)socket_close(ws->ws_socket, client_idx);
          } else {
        ...
        http_request_ws_handshake(&client->state.hs.handshake,
                                  &client->state.hs.header);
        memset(&client->state.hs.header, 0, sizeof(client->state.hs.header));
      }
    }
  }
...
static void ws_on_data(uint8_t client_idx, const uint8_t *data, size_t data_len,
                       void *protocol) {
  websocket_protocol_t *ws = (websocket_protocol_t *)protocol;
  ws_client_state_t *client;
  ...
  while (remaining != 0U && client->status == WS_S_OPEN) {
    if (client->state.frame.hdr_need == 0U ||
        client->state.frame.hdr_have < client->state.frame.hdr_need) {
      if (client->state.frame.hdr_have < 2U) {
        ...
        memcpy(&client->state.frame.hdr[client->state.frame.hdr_have], cursor,
               take);
      ...
      if (client->state.frame.hdr_have < client->state.frame.hdr_need) {
        size_t needed =
            client->state.frame.hdr_need - client->state.frame.hdr_have;
        size_t take = remaining < needed ? remaining : needed;
        memcpy(&client->state.frame.hdr[client->state.frame.hdr_have], cursor,
               take);
        client->state.frame.hdr_have += (uint8_t)take;
    ...
    if (client->state.frame.payload_rem != 0U) {
      ...
      while (available != 0U) {
        ...
        size_t chunk =
            available > WS_STREAM_CHUNK ? WS_STREAM_CHUNK : available;
        size_t i;
        for (i = 0U; i < chunk; ++i)
          unmasked[i] =
              (uint8_t)(cursor[i] ^
                        client->state.frame
                            .mask[(client->state.frame.mask_i++) & 3U]);

        if ((client->state.frame.frame_opcode & 0x08U) != 0U) {
          if (chunk > sizeof(client->state.frame.control_buf) -
                          client->state.frame.control_len) {
            ws_protocol_error_close(ws, client_idx, client, 1002U);
            return;
          }
          memcpy(
              &client->state.frame.control_buf[client->state.frame.control_len],
              unmasked, chunk);
          client->state.frame.control_len += (uint8_t)chunk;
        } else {
          bool final_chunk = (uint64_t)chunk == client->state.frame.payload_rem;
          ws_dispatch_payload(ws, client, client_idx, unmasked, chunk,
                              final_chunk);
        }
        cursor += chunk;
        remaining -= chunk;
...
static int ws_send_one(websocket_protocol_t *ws, uint8_t client_idx,
