/*
        ...
        Description: Generic, allocation-free TCP transport backend interface
   for uCNC.

        This header defines the contract between the generic uCNC TCP stream
   core (socket.c) and a platform/network backend. A backend may be implemented
   using BSD sockets, lwIP raw callbacks, a WIZnet hardware TCP engine, an RTOS
   network stack, or another reliable byte-stream provider.

        This is deliberately a TCP stream interface. It is not a BSD socket API
   and it does not expose UDP or IP raw sockets. TCP sequencing, retransmission,
        congestion control, checksums and connection state are owned by the
   native TCP implementation beneath the backend.
...
typedef struct socket_device_ {
   ...
   */
  int (*recv)(socket_device_handle_t client, void *destination,
              size_t capacity);
   ...
   * order, but no token may be used after closed(). TX readiness is
   * deliberately not an event: blocking core sends poll until completion, while
   * nonblocking callers own their retry policy.
   *
