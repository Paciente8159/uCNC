...
uint8_t http_discard_line(uint8_t status, uint8_t initial_condition, char **buf,
                          size_t *len) {
  char *buffer = *buf;
  char *target = NULL;
...
void http_request_parse_start(request_ctx_t *ctx, char **buf, size_t *len) {
  ...
  if (ctx->status < REQ_START_FINISHED) {
    ...
    // this also signals the line start (since a chunk of the http request may
    // not contain a complete line)
    if (ctx->status < REQ_START_METHOD_PARSED) {
      target = find_char(buffer, 10, ' ');
      if (!target) // it should at least find the method at the start of the
                   // string. If not something went wrong
      {
        return;
      }
    ...
    while (ctx->status < REQ_START_QUERY_PARSED) {
      ...
      case REQ_START_URI_PARSED:
        ...
        if (target) {
          ctx->status = (*target == ' ') ? REQ_START_QUERY_PARSED
                                         : REQ_START_QUERYVAR_FOUND;
        } else {
          *len = 0;
          break;
        }
      ...
      case REQ_START_QUERYVAR_FOUND:
        ...
        if (target) {
          ...
          *target++ = 0;
          // memset(ctx->arg_val[ctx->arg_count], 0,
          // sizeof(ctx->arg_val[ctx->arg_count]));
          counter++;
        } else {
          target = find_char(buffer, *len, ' ');
          if (target) {
      ...
      case REQ_START_QUERYARG_FOUND:
        ...
        if (target) {
          *target++ = 0;
          // memset(ctx->arg_name[ctx->arg_count], 0,
          // sizeof(ctx->arg_name[ctx->arg_count]));
          ctx->status = REQ_START_QUERYVAR_FOUND;
        } else {
          target = find_char(buffer, *len, ' ');
          if (target) {
        ...
        ctx->arg_val[ctx->arg_count - 1] =
            &ctx->arg_name[ctx->arg_count - 1][arg_offset];
        strncat_local(ctx->arg_val[ctx->arg_count - 1], arg_max, buffer,
                      strlen(buffer));
        (*len) -= (target - buffer);
        buffer = target;
        break;
    ...
    ctx->status =
        http_discard_line(ctx->status, REQ_START_QUERY_PARSED, &buffer, len);
  }

  *buf = buffer;
}

void http_request_parse_header(request_header_t *header, char **buf,
                               size_t *len) {
  char *buffer = *buf;
  char *target = NULL;
  ...
  header->status =
      http_discard_line(header->status, REQ_HEAD_VALUE_PARSED, &buffer, len);
  *buf = buffer;
}

void http_request_ws_handshake(ws_handshake_t *wsh, request_header_t *header) {
  if (!strncasecmp_local("upgrade", header->name, 7) &&
      strcasestr_local(header->value, "websocket")) {
    wsh->hs_got_upgrade = true;
  } else if (!strncasecmp_local("connection", header->name, 9) &&
             strcasestr_local(header->value, "upgrade")) {
    ...
    wsh->hs_got_key = true;
  } else if (!strncasecmp_local("sec-websocket-version", header->name, 21) &&
             strcasestr_local(header->value, "13")) {
    wsh->hs_got_version = true;
  }
}

void http_request_file_upload(request_upload_t *upload,
                              request_header_t *header) {
  if (!strncasecmp_local("content-type", header->name,
                         sizeof("content-type"))) {
  ...
  } else if (!strncasecmp_local("content-length", header->name,
                                sizeof("content-length"))) {
    strntrim_local(header->value);
    upload->upload_len = atol(header->value);
  } else if (!strncasecmp_local("content-disposition", header->name,
                                sizeof("content-disposition"))) {
...
void http_request_multipart_chunk(char **buf, size_t *len,
  ...
  if (upload->status < REQ_UPLOAD_INIT_FINISHED) {
    while (*len) {
      ...
      if (header->status == REQ_HEAD_FINISHED) {
        if (header->name[0] == 0) {
          upload->status = (upload->status == REQ_UPLOAD_FILENAME_FOUND)
                               ? REQ_UPLOAD_INIT_FINISHED
                               : REQ_UPLOAD_INIT;
        ...
        case REQ_UPLOAD_BOUNDARY_FOUND:
          if (!strncasecmp_local("content-disposition", header->name,
                                 sizeof("content-disposition"))) {
