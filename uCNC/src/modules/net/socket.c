/*
        Name: socket.c
        Description: Allocation-free, readiness-driven TCP stream server for
   uCNC.

        The backend reports normalized readiness/lifecycle events during poll().
   The core records those events in fixed state and dispatches application
   callbacks after poll() returns. RX is pulled into one shared scratch buffer,
...
enum { SOCKET_IF_FREE = 0, SOCKET_IF_ACTIVE, SOCKET_IF_STOPPING };
...
static socket_device_token_t
socket_device_accepted(socket_device_handle_t listener,
 ...
 * Address comparison is used instead of subtracting potentially foreign
 * pointers.
 */
 ...
 */
static socket_device_token_t socket_make_token(uint16_t slot,
                                               uint16_t generation) {
 ...
 * A pending-close record is returned; callers decide whether that state is
...
/*
 * Clears a listener only after stop-listening has delivered every pending
 * client disconnect. This preserves callback pointers and protocol context
 * until then.
 */
...
/*
 * Event sink: allocates a static logical client and returns its generation
 * token. No application callback is invoked here; connection notification is
 * deferred.
 */
static socket_device_token_t
socket_device_accepted(socket_device_handle_t listener,
...
static void socket_device_closed(socket_device_token_t token, int reason) {
  ...
  client->close_reason =
      (int8_t)((reason < 0 && reason >= SOCKET_DEVICE_TIMEOUT)
                   ? reason
                   : SOCKET_DEVICE_ERROR);
}
...
int socket_send(socket_if_t *socket, uint8_t client_idx, const void *data,
  ...
  if (!client || client->handle == SOCKET_DEVICE_INVALID_HANDLE ||
      (client->flags &
       (SOCKET_CLIENT_CONNECT_NOTIFIED | SOCKET_CLIENT_CLOSE_PENDING)) !=
          SOCKET_CLIENT_CONNECT_NOTIFIED) {
    return SOCKET_DEVICE_INVALID;
  }
  ...
  while (total < data_len) {
    ...
    if (client->handle == SOCKET_DEVICE_INVALID_HANDLE ||
      ...
      return client->close_reason < 0 ? client->close_reason
    ...
    if ((uint32_t)(mcu_millis() - start_ms) >=
        (uint32_t)SOCKET_SEND_TIMEOUT_MS) {
...
bool socket_client_is_connected(const socket_if_t *socket, uint8_t client_idx) {
  socket_client_state_t *client = socket_get_client(socket, client_idx, NULL);
 ...
 * Returns true when a pending event/RX attempt/idle callback consumed this
 * turn.
 */
static bool socket_dispatch_client(uint16_t slot) {
  ...
  if ((client->flags & SOCKET_CLIENT_CLOSE_PENDING) != 0U) {
    ...
    bool notified = (client->flags & SOCKET_CLIENT_CONNECT_NOTIFIED) != 0U;
  ...
  if ((client->flags & SOCKET_CLIENT_READABLE) != 0U) {
    ...
    received = socket_device->recv(client->handle, socket_rx_buffer,
    ...
    if (received > 0) {
      ...
      if (socket->client_ondata_cb) {
        socket->client_ondata_cb(local_idx, socket_rx_buffer, (size_t)received,
                                 socket->protocol);
      }
      ...
      if ((client->flags & SOCKET_CLIENT_CLOSE_PENDING) == 0U) {
        ...
        client->flags |= SOCKET_CLIENT_CLOSE_PENDING;
        client->close_reason =
            (int8_t)((received >= SOCKET_DEVICE_TIMEOUT) ? received
                                                         : SOCKET_DEVICE_ERROR);
      }
    }
    return true;
