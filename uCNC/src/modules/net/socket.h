/*
        ...
        scratch buffer. Native TCP buffering and TCP protocol correctness remain
   the responsibility of the registered socket_device_t backend.

        The API is cooperative and single-owner. Call socket_server_dotasks()
        frequently from the uCNC owner loop. Application callbacks execute only
   from that function and never directly from an ISR, another core, an RTOS
   network task, or backend poll().
 ...
 */
typedef void (*socket_data_delegate)(uint8_t client_idx, const uint8_t *data,
 ...
 */
typedef void (*socket_disconnect_delegate)(uint8_t client_idx, int reason,
                                           void *protocol);
 ...
 */
typedef void (*socket_idle_delegate)(uint8_t client_idx, uint32_t idle_ms,
                                     void *protocol);
