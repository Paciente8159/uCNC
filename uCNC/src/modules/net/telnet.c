...
static const uint8_t telnet_welcome[] = {TELNET_IAC, TELNET_WILL, 0x01U,
...
static void telnet_refuse_option(telnet_protocol_t *telnet, uint8_t client_idx,
  ...
  if (!telnet || !telnet->telnet_socket || client_idx >= SOCKET_MAX_CLIENTS)
    return;
  reply[0] = TELNET_IAC;
  reply[1] = (command == TELNET_DO || command == TELNET_DONT) ? TELNET_WONT
                                                              : TELNET_DONT;
  reply[2] = option;
  if (socket_send(telnet->telnet_socket, client_idx, reply, sizeof(reply),
                  false) < 0 &&
 ...
 * modified in place.
 */
static void telnet_on_data(uint8_t client_idx, const uint8_t *data,
  ...
  for (i = 0U; i < data_len; ++i) {
    ...
    case TELNET_PARSE_SUBNEGOTIATION_IAC:
      client->parse_state =
          (byte == TELNET_SE) ? TELNET_PARSE_DATA : TELNET_PARSE_SUBNEGOTIATION;
      break;
...
static void telnet_on_disconnected(uint8_t client_idx, int reason,
                                   void *protocol) {
  telnet_protocol_t *telnet = (telnet_protocol_t *)protocol;
  (void)reason;
...
void telnet_broadcast(telnet_protocol_t *telnet, const void *data,
  ...
  for (;;) {
    ...
    for (i = 0U; i < SOCKET_MAX_CLIENTS; ++i) {
      ...
      sent = socket_send(telnet->telnet_socket, i, &payload[offsets[i]],
                         data_len - offsets[i], true);
      if (sent > 0) {
