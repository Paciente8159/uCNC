/*
        Name: http.c
        Description: Implements a simple Telnet Server based on BSD/POSIX
   Sockets for µCNC.
...
static void release_client(int client_idx) {
  if (client_idx >= 0 && client_idx < SOCKET_MAX_CLIENTS) {
    if (clients[client_idx].upl.status == REQ_UPLOAD_INIT_FINISHED) {
      ...
      if (clients[client_idx].route &&
          clients[client_idx].route->file_handler) {
...
void http_add(const char *uri, uint8_t method, http_delegate request_handler,
              http_delegate file_handler) {
  if (!uri || uri[0] == '\0')
    return;
  for (uint8_t i = 0U; i < route_count; ++i) {
    // check if handler already exists
    if (!strncasecmp_local(routes[i].uri, (char *)uri, strlen(uri)) &&
        (strlen(uri) == strlen(routes[i].uri)) &&
...
bool http_request_arg(int client_idx, const char *argname, char *argvalue,
                      size_t maxlen) {
  http_client_t *c;
...
static int http_send_headers(http_client_t *c, int client_idx, int code,
  ...
  if (!c)
    return SOCKET_DEVICE_INVALID;
  if (content_type &&
      strlen(content_type) > sizeof(line) - sizeof("Content-Type: \r\n"))
    return SOCKET_DEVICE_INVALID;
  length = str_snprintf(line, sizeof(line), "HTTP/1.1 %d %s\r\n", code,
                        http_reason_phrase(code));
  ...
  if (content_type) {
    length =
        str_snprintf(line, sizeof(line), "Content-Type: %s\r\n", content_type);
    if (length <= 0 || (size_t)length >= sizeof(line))
      return SOCKET_DEVICE_INVALID;
    result = http_send_bytes(client_idx, line, (size_t)length);
    if (result < 0)
...
bool http_send_header(int client_idx, const char *name, const char *data,
                      bool first) {
  http_client_t *c;
  size_t name_len;
  size_t data_len;
  ...
  if (name_len == 0U || name_len >= HTTP_MAX_HEADER_LEN ||
      data_len > HTTP_MAX_HEADER_LEN - name_len - 3U || strchr(name, '\r') ||
      strchr(name, '\n') || strchr(name, ':') || strchr(data, '\r') ||
      strchr(data, '\n'))
    return false;
...
int http_send(int client_idx, int code, const char *content_type,
...
static void http_progress_file(int client_idx) {
  ...
  length =
      fs_read(c->response_file, http_file_buffer, sizeof(http_file_buffer));
  if (length == 0U) {
...
bool http_send_file(int client_idx, const char *file_path,
...
static void handle_upload_bytes(int client_idx, char **buf, size_t *len) {
  ...
  if (c->upl.status == REQ_UPLOAD_FINISH) {
    ...
    if (c->head.status == REQ_HEAD_FINISHED) {
      ...
      } else if (!strncmp(c->head.name, c->upl.boundary,
                          strlen(c->upl.boundary))) {
        http_send_str(client_idx, 200, "text/plain; charset=UTF-8",
                      "File uploaded successfully");
        http_send(client_idx, 200, "text/plain; charset=UTF-8", NULL, 0);
      } else {
        http_send_str(client_idx, 413, "text/plain; charset=UTF-8",
                      "File error");
...
static void http_on_disconnected(uint8_t client_idx, int reason,
                                 void *protocol) {
  (void)reason;
  (void)protocol;
  if (client_idx < SOCKET_MAX_CLIENTS)
...
static void http_on_data(uint8_t client_idx, const uint8_t *data,
                         size_t data_len, void *protocol) {
  http_client_t *c;
  char *bytes;
  (void)protocol;
  ...
  if (c->response_file)
    return;
  /* Request parser advances the pointer but must treat pointed RX bytes
   * read-only. */
