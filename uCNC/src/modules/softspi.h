...
/**
 * The new softspi port structure allow to create software SPI ports with
 * different configurations. It also allows to take advantage of the Arduino
 * library to allow creation of new HW SPI ports.
 *
 * For example to create a new SPI port using Arduino for ESP32 you can do
 * something like this #ifdef ARDUINO #include <SPI.h> #include <softspi.h>
 * SPIClass* myspiport;
 *
 ...
 * 	myspiport->beginTransaction(SPISettings(uint32_t frequency,
 * SPI_MSBFIRST, uint8_t mode));
 * }
 *
 ...
 * }
 *
 * extern "C" spi_port_t __attribute__((used)) arduino_spi_port {.start =
 * myspiport_start, .xmit = myspiport_xmit, .bulk_xmit=myspiport_bulk .stop =
 * myspiport_stop} extern "C" softspi_port_t __attribute__((used)) ARDUINO_SPI =
 * {.spiconfig = {0}, .spifreq = 20000000UL, .spiport = &arduino_spi_port, .clk
 * = NULL, .mosi = NULL, .miso = NULL, .config = myspiport_config}; #endif
 *
 */
...
// the maximum amount of time in milliseconds it will transmit data without
...
#define SOFTSPI(NAME, FREQ, MODE, MOSIPIN, MISOPIN, CLKPIN)                    \
  ...
  __attribute__((used)) softspi_port_t NAME = {.spiconfig = {.mode = MODE},    \
                                               .spifreq = FREQ,                \
                                               .spiport = NULL,                \
                                               .clk = &NAME##_clk,             \
                                               .mosi = &NAME##_mosi,           \
...
#define HARDSPI(NAME, FREQ, MODE, PORT)                                        \
  __attribute__((used)) softspi_port_t NAME = {.spiconfig = {.mode = MODE},    \
                                               .spifreq = FREQ,                \
                                               .spiport = &PORT,               \
                                               .clk = NULL,                    \
...
void softspi_config(softspi_port_t *port, spi_config_t config,
                    uint32_t frequency);
...
// software emulated SPI sends data for a maximum BULK_SPI_TIMEOUT before
// recalling the main loop
void softspi_bulk_xmit(softspi_port_t *port, const uint8_t *out, uint8_t *in,
                       uint16_t len);
void softspi_stop(softspi_port_t *port);

// helper functions
static FORCEINLINE void softspi_set_mode(softspi_port_t *port,
                                         uint8_t spi_mode) {
  port->spiconfig.mode = spi_mode;
  softspi_config(port, port->spiconfig, port->spifreq);
}
static FORCEINLINE void softspi_set_frequency(softspi_port_t *port,
                                              uint32_t spi_freq) {
  softspi_config(port, port->spiconfig, spi_freq);
}
