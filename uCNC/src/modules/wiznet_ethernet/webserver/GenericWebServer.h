/*
  GenericWebServer.h - Dead simple web-server.
...
#ifndef GenericWebServer_H
#define GenericWebServer_H
...
class GenericWebServerTemplate;
...
{
public:
  GenericWebServerTemplate(IPAddress addr, int port = 80);
  GenericWebServerTemplate(int port = 80);
  ~GenericWebServerTemplate();

  using RequestHandlerType = RequestHandler<ServerType>;
  // using WebServerType = GenericWebServerTemplate<ServerType>;
	// using WebClientType = GenericWebClientTemplate<ClientType>;
  ...
  using HookFunction = std::function<ClientFuture(const String& method, const String& url, ClientType* client, ContentTypeFunction contentType)>;
...
#include "GenericWebServer-impl.h"
#include "Parsing-impl.h"

using GenericWebServer = GenericWebServer::GenericWebServerTemplate<WiFiServer>;
using RequestHandler = GenericWebServer::RequestHandler<WiFiServer>;

#endif //GenericWebServer_H