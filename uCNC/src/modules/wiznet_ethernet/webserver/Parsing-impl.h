...
template <typename ServerType>
typename GenericWebServerTemplate<ServerType>::ClientFuture GenericWebServerTemplate<ServerType>::_parseRequest(ClientType& client) {
...
template <typename ServerType>
bool GenericWebServerTemplate<ServerType>::_collectHeader(const char* headerName, const char* headerValue) {
  for (int i = 0; i < _headerKeysCount; i++) {
...
struct storeArgHandler
{
  ...
  {
    key = GenericWebServerTemplate<ServerType>::urlDecode(data.substring(pos, key_end_pos));
    if ((equal_index != -1) && ((equal_index < next_index - 1) || (next_index == -1)))
      value = GenericWebServerTemplate<ServerType>::urlDecode(data.substring(equal_index + 1, next_index));
  }
};
...
template <typename ServerType>
void GenericWebServerTemplate<ServerType>::_parseArguments(const String& data) {
  if (_currentArgs)
    delete[] _currentArgs;
...
template <typename ServerType>
int GenericWebServerTemplate<ServerType>::_parseArgumentsPrivate(const String& data, std::function<void(String&,String&,const String&,int,int,int,int)> handler) {
...
template <typename ServerType>
void GenericWebServerTemplate<ServerType>::_uploadWriteByte(uint8_t b){
...
template <typename ServerType>
int GenericWebServerTemplate<ServerType>::_uploadReadByte(ClientType& client){
  int res = client.read();
  if(res == -1){
...
template <typename ServerType>
bool GenericWebServerTemplate<ServerType>::_parseForm(ClientType& client, const String& boundary, uint32_t len){
  (void) len;
...
template <typename ServerType>
String GenericWebServerTemplate<ServerType>::urlDecode(const String& text)
{
  String decoded;
  char temp[] = "0x00";
...
template <typename ServerType>
bool GenericWebServerTemplate<ServerType>::_parseFormUploadAborted(){
