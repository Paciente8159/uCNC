...
int DhcpClass::request_DHCP_lease()
{
	...
	unsigned long startTime = mcu_millis();

	while (_dhcp_state != STATE_DHCP_LEASED) {
		if (_dhcp_state == STATE_DHCP_START) {
			_dhcpTransactionId++;
			send_DHCP_MESSAGE(DHCP_DISCOVER, ((mcu_millis() - startTime) / 1000));
			_dhcp_state = STATE_DHCP_DISCOVER;
		} else if (_dhcp_state == STATE_DHCP_REREQUEST) {
			_dhcpTransactionId++;
			send_DHCP_MESSAGE(DHCP_REQUEST, ((mcu_millis() - startTime)/1000));
		...
		if (result != 1 && ((mcu_millis() - startTime) > _timeout))
			break;
	}
	...
	_lastCheckLeaseMillis = mcu_millis();
	return result;
}
...
uint8_t DhcpClass::parseDHCPResponse(unsigned long responseTimeout, uint32_t& transactionId)
{
	...
	while (_dhcpUdpSocket.parsePacket() <= 0) {
		if ((mcu_millis() - startTime) > responseTimeout) {
			return 255;
		}
		cnc_delay_ms(50);
	}
