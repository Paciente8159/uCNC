...
static bool system_menu_action_settings_cmd(uint8_t action,
                                            system_menu_item_t *item);
static bool system_menu_action_jog(uint8_t action, system_menu_item_t *item);
static bool system_menu_action_overrides(uint8_t action,
                                         system_menu_item_t *item);
static void system_menu_render_axis_position(uint8_t render_flags,
                                             system_menu_item_t *item);
static bool system_menu_action_nav_back(uint8_t action,
                                        const system_menu_page_t *item);
...
DECL_MODULE(system_menu) {
  ...
  // append idle menu
  DECL_DYNAMIC_MENU(SYSTEM_MENU_ID_IDLE, 0, system_menu_idle,
                    system_menu_main_open);

  // append main
  DECL_MENU(SYSTEM_MENU_ID_MAIN_MENU, SYSTEM_MENU_ID_IDLE, STR_MAIN_MENU);

  // main menu entries
  DECL_MENU_ACTION(SYSTEM_MENU_ID_MAIN_MENU, hold, STR_HOLD,
                   system_menu_action_rt_cmd, CONST_VARG(CMD_CODE_FEED_HOLD));
  DECL_MENU_ACTION(SYSTEM_MENU_ID_MAIN_MENU, resume, STR_RESUME,
                   system_menu_action_rt_cmd, CONST_VARG(CMD_CODE_CYCLE_START));
  DECL_MENU_ACTION(SYSTEM_MENU_ID_MAIN_MENU, unlock, STR_UNLOCK,
                   system_menu_action_serial_cmd, "$X\r");
  DECL_MENU_ACTION(SYSTEM_MENU_ID_MAIN_MENU, home, STR_HOME,
                   system_menu_action_serial_cmd, "$H\r");
  DECL_MENU_GOTO(SYSTEM_MENU_ID_MAIN_MENU, jog, STR_JOG,
                 CONST_VARG(SYSTEM_MENU_ID_JOG));
  DECL_MENU_GOTO(SYSTEM_MENU_ID_MAIN_MENU, overrides, STR_OVERRIDES,
                 CONST_VARG(SYSTEM_MENU_ID_OVERRIDES));
  DECL_MENU_GOTO(SYSTEM_MENU_ID_MAIN_MENU, settings, STR_SETTINGS,
                 CONST_VARG(SYSTEM_MENU_ID_SETTINGS));

  DECL_MENU(SYSTEM_MENU_ID_OVERRIDES, SYSTEM_MENU_ID_MAIN_MENU, STR_OVERRIDES);
  DECL_MENU_VAR_CUSTOM_EDIT(SYSTEM_MENU_ID_OVERRIDES, ovf, STR_FEED_OVR,
                            &g_planner_state.feed_override, VAR_TYPE_UINT8,
                            system_menu_action_overrides, CONST_VARG('f'));
  DECL_MENU_ACTION(SYSTEM_MENU_ID_OVERRIDES, ovf_100, STR_FEED_100,
                   system_menu_action_rt_cmd, CONST_VARG(CMD_CODE_FEED_100));
#if (TOOL_COUNT > 0)
  DECL_MENU_VAR_CUSTOM_EDIT(SYSTEM_MENU_ID_OVERRIDES, ovt, STR_TOOL_OVR,
                            &g_planner_state.spindle_speed_override,
                            VAR_TYPE_UINT8, system_menu_action_overrides,
                            CONST_VARG('s'));
#endif
  DECL_MENU_ACTION(SYSTEM_MENU_ID_OVERRIDES, ovt_100, STR_TOOL_100,
                   system_menu_action_rt_cmd, CONST_VARG(CMD_CODE_SPINDLE_100));

  // append Jog menu
  // default initial distance
  DECL_MENU(SYSTEM_MENU_ID_JOG, SYSTEM_MENU_ID_MAIN_MENU, STR_JOG);
  DECL_MENU_ENTRY(SYSTEM_MENU_ID_JOG, jogx, STR_JOG_AXIS("X"), NULL,
                  system_menu_render_axis_position, NULL,
                  system_menu_action_jog, "X");
#if (AXIS_COUNT > 1)
  DECL_MENU_ENTRY(SYSTEM_MENU_ID_JOG, jogy, STR_JOG_AXIS("Y"), NULL,
                  system_menu_render_axis_position, NULL,
                  system_menu_action_jog, "Y");
#endif
#if (AXIS_COUNT > 2)
  DECL_MENU_ENTRY(SYSTEM_MENU_ID_JOG, jogz, STR_JOG_AXIS("Z"), NULL,
                  system_menu_render_axis_position, NULL,
                  system_menu_action_jog, "Z");
#endif
#if (AXIS_COUNT > 3)
  DECL_MENU_ENTRY(SYSTEM_MENU_ID_JOG, joga, STR_JOG_AXIS("A"), NULL,
                  system_menu_render_axis_position, NULL,
                  system_menu_action_jog, "A");
#endif
#if (AXIS_COUNT > 4)
  DECL_MENU_ENTRY(SYSTEM_MENU_ID_JOG, jogb, STR_JOG_AXIS("B"), NULL,
                  system_menu_render_axis_position, NULL,
                  system_menu_action_jog, "B");
#endif
#if (AXIS_COUNT > 5)
  DECL_MENU_ENTRY(SYSTEM_MENU_ID_JOG, jogc, STR_JOG_AXIS("C"), NULL,
                  system_menu_render_axis_position, NULL,
                  system_menu_action_jog, "C");
#endif
  DECL_MENU_VAR(SYSTEM_MENU_ID_JOG, jogdist, STR_JOG_DIST,
                &g_system_menu_jog_distance, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_JOG, jogfeed, STR_JOG_FEED,
                &g_system_menu_jog_feed, VAR_TYPE_FLOAT);

  // append settings menu
  DECL_MENU(SYSTEM_MENU_ID_SETTINGS, SYSTEM_MENU_ID_MAIN_MENU, STR_SETTINGS);

  // settings menu
  DECL_MENU_GOTO(SYSTEM_MENU_ID_SETTINGS, ioconfig, STR_IO_CONFIG,
                 CONST_VARG(SYSTEM_MENU_ID_IO_SETTIGNS));
  DECL_MENU_GOTO(SYSTEM_MENU_ID_SETTINGS, gohome, STR_HOMING,
                 CONST_VARG(SYSTEM_MENU_ID_HOMING));
#if (AXIS_COUNT > 0)
  DECL_MENU_GOTO(SYSTEM_MENU_ID_SETTINGS, goaxis, STR_AXIS,
                 CONST_VARG(SYSTEM_MENU_ID_STEPPER_SETTINGS));
#endif
#if (defined(ENABLE_SKEW_COMPENSATION) ||                                      \
     (KINEMATIC == KINEMATIC_LINEAR_DELTA) || (KINEMATIC == KINEMATIC_DELTA))
  DECL_MENU_GOTO(SYSTEM_MENU_ID_SETTINGS, kinemats, STR_KINEMATICS,
                 CONST_VARG(SYSTEM_MENU_ID_KINEMATIC_SETTINGS));
#endif
  DECL_MENU_GOTO(SYSTEM_MENU_ID_SETTINGS, other_config, STR_OTHER,
                 CONST_VARG(SYSTEM_MENU_ID_MISC_SETTINGS));
  DECL_MENU_ACTION(SYSTEM_MENU_ID_SETTINGS, set_load, STR_LOAD_SETTINGS,
                   system_menu_action_settings_cmd, CONST_VARG(0));
  DECL_MENU_ACTION(SYSTEM_MENU_ID_SETTINGS, set_save, STR_SAVE_SETTINGS,
                   system_menu_action_settings_cmd, CONST_VARG(1));
  DECL_MENU_ACTION(SYSTEM_MENU_ID_SETTINGS, set_reset, STR_RESET_SETTINGS,
                   system_menu_action_settings_cmd, CONST_VARG(2));

  DECL_MENU(SYSTEM_MENU_ID_MISC_SETTINGS, SYSTEM_MENU_ID_SETTINGS, STR_OTHER);
  DECL_MENU_VAR(SYSTEM_MENU_ID_MISC_SETTINGS, s11, STR_G64_FACT,
                &g_settings.g64_angle_factor, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_MISC_SETTINGS, s12, STR_ARC_TOL,
                &g_settings.arc_tolerance, VAR_TYPE_FLOAT);

#ifdef H_MAPPING_EEPROM_STORE_ENABLED
  DECL_MENU_VAR(SYSTEM_MENU_ID_MISC_SETTINGS, s215, STR_ARC_TOL,
                &g_settings.hmap_x, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_MISC_SETTINGS, s216, STR_ARC_TOL,
                &g_settings.hmap_y, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_MISC_SETTINGS, s217, STR_ARC_TOL,
                &g_settings.hmap_x_offset, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_MISC_SETTINGS, s218, STR_ARC_TOL,
                &g_settings.hmap_y_offset, VAR_TYPE_FLOAT);
  // TODO allow edit each offset individually
#endif

  DECL_MENU(SYSTEM_MENU_ID_IO_SETTIGNS, SYSTEM_MENU_ID_SETTINGS, STR_IO_CONFIG);
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s2, STR_STEP_INV,
                &g_settings.dir_invert_mask, VAR_TYPE_UINT8);
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s3, STR_DIR_INV,
                &g_settings.dir_invert_mask, VAR_TYPE_UINT8);
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s4, STR_ENABLE_INV,
                &g_settings.step_enable_invert, VAR_TYPE_UINT8);
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s5, STR_LIMITS_INV,
                &g_settings.limits_invert_mask, VAR_TYPE_UINT8);
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s6, STR_PROBE_INV,
                &g_settings.probe_invert_mask, VAR_TYPE_BOOLEAN);
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s7, STR_CONTROL_INV,
                &g_settings.control_invert_mask, VAR_TYPE_UINT8);
#if ENCODERS > 0
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s8, STR_ENC_P_INV,
                &g_settings.encoders_pulse_invert_mask, VAR_TYPE_UINT8);
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s9, STR_ENC_D_INV,
                &g_settings.encoders_dir_invert_mask, VAR_TYPE_UINT8);
#endif
#ifdef ENABLE_STEPPERS_DISABLE_TIMEOUT
  DECL_MENU_VAR(SYSTEM_MENU_ID_IO_SETTIGNS, s10, STR_STEPPER_TIMEOUT,
                &g_settings.step_disable_timeout, VAR_TYPE_UINT16);
#endif

  // append homing settings menu
  DECL_MENU(SYSTEM_MENU_ID_HOMING, SYSTEM_MENU_ID_SETTINGS, STR_HOMING);

  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s20, STR_SOFTLIMITS,
                &g_settings.soft_limits_enabled, VAR_TYPE_BOOLEAN);
  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s21, STR_HARDLIMITS,
                &g_settings.hard_limits_enabled, VAR_TYPE_BOOLEAN);
  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s22, STR_ENABLE_HOMING,
                &g_settings.homing_enabled, VAR_TYPE_BOOLEAN);
  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s23, STR_DIR_INV_MASK,
                &g_settings.homing_dir_invert_mask, VAR_TYPE_UINT8);
  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s24, STR_SLOW_FEED,
                &g_settings.homing_slow_feed_rate, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s25, STR_FAST_FEED,
                &g_settings.homing_fast_feed_rate, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s26, STR_DEBOUNCEMS,
                &g_settings.debounce_ms, VAR_TYPE_UINT16);
  DECL_MENU_VAR(SYSTEM_MENU_ID_HOMING, s27, STR_OFFSET,
                &g_settings.homing_offset, VAR_TYPE_FLOAT);

  // append steppers settings menu
  DECL_MENU(SYSTEM_MENU_ID_STEPPER_SETTINGS, SYSTEM_MENU_ID_SETTINGS, STR_AXIS);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s100, STR_STEPMM("X"),
                &g_settings.step_per_mm[0], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s110, STR_VMAX("X"),
                &g_settings.max_feed_rate[0], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s120, STR_ACCEL("X"),
                &g_settings.acceleration[0], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s130, STR_MAX_DIST("X"),
                &g_settings.max_distance[0], VAR_TYPE_FLOAT);
#ifdef ENABLE_BACKLASH_COMPENSATION
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s140, STR_BACKLASH("X"),
                &g_settings.backlash_steps[0], VAR_TYPE_UINT16);
#endif

#if (AXIS_COUNT > 1)
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s101, STR_STEPMM("Y"),
                &g_settings.step_per_mm[1], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s111, STR_VMAX("Y"),
                &g_settings.max_feed_rate[1], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s121, STR_ACCEL("Y"),
                &g_settings.acceleration[1], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s131, STR_MAX_DIST("Y"),
                &g_settings.max_distance[1], VAR_TYPE_FLOAT);
#ifdef ENABLE_BACKLASH_COMPENSATION
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s141, STR_BACKLASH("Y"),
                &g_settings.backlash_steps[1], VAR_TYPE_UINT16);
#endif
#endif
#if (AXIS_COUNT > 2)
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s102, STR_STEPMM("Z"),
                &g_settings.step_per_mm[2], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s112, STR_VMAX("Z"),
                &g_settings.max_feed_rate[2], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s122, STR_ACCEL("Z"),
                &g_settings.acceleration[2], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s132, STR_MAX_DIST("Z"),
                &g_settings.max_distance[2], VAR_TYPE_FLOAT);
#ifdef ENABLE_BACKLASH_COMPENSATION
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s142, STR_BACKLASH("Z"),
                &g_settings.backlash_steps[2], VAR_TYPE_UINT16);
#endif
#endif
#if (AXIS_COUNT > 3)
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s103, STR_STEPMM("A"),
                &g_settings.step_per_mm[3], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s113, STR_VMAX("A"),
                &g_settings.max_feed_rate[3], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s123, STR_ACCEL("A"),
                &g_settings.acceleration[3], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s133, STR_MAX_DIST("A"),
                &g_settings.max_distance[3], VAR_TYPE_FLOAT);
#ifdef ENABLE_BACKLASH_COMPENSATION
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s143, STR_BACKLASH("A"),
                &g_settings.backlash_steps[3], VAR_TYPE_UINT16);
#endif
#endif
#if (AXIS_COUNT > 4)
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s104, STR_STEPMM("B"),
                &g_settings.step_per_mm[4], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s114, STR_VMAX("B"),
                &g_settings.max_feed_rate[4], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s124, STR_ACCEL("B"),
                &g_settings.acceleration[4], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s134, STR_MAX_DIST("B"),
                &g_settings.max_distance[4], VAR_TYPE_FLOAT);
#ifdef ENABLE_BACKLASH_COMPENSATION
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s144, STR_BACKLASH("B"),
                &g_settings.backlash_steps[4], VAR_TYPE_UINT16);
#endif
#endif
#if (AXIS_COUNT > 5)
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s105, STR_STEPMM("C"),
                &g_settings.step_per_mm[5], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s115, STR_VMAX("C"),
                &g_settings.max_feed_rate[5], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s125, STR_ACCEL("C"),
                &g_settings.acceleration[5], VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s135, STR_MAX_DIST("C"),
                &g_settings.max_distance[5], VAR_TYPE_FLOAT);
#ifdef ENABLE_BACKLASH_COMENSATION
  DECL_MENU_VAR(SYSTEM_MENU_ID_STEPPER_SETTINGS, s145, STR_BACKLASH("C"),
                &g_settings.backlash_steps[5], VAR_TYPE_UINT16);
#endif
#endif

#if (defined(ENABLE_SKEW_COMPENSATION) ||                                      \
     defined(KINEMATICS_VARS_SYSTEM_MENU_INIT))
  DECL_MENU(SYSTEM_MENU_ID_KINEMATIC_SETTINGS, SYSTEM_MENU_ID_SETTINGS,
            STR_KINEMATICS);
#ifdef ENABLE_SKEW_COMPENSATION
  DECL_MENU_VAR(SYSTEM_MENU_ID_KINEMATIC_SETTINGS, s37, STR_SKEW_FACTOR("XY"),
                &g_settings.skew_xy_factor, VAR_TYPE_FLOAT);
#ifndef SKEW_COMPENSATION_XY_ONLY
  DECL_MENU_VAR(SYSTEM_MENU_ID_KINEMATIC_SETTINGS, s38, STR_SKEW_FACTOR("XZ"),
                &g_settings.skew_xz_factor, VAR_TYPE_FLOAT);
  DECL_MENU_VAR(SYSTEM_MENU_ID_KINEMATIC_SETTINGS, s39, STR_SKEW_FACTOR("YZ"),
                &g_settings.skew_yz_factor, VAR_TYPE_FLOAT);
#endif
#endif
...
/**
 *
 * system_menu_action and system_menu_render are the two primary functions to be
 * executed in the display's loop always call system_menu_action with the user
 * action (or no action) followed by system_menu_render to update the display if
 * needed
 *
 * **/
void system_menu_action(uint8_t action) {
  ...
  const system_menu_item_t *menuitem_ptr =
      (system_menu_item_t *)system_menu_get_current_item();
...
void system_menu_render(void) {
  ...
  if (render_flags & SYSTEM_MENU_MODE_REDRAW) {
        ...
        if (g_system_menu.flags & SYSTEM_MENU_MODE_EDIT) {
          ...
          while (item) {
            if (system_menu_render_menu_item_filter(item_index)) {
              system_menu_render_menu_item(
                  render_flags |
                      ((cur_index == item_index) ? SYSTEM_MENU_MODE_SELECT : 0),
                  item->menu_item);
            }
            item = item->next;
            item_index++;
    ...
    system_menu_render_nav_back((g_system_menu.current_index < 0 ||
                                 g_system_menu.current_multiplier < 0));
...
void system_menu_show_modal_popup(uint32_t timeout, const char *__s) {
  // prevents redraw
  g_system_menu.flags &=
      ~(SYSTEM_MENU_MODE_REDRAW | SYSTEM_MENU_MODE_DELAYED_REDRAW);
  // renders the popup
...
static bool system_menu_action_settings_cmd(uint8_t action,
  ...
  if (action == SYSTEM_MENU_ACTION_SELECT) {
    ...
    case 0:
      ...
      break;
    case 1:
      settings_save(SETTINGS_ADDRESS_OFFSET, (uint8_t *)&g_settings,
                    (uint8_t)sizeof(settings_t));
...
bool system_menu_action_edit_simple(uint8_t action, system_menu_item_t *item) {
  ...
  if (action == SYSTEM_MENU_ACTION_SELECT) {
    ...
    case VAR_TYPE_UINT16:
      ...
      (*(uint16_t *)item->argptr) =
          CLAMP(0, (*(uint16_t *)item->argptr), 0xFFFF);
      break;
    case VAR_TYPE_INT32:
    case VAR_TYPE_UINT32:
      (*(uint32_t *)item->argptr) += (inc) ? 1 : -1;
      (*(uint32_t *)item->argptr) =
          CLAMP(0, (*(uint32_t *)item->argptr), 0xFFFFFFFF);
      break;
    case VAR_TYPE_FLOAT:
      (*(float *)item->argptr) += (inc) ? 1 : -1;
      (*(float *)item->argptr) =
          CLAMP(__FLT_MIN__, (*(float *)item->argptr), __FLT_MAX__);
      break;
    }
...
bool system_menu_action_edit(uint8_t action, system_menu_item_t *item) {
  ...
  case SYSTEM_MENU_ACTION_NEXT:
    if (flags & SYSTEM_MENU_MODE_MODIFY) {
      ...
      case VAR_TYPE_BOOLEAN:
        ...
        break;
      case VAR_TYPE_FLOAT:
        modifier = (action == SYSTEM_MENU_ACTION_NEXT)
                       ? powf(10.0f, (currentmult - 3))
                       : -powf(10.0f, (currentmult - 3));
        break;
      default:
        modifier = (action == SYSTEM_MENU_ACTION_NEXT)
                       ? powf(10.0f, currentmult)
                       : -powf(10.0f, currentmult);
        break;
      }
  ...
  case SYSTEM_MENU_ACTION_CHAR_INPUT('9'): {
    ...
    case VAR_TYPE_FLOAT:
      // Modify digit
      current_digit =
          (int)((*(float *)item->argptr) / powf(10.0f, (currentmult - 3))) % 10;
      (*(float *)item->argptr) +=
          (digit - current_digit) * powf(10.0f, (currentmult - 3));
      // Clamp
      (*(float *)item->argptr) =
          CLAMP(__FLT_MIN__, (*(float *)item->argptr), __FLT_MAX__);
    ...
    case VAR_TYPE_UINT8:
      // Modify digit
      current_digit =
          ((*(uint8_t *)item->argptr) / (int)powf(10.0f, currentmult)) % 10;
      (*(uint8_t *)item->argptr) +=
          (digit - current_digit) * powf(10.0f, currentmult);
      // Clamp
      (*(uint8_t *)item->argptr) = CLAMP(0, (*(uint8_t *)item->argptr), 0xFF);
    ...
    case VAR_TYPE_UINT16:
      current_digit =
          ((*(uint16_t *)item->argptr) / (int)powf(10.0f, currentmult)) % 10;
      (*(uint16_t *)item->argptr) +=
          (digit - current_digit) * powf(10.0f, currentmult);
      // Clamp
      (*(uint16_t *)item->argptr) =
    ...
    case VAR_TYPE_UINT32:
      // Modify digit
      current_digit =
          ((*(uint32_t *)item->argptr) / (int)powf(10.0f, currentmult)) % 10;
      (*(uint32_t *)item->argptr) +=
          (digit - current_digit) * powf(10.0f, currentmult);
      // Clamp
      (*(uint32_t *)item->argptr) =
 ...
 * These can be overriten by the display to perform the rendering of the menu
 * content
 * **/
...
system_menu_render_menu_item_filter(uint8_t item_index) {
  // filters if the menu item in an item page is to be printed (true) or not
  // (false)
  return true;
}

void __attribute__((weak))
system_menu_render_menu_item(uint8_t render_flags,
                             const system_menu_item_t *item) {
 ...
 * Helper µCNC render callbacks
 * **/
void __attribute__((weak)) system_menu_item_render_label(uint8_t item_index,
                                                         const char *label) {
  // this is were the display renders the item label
}

void __attribute__((weak)) system_menu_item_render_arg(uint8_t render_flags,
                                                       const char *label) {
  // this is were the display renders the item variable
}

void system_menu_item_render_var_arg(uint8_t render_flags,
                                     system_menu_item_t *item) {
...
static void system_menu_render_axis_position(uint8_t render_flags,
                                             system_menu_item_t *item) {
  if ((render_flags &
       (SYSTEM_MENU_MODE_SELECT | SYSTEM_MENU_MODE_SIMPLE_EDIT)) ==
      (SYSTEM_MENU_MODE_SELECT | SYSTEM_MENU_MODE_SIMPLE_EDIT)) {
    ...
    uint8_t axis_index =
        (axis_letter >= 'X') ? (axis_letter - 'X') : (3 + axis_letter - 'A');
...
void __attribute__((weak)) system_menu_action_custom_code(uint8_t action) {
  // used to implement custom action codes for different user interface systems
  // (keypads, keyboards, screens, etc..)
}

// allow modification of a page render after the page is initialized
void system_menu_set_render_callback(uint8_t menu_id,
                                     system_menu_page_render_cb callback) {
  MENU_LOOP(g_system_menu.menu_entry, menu_page) {
    if (menu_page->menu_id == menu_id) {
...
void system_menu_set_action_callback(uint8_t menu_id,
                                     system_menu_page_action_cb callback) {
  MENU_LOOP(g_system_menu.menu_entry, menu_page) {
    if (menu_page->menu_id == menu_id) {
