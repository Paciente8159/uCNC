...
static void flash_update_upload_cb(int client_idx) {
  ...
  if (up.status == HTTP_UPLOAD_START) {
    ...
    uint32_t maxflash =
        (!!flash_upd->get_flash_size) ? flash_upd->get_flash_size() : 0;
    if (maxflash < up.filelen) {
      // ESP_LOGI("OTA", "File size of %ld exceeds available space: %ld",
      // maxSketchSpace); Update.printError(Serial);
    ...
    if (!flash_upd->flash_write ||
        (flash_upd->flash_write(up.data, up.datalen) != up.datalen)) {
...
DECL_MODULE(flash_update) {
  ...
  http_add(update_uri, HTTP_REQ_ANY, flash_update_page_cb,
           flash_update_upload_cb);
#endif
}
