...
static const char fs_write_page[FS_WRITE_GZ_SIZE] __rom__
    __attribute__((unused)) = {
        };
...
static fs_file_t *fs_path_parse(fs_file_info_t *current_path,
                                const char *new_path, const char *mode) {
    ...
    if (token_len) {
      if (strncmp(token_start, ".", token_len) == 0) {
        ...
        if (tail) {
          ...
          *tail = 0;
          memset(&full_path[strlen(full_path)], 0,
                 FS_PATH_NAME_MAX_LEN - strlen(full_path));
        }
      } else {
  ...
  if (!strlen(fp_path)) {
    ...
    fp = (strcmp(mode, "d") == 0) ? fs->opendir(fp_path)
                                  : fs->open(fp_path, mode);
  }
...
static void fs_dir_list(void) {
  ...
  if (dir) {
    ...
    while (fs_next_file(dir, &finfo)) {
      if (finfo.is_dir) { /* It is a directory */
        proto_print("<dir>\t");
      } else { /* It is a file. */
        proto_print("     \t");
      }
...
void fs_file_run(char *params) {
  ...
  if (fp) {
    ...
    fs_running_file = fp;
    serial_stream_readonly(&running_file_getc, &running_file_available,
                           &running_file_clear);
    while (--startline) {
...
void fs_json_api(int client_idx) {
  fs_t *ptr = fs_default_drive;

  if (ptr) {
    char path[32];
    http_send(client_idx, 200, NULL, NULL, 0);
    http_send_str(client_idx, 200, "application/json",
                  "{\"result\":\"ok\",\"path\":\"\",\"data\":[");
    while (ptr) {
      memset(path, 0, sizeof(path));
      str_snprintf(path, 32, "{\"type\":\"drive\",\"name\":\"%c\"}",
                   ptr->drive);
      ptr = ptr->next;
      if (ptr) {
        // trailling comma
      ...
      http_send_str(client_idx, 200, "application/json", path);
    }
    http_send_str(client_idx, 200, "application/json", "]}\n");
    // close the stream
    http_send(client_idx, 200, "application/json", NULL, 0);
  } else {
    http_send_str(client_idx, 200, "application/json",
                  "{\"result\":\"ok\",\"path\":\"\",\"data\":[]}");
  }
...
void fs_file_json_api(int client_idx) {
  bool update = false;

  if (http_request_hasargs(client_idx)) {
    char arg = 0;
    http_request_arg(client_idx, "update", &arg, 1);
    if (arg && arg != '0') {
      update = true;
    }
  }
  ...
  // updated page
  if (update && http_request_method(client_idx) == HTTP_REQ_GET) {
    ...
    http_send(client_idx, 200, "text/html", (char *)updatepage,
              FS_WRITE_GZ_SIZE);
    return;
  }

  char urlpath[FS_PATH_NAME_MAX_LEN];
  memset(urlpath, 0, sizeof(urlpath));
  http_request_uri(client_idx, urlpath, FS_PATH_NAME_MAX_LEN);
  ...
  if (!fs_finfo(fs_url, &finfo)) {
    http_send_str(client_idx, 404, "application/json",
                  "{\"result\":\"notfound\"}");
    return;
  }
  ...
  switch (http_request_method(client_idx)) {
  case HTTP_REQ_DELETE:
    fs_remove(fs_url);
    __FALL_THROUGH__
  case HTTP_REQ_PUT:
  case HTTP_REQ_POST:
    http_request_arg(client_idx, "redirect", args, 128);
    if (strlen(args)) {
      http_send_header(client_idx, "Location", args, true);
      memset(urlpath, 0, sizeof(urlpath));
      str_snprintf(urlpath, FS_PATH_NAME_MAX_LEN, "{\"redirect\":\"%s\"}",
                   args);
      http_send_str(client_idx, 303, "application/json", urlpath);
    } else {
      http_send_str(client_idx, 200, "application/json", "{\"result\":\"ok\"}");
    }
  ...
  default: // handle as get
    ...
    if (strlen(finfo.full_name) || finfo.is_dir) {
      if (finfo.is_dir) {
        ...
        http_send_str(client_idx, 200, "application/json", "\",\"data\":[");
        fs_file_info_t child = {0};
        file = fs_opendir(fs_url);
        bool has_child = fs_next_file(file, &child);

        while (has_child) {
          memset(urlpath, 0, FS_PATH_NAME_MAX_LEN);
          if (child.is_dir) {
            str_snprintf(urlpath, FS_PATH_NAME_MAX_LEN,
                         "{\"type\":\"dir\",\"name\":\"%s\",\"attr\":%d}",
                         fs_filename(&child), 0);
          } else {
            str_snprintf(urlpath, FS_PATH_NAME_MAX_LEN,
                         "{\"type\":\"file\",\"name\":\"%s\",\"attr\":0,"
                         "\"size\":%d,\"date\":0}",
                         fs_filename(&child), child.size);
          }
          ...
          http_send_str(client_idx, 200, "application/json", urlpath);
        }
        http_send_str(client_idx, 200, "application/json", "]}\n");
        // close the stream
        http_send(client_idx, 200, "application/json", NULL, 0);
        ...
        uint8_t content[HTTP_MAX_CHUNCK_LEN / sizeof(char)];
        if (finfo.size > HTTP_MAX_CHUNCK_LEN) {
          http_send(client_idx, 200, NULL, NULL, 0);
        }
        while (fs_available(file)) {
          size_t content_len = fs_read(file, (uint8_t *)content,
                                       HTTP_MAX_CHUNCK_LEN / sizeof(char));
          http_send(client_idx, 200, "application/octet-stream",
      ...
      // is root dir
      fs_json_api(client_idx);
    }
    break;
  }
}

void fs_json_uploader(int client_idx) {
  static fs_file_t *file_upload = NULL;
  char urlpath[FS_PATH_NAME_MAX_LEN];
  memset(urlpath, 0, sizeof(urlpath));
  http_request_uri(client_idx, urlpath, FS_PATH_NAME_MAX_LEN);
  if ((strncmp(urlpath, FS_JSON_ENDPOINT, FS_JSON_ENDPOINT_LEN) != 0) ||
      (http_request_method(client_idx) != HTTP_REQ_POST &&
       http_request_method(client_idx) != HTTP_REQ_PUT)) {
    return;
  }
  ...
  http_upload_t upload = http_file_upload_status(client_idx);
  switch (upload.status) {
  case HTTP_UPLOAD_START:
    if (http_request_method(client_idx) == HTTP_REQ_POST) {
      uint8_t len = strlen(urlpath);
      if (urlpath[len - 1] != '/' && len < FS_PATH_NAME_MAX_LEN) {
        urlpath[len] = '/';
        len++;
      }
      // append the file name
      http_file_upload_name(client_idx, urlpath, FS_PATH_NAME_MAX_LEN - len);
    }
    file_upload = fs_open(file, "w");
    break;
  case HTTP_UPLOAD_PART:
...
void system_menu_render_fs_item(uint8_t render_flags,
    ...
    if (!strlen(fs_sm_cwd.full_name)) {
      ...
      while (drive) {
        if (system_menu_render_menu_item_filter(index)) {
          ...
          system_menu_item_render_label(
              render_flags |
                  ((cur_index == index) ? SYSTEM_MENU_MODE_SELECT : 0),
              buffer);
          if ((cur_index == index)) {
        ...
        if (dir->file_info.is_dir) {
          ...
          while (fs_next_file(dir, &finfo)) {
            if (system_menu_render_menu_item_filter(index)) {
              ...
              memcpy(&buffer[1], fs_filename(&finfo),
                     MIN(SYSTEM_MENU_MAX_STR_LEN - 1,
                         strlen(fs_filename(&finfo))));
              system_menu_item_render_label(
                  render_flags |
                      ((cur_index == index) ? SYSTEM_MENU_MODE_SELECT : 0),
                  buffer);
...
bool system_menu_fs_action(uint8_t action) {
  ...
  bool go_back =
      (g_system_menu.current_index < 0 || g_system_menu.current_multiplier < 0);
  ...
  if (action == SYSTEM_MENU_ACTION_SELECT) {
    ...
    if (render_flags & SYSTEM_MENU_MODE_EDIT) {
      ...
      if (go_back) {
        ...
        // run file
        fs_running_file =
            fs_path_parse(&fs_sm_cwd, fs_filename(&fs_pointed_file), "r");
        if (fs_running_file) {
          proto_feedback(FS_STR_FILE_RUNNING);
          system_menu_go_idle();
          rom_strcpy(buffer, __romstr__(FS_STR_FILE_RUNNING));
          system_menu_show_modal_popup(SYSTEM_MENU_MODAL_POPUP_MS, buffer);
          grbl_stream_readonly(&running_file_getc, &running_file_available,
                               &running_file_clear);
        } else {
      ...
      if (strlen(fs_pointed_file.full_name) != 0) {
        if (fs_pointed_file.is_dir) {
          fs_file_t *fp =
              fs_path_parse(&fs_sm_cwd, fs_filename(&fs_pointed_file), "d");
          if (fp) {
            fs_close(fp);
            fs_pointed_file = fs_sm_cwd;
            g_system_menu.current_index = 0;
...
void fs_mount(fs_t *drive) {
  ...
  RUNONCE {
    ...
    http_add(FS_JSON_ENDPOINT, HTTP_REQ_ANY, fs_file_json_api,
             fs_json_uploader);
    http_add(FS_JSON_ENDPOINT "/*", HTTP_REQ_ANY, fs_file_json_api,
             fs_json_uploader);
#endif
...
fs_file_t *fs_opendir(const char *path) {
  ...
  return fs_path_parse(NULL, newpath, "d");
}
