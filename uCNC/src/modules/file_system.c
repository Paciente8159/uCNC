...
void fs_json_api(int client_idx)
{
	...
	if (ptr)
	{
		...
		http_send_str(client_idx, 200, "application/json", "{\"result\":\"ok\",\"path\":\"\",\"data\":[");
		while (ptr)
		{
...
void fs_file_json_api(int client_idx)
{
	...
	{
		http_send_str(client_idx, 404, "application/json", "{\"result\":\"notfound\"}");
		return;
	}
	...
	case HTTP_REQ_POST:
		...
		if (strlen(args))
		{
			...
			memset(urlpath, 0, sizeof(urlpath));
			str_snprintf(urlpath, FS_PATH_NAME_MAX_LEN, "{\"redirect\":\"%s\"}", args);
		...
		if (strlen(finfo.full_name) || finfo.is_dir)
		{
			if (finfo.is_dir)
			{
				...
				while (has_child)
				{
					...
					if (child.is_dir)
					{
						str_snprintf(urlpath, FS_PATH_NAME_MAX_LEN, "{\"type\":\"dir\",\"name\":\"%s\",\"attr\":%d}", fs_filename(&child), 0);
					}
					else
					{
						str_snprintf(urlpath, FS_PATH_NAME_MAX_LEN, "{\"type\":\"file\",\"name\":\"%s\",\"attr\":0,\"size\":%d,\"date\":0}", fs_filename(&child), child.size);
					}
			...
			else
			{
				...
				{
					size_t content_len = fs_read(file, (uint8_t *)content, HTTP_MAX_CHUNCK_LEN / sizeof(char));
...
void fs_json_uploader(int client_idx)
{
	...
	memset(urlpath, 0, sizeof(urlpath));
	http_request_uri(client_idx, urlpath, FS_PATH_NAME_MAX_LEN);
	if ((strncmp(urlpath, FS_JSON_ENDPOINT, FS_JSON_ENDPOINT_LEN) != 0) || (http_request_method(client_idx) != HTTP_REQ_POST && http_request_method(client_idx) != HTTP_REQ_PUT))
	{
		return;
	}
