/*
        Name: atomic.h
        Description: Some useful macros for thread syncronzation and atomic
   operations.
...
#define ATOMIC_CODEBLOCK                                                       \
  for (bool __restore_atomic__                                                 \
       __attribute__((__cleanup__(__atomic_out))) = mcu_get_global_isr(),      \
       __AtomLock = __atomic_in();                                             \
       __AtomLock; __AtomLock = false)
#endif
#ifndef ATOMIC_CODEBLOCK_NR
#define ATOMIC_CODEBLOCK_NR                                                    \
  for (bool __restore_atomic__                                                 \
       __attribute__((__cleanup__(__atomic_out_on))) = mcu_get_global_isr(),   \
       __AtomLock = __atomic_in();                                             \
       __AtomLock; __AtomLock = false)
#endif
...
// #define ATOMIC_STORE_N(ptr, val) __atomic_store_n((ptr), (val),
// __ATOMIC_RELEASE) #define ATOMIC_COMPARE_EXCHANGE_N(ptr, expected, desired)
// __atomic_compare_exchange_n((ptr), &(expected), (desired), false,
// __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE) #define ATOMIC_FETCH_ADD(T, ptr, val)
// __atomic_fetch_add((ptr), (val), __ATOMIC_ACQ_REL) #define ATOMIC_SUB(ptr,
// val) __atomic_fetch_sub((ptr), (val), __ATOMIC_ACQ_REL)
#ifdef __GNUC__
#ifndef ATOMIC_LOAD_N
#define ATOMIC_LOAD_N(src, mode)                                               \
  ({                                                                           \
    __typeof__(*(src)) _res;                                                   \
    ATOMIC_CODEBLOCK { _res = *(src); }                                        \
    _res;                                                                      \
  })
#endif
...
#define ATOMIC_FETCH_XOR(dst, val, mode)                                       \
  ({                                                                           \
    __typeof__(*(dst)) __tmp;                                                  \
    ATOMIC_CODEBLOCK {                                                         \
      __tmp = *(dst);                                                          \
      MEM_BARRIER;                                                             \
      *(dst) = __tmp ^ (val);                                                  \
...
#define ATOMIC_FETCH_ADD(dst, val, mode)                                       \
  ({                                                                           \
    __typeof__(*(dst)) __tmp;                                                  \
    ATOMIC_CODEBLOCK {                                                         \
      __tmp = *(dst);                                                          \
...
#define ATOMIC_FETCH_SUB(dst, val, mode)                                       \
  ({                                                                           \
    __typeof__(*(dst)) __tmp;                                                  \
    ATOMIC_CODEBLOCK {                                                         \
      __tmp = *(dst);                                                          \
...
#define ATOMIC_FETCH_AND(dst, val, mode)                                       \
  ({                                                                           \
    __typeof__(*(dst)) __tmp;                                                  \
    ATOMIC_CODEBLOCK {                                                         \
      __tmp = *(dst);                                                          \
...
#define ATOMIC_FETCH_OR(dst, val, mode)                                        \
  ({                                                                           \
    __typeof__(*(dst)) __tmp;                                                  \
    ATOMIC_CODEBLOCK {                                                         \
      __tmp = *(dst);                                                          \
...
#define BIN_SEMPH_INIT(name, locked)                                           \
  ({                                                                           \
    ATOMIC_TYPE name##_semph_temp = BIN_SEMPH_UNDEF;                           \
    ATOMIC_COMPARE_EXCHANGE_N(&name, &name##_semph_temp, (locked),             \
                              __ATOMIC_RELEASE, __ATOMIC_RELAXED);             \
  })
#define BIN_SEMPH_UNLOCK(name)                                                 \
  ({                                                                           \
    ATOMIC_TYPE name##_semph_temp = BIN_SEMPH_LOCKED;                          \
    ATOMIC_COMPARE_EXCHANGE_N(&name, &name##_semph_temp, BIN_SEMPH_UNLOCKED,   \
                              __ATOMIC_RELEASE, __ATOMIC_RELAXED);             \
  })

static FORCEINLINE bool semph_try_safe_lock(ATOMIC_TYPE *lock) {
  ATOMIC_TYPE expected = BIN_SEMPH_UNLOCKED;
  return ATOMIC_COMPARE_EXCHANGE_N(lock, &expected, BIN_SEMPH_LOCKED,
                                   __ATOMIC_RELEASE, __ATOMIC_RELAXED);
}
