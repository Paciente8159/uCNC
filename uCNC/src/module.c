...
void mod_init(void) {
  ...
  // 	// by setting the destination to NULL it will not reload g_settings
  // again to avoid re-reading this area again
...
bool mod_event_default_handler(mod_delegate_event_t **event,
  ...
  while (ptr != NULL) {
    *last = ptr->next;
    if (ptr->fptr != NULL &&
        !CHECKFLAG(ptr->fplock, (g_module_lockguard | LISTENER_RUNNING_LOCK))) {
