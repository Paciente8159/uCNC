/*
        ...
        Description: Implements all kinematics math equations to translate the
   motion of a coreXY machine. Also implements the homing motion for this type
   of machine.
...
#if (COREXY_AXIS == COREXY_AXIS_XZ)
  axis[AXIS_X] =
      (float)(0.5f * (float)(steps[0] + steps[2]) / g_settings.step_per_mm[0]);
  axis[AXIS_Z] =
      (float)(0.5f * (float)(steps[0] - steps[2]) / g_settings.step_per_mm[2]);
  axis[AXIS_Y] = (((float)steps[1]) / g_settings.step_per_mm[1]);
#elif (COREXY_AXIS == COREXY_AXIS_YZ)
  axis[AXIS_X] = (((float)steps[0]) / g_settings.step_per_mm[0]);
  axis[AXIS_Y] =
      (float)(0.5f * (float)(steps[1] + steps[2]) / g_settings.step_per_mm[1]);
  axis[AXIS_Z] =
      (float)(0.5f * (float)(steps[1] - steps[2]) / g_settings.step_per_mm[2]);
#else
  axis[AXIS_X] =
      (float)(0.5f * (float)(steps[0] + steps[1]) / g_settings.step_per_mm[0]);
  axis[AXIS_Y] =
      (float)(0.5f * (float)(steps[0] - steps[1]) / g_settings.step_per_mm[1]);
#endif
