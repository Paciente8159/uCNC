/*
   ...
   translate the motion of a scara machine. Also implements the homing motion
...
void kinematics_apply_inverse(float *axis, int32_t *steps) {
  ...
  float distance = (axis[AXIS_X] * axis[AXIS_X] + axis[AXIS_Y] * axis[AXIS_Y] -
                    arm * arm - forearm * forearm) /
                   (2.0f * arm * forearm);
  float angle2 = acosf(distance);
  float angle1 = atan2f(axis[AXIS_Y], axis[AXIS_X]) -
                 atan2f(forearm * sin(angle2), (arm + forearm * distance));
  ...
  steps[0] =
      (int32_t)roundf(angle1 * DOUBLE_PI_INV * g_settings.step_per_mm[0]);
  steps[1] =
      (int32_t)roundf(angle2 * DOUBLE_PI_INV * g_settings.step_per_mm[1]);
...
uint8_t kinematics_home(void) {
  ...
  steps_homing[0] = g_settings.scara_arm_homing_angle *
                    g_settings.step_per_mm[0] * FULL_TURN_INV;
  steps_homing[1] = g_settings.scara_forearm_homing_angle *
                    g_settings.step_per_mm[1] * FULL_TURN_INV;
