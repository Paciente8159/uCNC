/*
        Name: kinematic.h
        Description: Declares the functions needed to implement the machine
   kinematics and homing motion. This defines an opac interface that allows to
   adapt µCNC to different architectures/mechanics of different machines.
...
// this should match the number of linear actuators on the machines (do not
// change unless you know what you are doing) laser PPI requires an additional
// stepper
...
#define KINEMATIC_HOMING_ERROR_XY                                              \
  (KINEMATIC_HOMING_ERROR_X | KINEMATIC_HOMING_ERROR_Y)
 ...
 * This is done after computing position relative to the active coordinate
 * system
 *
 ...
 * In the kinematics home function the axis order of the homing motion and other
 * custom motions can be defined
 *
 ...
 * This is aplied only on normal and jog moves. Homing motions go directly to
 * planner.
 *
...
/**
 * @brief Aplies a reverse transformation to the position returned from the
 * planner. This is aplied only on normal and jog moves. Homing motions go
 * directly to planner.
 *
 * @param axis Target in absolute coordinates
 */
...
/**
 * @brief Converts from machine absolute coordinates to step position.
 * This calls kinematics_apply_inverse after applying any custom geometry
 * transformation (like skew compensation)
 *
 * @param axis Position in world coordinates
 * @param steps Position in steps
 */
...
/**
 * @brief Converts from step position to machine absolute coordinates.
 * This calls kinematics_apply_forward and then recomputes any custom geometry
 * transformation inversion (like skew compensation)
 *
 * @param steps Position in steps
 * @param axis Position in world coordinates
 */
...
//  * This internally calls kinematics_check_boundaries after applying machine
//  transformations
//  * This ensure that in a transformation makes an axis travell past it's
//  limits
//  *
//  * @param axis Target in absolute coordinates
//  * @return true If inside boundries
