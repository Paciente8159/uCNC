...
void kinematics_init(void) {
  ...
  delta_base_height =
      sqrtf(delta_arm_sqr - delta_x[0] * delta_x[0] - delta_y[0] * delta_y[0]);
  float min_travel =
      (g_settings.delta_arm_length * cos(DELTA_ARM_MIN_ANGLE * DEG_RAD_MULT)) -
      delta_triang_base;
  float max_travel =
      (g_settings.delta_arm_length * cos(DELTA_ARM_MAX_ANGLE * DEG_RAD_MULT)) -
      delta_triang_base;
...
void kinematics_apply_forward(int32_t *steps, float *axis) {
  ...
  // Subtract the X component from the original vector leaving only Y. We use
  // the variable that will be the unit vector after we scale it.
  ...
  float ez[3] = {ex[1] * ey[2] - ex[2] * ey[1], ex[2] * ey[0] - ex[0] * ey[2],
                 ex[0] * ey[1] - ex[1] * ey[0]};
...
bool kinematics_check_boundaries(float *axis) {
  ...
  if (axis[AXIS_X] < -delta_base_max_travel ||
      axis[AXIS_X] > delta_base_max_travel) {
#ifdef ALLOW_SOFT_LIMIT_JOG_MOTION_CLAMPING
    axis[AXIS_X] =
        CLAMP(-delta_base_max_travel, axis[AXIS_X], delta_base_max_travel);
#endif
    return false;
  }

  if (axis[AXIS_Y] < -delta_base_max_travel ||
      axis[AXIS_Y] > delta_base_max_travel) {
#ifdef ALLOW_SOFT_LIMIT_JOG_MOTION_CLAMPING
    axis[AXIS_Y] =
        CLAMP(-delta_base_max_travel, axis[AXIS_Y], delta_base_max_travel);
#endif
    return false;
  }
