/*
   ...
   translate the motion of a r-theta machine. Also implements the homing motion
...
void kinematics_apply_inverse(float *axis, int32_t *steps) {
  ...
  float distance =
      sqrt(axis[AXIS_X] * axis[AXIS_X] + axis[AXIS_Y] * axis[AXIS_Y]);

  steps[0] = (int32_t)roundf(theta_reduction_ratio * angle * DOUBLE_PI_INV *
                             g_settings.step_per_mm[0]);
...
uint8_t kinematics_home(void) {
  ...
  steps_homing[0] = g_settings.rtheta_theta_homing_angle *
                    g_settings.step_per_mm[0] * FULL_TURN_INV;
  steps_homing[1] = g_settings.rthera_arm_homing_distance *
