...
void kinematics_coordinates_to_steps(float *axis, int32_t *steps) {
  ...
  // In homing mode no kinematics modifications is applied to prevent unwanted
  // axis movements
  if (!cnc_get_exec_state(EXEC_HOMING)) {
    ...
    axis_copy[AXIS_X] -= axis_copy[AXIS_Z] * (g_settings.skew_xy_factor -
                                              g_settings.skew_xz_factor *
...
//         float axis_copy[AXIS_COUNT];
//         memcpy(axis_copy, axis, sizeof(axis_copy));
//         // In homing mode no kinematics modifications is applied to prevent
//         unwanted axis movements if (!cnc_get_exec_state(EXEC_HOMING))
//         {
//                 kinematics_apply_transform(axis_copy);

// #ifdef ENABLE_SKEW_COMPENSATION
//                 // apply correction skew factors that compensate for machine
//                 axis alignemnt axis_copy[AXIS_X] -= axis_copy[AXIS_Y] *
//                 g_settings.skew_xy_factor;
// #ifndef SKEW_COMPENSATION_XY_ONLY
//                 axis_copy[AXIS_X] -= axis_copy[AXIS_Z] *
//                 (g_settings.skew_xy_factor - g_settings.skew_xz_factor *
//                 g_settings.skew_yz_factor); axis_copy[AXIS_Y] -=
//                 axis_copy[AXIS_Z] * g_settings.skew_yz_factor;
// #endif
// #endif
//         }
