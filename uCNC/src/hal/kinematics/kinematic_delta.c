/*
   ...
   translate the motion of a delta machine. Also implements the homing motion
...
static void delta_calc_bounds(void) {
  float maxx = -g_settings.delta_effector_radius -
               g_settings.delta_base_radius - g_settings.delta_forearm_length -
               g_settings.delta_bicep_length;
  float maxz = maxx;
  float minz = -maxx;
  float s = MAX(g_settings.step_per_mm[0],
                MAX(g_settings.step_per_mm[1], g_settings.step_per_mm[2]));
  ...
  do {
    ...
    if (r[0][0] == INT32_MAX || r[1][0] == INT32_MAX || r[2][0] == INT32_MAX ||
        r[3][0] == INT32_MAX || r[4][0] == INT32_MAX || r[5][0] == INT32_MAX ||
        r[6][0] == INT32_MAX || r[7][0] == INT32_MAX) {
      sum -= dist;
      dist *= 0.5;
    } else {
      minz = middlez - sum;
...
void kinematics_init(void) {
  ...
  delta_base_effector_half_f_tg30 =
      HALF_TAN30 *
      (g_settings.delta_base_radius - g_settings.delta_effector_radius);
...
int8_t delta_calcAngleYZ(float x0, float y0, float z0, float *theta) {
  ...
  // z = a + b*y
  float a = fast_flt_div2(
                (x0 * x0 + y0 * y0 + z0 * z0 + rf * rf - re * re - y1 * y1)) /
            z0;
  float b = (y1 - y0) / z0;
  // discriminant
  ...
  float zj = a + b * yj;
  // original code
  // *theta = 180.0f * atan(-zj / (y1 - yj)) * M_PI_INV + ((yj > y1) ? 180.0f :
  // 0);
...
void kinematics_apply_inverse(float *axis, int32_t *steps) {
  ...
  if (!delta_calcAngleYZ(axis[AXIS_X], axis[AXIS_Y], z_offset, &theta1)) {
    if (!delta_calcAngleYZ(axis[AXIS_X] * COS120 + axis[AXIS_Y] * SIN120,
                           axis[AXIS_Y] * COS120 - axis[AXIS_X] * SIN120,
                           z_offset, &theta2)) {
      if (!delta_calcAngleYZ(axis[AXIS_X] * COS120 - axis[AXIS_Y] * SIN120,
                             axis[AXIS_Y] * COS120 + axis[AXIS_X] * SIN120,
                             z_offset, &theta3)) {
...
void kinematics_apply_forward(int32_t *steps, float *axis) {
  ...
  float c = (b2 - y1 * dnm) * (b2 - y1 * dnm) + b1 * b1 +
            dnm * dnm * (z1 * z1 - re * re);
...
uint8_t kinematics_home(void) {
  // delta starts by invalidating the current position and considers it's at the
  // far end of the homing position
  float axis[AXIS_COUNT];
  ...
  axis[AXIS_Z] = ((g_settings.homing_dir_invert_mask & (1 << AXIS_Z))
                      ? delta_cuboid_z_min
                      : delta_cuboid_z_max);
  ...
  // sync systems again to the origin (interpolator, motion control and parser -
  // the latest is synched ny motion control)
...
bool kinematics_check_boundaries(float *axis) {
  if (!g_settings.soft_limits_enabled || cnc_get_exec_state(EXEC_HOMING)) {
    // modifies the homing seek position to not exceed the maximum distance that
    // the maxine can run
  ...
  if (axis[AXIS_Z] < (delta_cuboid_z_min - z_offset) ||
      axis[AXIS_Z] > (delta_cuboid_z_max - z_offset)) {
#ifdef ALLOW_SOFT_LIMIT_JOG_MOTION_CLAMPING
    axis[AXIS_Z] = CLAMP((delta_cuboid_z_min - z_offset), axis[AXIS_Z],
                         (delta_cuboid_z_max - z_offset));
#endif
    return false;
  }
