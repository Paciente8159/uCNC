...
static uint32_t
    eeprom_data[EEPROM_DATA_SIZE >> 2]; // loads a full page into RAM
uint8_t *const eeprom_ptr = (uint8_t *)eeprom_data;
static uint16_t eeprom_current_page;
static uint8_t eeprom_status;

#define EEPROM_FLASH_SECTORS                                                   \
  ((uint16_t)((NVM_STORAGE_SIZE_ALIGNED - 1) / SPI_FLASH_SEC_SIZE) + 1)
#define EEPROM_MAX_PAGES                                                       \
  ((uint16_t)((SPI_FLASH_SEC_SIZE - 1) / NVM_STORAGE_SIZE_ALIGNED) + 1)
#define EEPROM_FLASH_BASE_SECTOR                                               \
  ((EEPROM_start - 0x40200000) / SPI_FLASH_SEC_SIZE)
#define EEPROM_SECTOR(offset)                                                  \
  (EEPROM_FLASH_BASE_SECTOR + (((offset) / SPI_FLASH_SEC_SIZE)))
#define EEPROM_FLASH_ADDR(sector, page, offset)                                \
  ((EEPROM_FLASH_BASE_SECTOR * SPI_FLASH_SEC_SIZE * (sector + 1)) +            \
   (page) * NVM_STORAGE_SIZE_ALIGNED + (offset))
#define EEPROM_SECTOR_ADDR(page, offset)                                       \
  (((page) * NVM_STORAGE_SIZE_ALIGNED + (offset)) & (SPI_FLASH_SEC_SIZE - 1))
...
static uint16_t mcu_access_flash_page(uint16_t address) {
  ...
  if (eeprom_current_sector != sector) {
    ...
    ATOMIC_CODEBLOCK {
      spi_flash_read(EEPROM_FLASH_ADDR(sector, 0, 0), eeprom_data,
                     EEPROM_DATA_SIZE);
    }
  }
...
void mcu_eeprom_init(void) {
  ...
  for (uint16_t i = 0; i < EEPROM_MAX_PAGES; i++) {
    ...
    if (tmp[0] == FLASH_VALUE('V')) {
      ...
      if (i == 0) {
        ...
        // jump to last that means that all memory is erased and the first index
  ...
  ATOMIC_CODEBLOCK {
    spi_flash_read(EEPROM_FLASH_ADDR(0, eeprom_current_page, 0), eeprom_data,
                   EEPROM_DATA_SIZE);
  }
#endif
}
...
void mcu_eeprom_putc(uint16_t address, uint8_t value) {
  ...
  if (eeprom_ptr[data_address] != value) {
    if (((eeprom_ptr[data_address] ^ value) &
         value)) // bits have changed and need to return to 1
    {
      eeprom_status |= EEPROM_NEWPAGE_REQUIRED;
    }
...
void mcu_eeprom_flush(void) {
  ...
  if (eeprom_status & EEPROM_NEWPAGE_REQUIRED) {
    ...
    if (needs_erase) {
      ...
      for (uint16_t i = 0; i < EEPROM_FLASH_SECTORS;
           i++) { // erases the sector if needed

        DBGMSG("erasing sector %u\n", i);
        ATOMIC_CODEBLOCK {
          if (spi_flash_erase_sector(EEPROM_FLASH_BASE_SECTOR + i) !=
              SPI_FLASH_RESULT_OK) {
            DBGMSG("erase error");
            return;
          }
  ...
  if (eeprom_status & EEPROM_MODIFIED) {
    for (uint16_t i = 0; i < EEPROM_FLASH_SECTORS;
      ...
      ATOMIC_CODEBLOCK {
        if (spi_flash_write(EEPROM_FLASH_ADDR(i, eeprom_current_page, 0),
                            eeprom_data,
                            EEPROM_DATA_SIZE) != SPI_FLASH_RESULT_OK) {
          DBGMSG("write error");
        }
      }
    }
