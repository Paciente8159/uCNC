 ...
 *
 * Deliberately, poll() does not call yield()/delay()/SDK task dispatch. Doing
 * so would violate the socket_device_t bounded non-blocking contract. lwIP
 * input, ACK processing and timers remain owned by the Arduino/NONOS SDK
 * service path.
 */
...
#define ESP8266_SOCKET_TOTAL_SLOTS                                             \
  ((uint32_t)MAX_SOCKETS + (uint32_t)SOCKET_MAX_CONNECTIONS)

#if (MAX_SOCKETS + SOCKET_MAX_CONNECTIONS) >= UINT16_MAX
#error "ESP8266 socket listener + client count must be less than 65535"
#endif

enum { ESP8266_LISTENER_FREE = 0, ESP8266_LISTENER_ACTIVE };
...
static void esp8266_client_set_state(esp8266_client_t *client, uint8_t state) {
  client->status = (uint8_t)((client->status & ~ESP8266_CLIENT_STATE_MASK) |
                             (state & ESP8266_CLIENT_STATE_MASK));
}

static err_t esp8266_accept_callback(void *arg, struct tcp_pcb *newpcb,
                                     err_t error);
static err_t esp8266_recv_callback(void *arg, struct tcp_pcb *pcb,
...
static bool esp8266_decode_handle(socket_device_handle_t handle,
                                  uint16_t *global_slot, uint16_t *generation) {
...
static esp8266_client_t *
esp8266_client_from_handle(socket_device_handle_t handle) {
  uint16_t slot;
  uint16_t generation;
...
static esp8266_listener_t *esp8266_allocate_listener(void) {
  for (uint16_t i = 0; i < MAX_SOCKETS; ++i) {
    ...
    if (listener->state == ESP8266_LISTENER_FREE) {
      listener->generation = esp8266_next_generation(listener->generation);
      listener->pcb = NULL;
...
static esp8266_client_t *esp8266_allocate_client(void) {
  for (uint16_t i = 0; i < SOCKET_MAX_CONNECTIONS; ++i) {
    ...
    if (esp8266_client_state(client) == ESP8266_CLIENT_FREE) {
      uint16_t generation = esp8266_next_generation(client->generation);
...
static void esp8266_finish_client(esp8266_client_t *client, int reason) {
  ...
  if (token != SOCKET_DEVICE_INVALID_TOKEN) {
    esp8266_events->closed(token, (reason < 0) ? reason : SOCKET_DEVICE_ERROR);
  }
}
...
static bool esp8266_append_rx(esp8266_client_t *client, struct pbuf *p) {
  ...
  if (incoming == 0U || (client->rx && (uint32_t)client->rx->tot_len >
                                           (uint32_t)UINT16_MAX - incoming)) {
    return false;
  }
...
static err_t esp8266_recv_callback(void *arg, struct tcp_pcb *pcb,
  ...
  if (!p) {
    esp8266_mark_close(client, (error == ERR_OK)
...
static err_t esp8266_accept_callback(void *arg, struct tcp_pcb *newpcb,
                                     err_t error) {
  esp8266_listener_t *listener = esp8266_listener_from_callback_arg(arg);
  esp8266_client_t *client;
...
static socket_device_handle_t
esp8266_socket_listen(const socket_device_endpoint_t *endpoint,
                      uint8_t backlog) {
  esp8266_listener_t *listener;
  struct tcp_pcb *pcb;
...
static int esp8266_socket_recv(socket_device_handle_t handle, void *destination,
                               size_t capacity) {
...
static int esp8266_socket_send(socket_device_handle_t handle,
  ...
  error = tcp_write(client->pcb, source, attempt, TCP_WRITE_FLAG_COPY);
  if (error != ERR_OK) {
...
static void esp8266_accept_in_owner_context(esp8266_client_t *client) {
  socket_device_handle_t listener_handle = client->association.listener_handle;
  socket_device_token_t token;

  if (client->pcb && (client->status & ESP8266_CLIENT_BACKLOG_DELAYED) != 0U) {
    tcp_backlog_accepted(client->pcb);
    client->status &= (uint8_t)~ESP8266_CLIENT_BACKLOG_DELAYED;
  }

  token =
      esp8266_events->accepted(listener_handle, esp8266_client_handle(client));
  if (token == SOCKET_DEVICE_INVALID_TOKEN) {
...
static void esp8266_socket_poll(uint16_t budget) {
  ...
  while (inspected < SOCKET_MAX_CONNECTIONS && emitted < budget) {
    ...
    esp8266_poll_cursor =
        (uint16_t)((esp8266_poll_cursor + 1U) % SOCKET_MAX_CONNECTIONS);
    ++inspected;
    ...
    if ((client->status & ESP8266_CLIENT_CLOSE_PENDING) != 0U && !client->rx) {
      int reason = client->close_reason;
      esp8266_finish_client(client, reason);
      ++emitted;
      continue;
...
socket_device_t wifi_socket = {.init = esp8266_socket_init,
