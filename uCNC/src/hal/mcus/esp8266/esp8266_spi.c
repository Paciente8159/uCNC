...
#define ESP8266_SPI_CLK(PRE, CNT)                                              \
  ((PRE & SPICLKDIVPRE) << SPICLKDIVPRE_S) |                                   \
      ((CNT & SPICLKCN) << SPICLKCN_S) |                                       \
      (((CNT ? (((CNT + 1) >> 1) - 1) : 0) & SPICLKCH) << SPICLKCH_S) |        \
      ((CNT & SPICLKCL) & SPICLKCL_S)
void mcu_spi_config(spi_config_t config, uint32_t frequency) {
#if SPI_PORT == 0
  IOSWAP |= (1 << IOSWAP2CS);
  // SPI0E3 |= 0x1; This is in the MP3_DECODER example, but makes the WD kick in
  // here.
  SPI1E3 |= 0x3;
