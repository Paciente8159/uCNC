/*
        ...
        Description: Implements Wifi and support services (including webserver,
   sockets and flash file system) for ESP8266 using Arduino libraries.
...
size_t esp8266_get_flash_size(void) {
  return (size_t)((ESP.getFreeSketchSpace() - 0x1000) & 0xFFFFF000);
}

bool esp8266_flash_begin(size_t filesize) {
#ifdef FLASH_FS
  if (!FLASH_FS.begin()) {
    return false;
  }
...
bool esp8266_flash_end(bool flush) {
...
size_t esp8266_flash_write(uint8_t *data, size_t len) {
...
static flash_udpate_t esp8266_flashupdate = {.get_flash_size =
                                                 esp8266_get_flash_size,
                                             .flash_begin = esp8266_flash_begin,
                                             .flash_write = esp8266_flash_write,
                                             .flash_end = esp8266_flash_end,
                                             .device_restart = esp8266_restart};
}
#endif
...
#ifdef MCU_HAS_FLASHUPDATE
  flash_update_register(&esp8266_flashupdate);
#endif
