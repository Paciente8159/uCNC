 ...
 * The ESP8266 has a UART FIFO. Should be enough to coupled with the Software
 * buffer and the mcu task loop
 * */
...
/**
 * ISR implementation (not used)
 * The ESP8266 has a UART2 FIFO. Should be enough to coupled with the Software
 * buffer and the mcu task loop
 * */
...
void mcu_uart_init(void) {
  ...
  // USIC(UART_PORT) = 0xffff;
  // USIE(UART_PORT) = (1 << UIFF) | (1 << UIOF) | (1 << UIFR) | (1 << UIPE) |
  // (1 << UITO); ETS_UART_INTR_ATTACH(mcu_uart_isr, NULL);
