/*
                ...
                Description: Implements mcu interface on AVR.
                                Besides all the functions declared in the mcu.h
   it also implements the code responsible for handling: interpolator.h void
   mcu_step_cb(); void mcu_step_reset_cb(); serial.h void mcu_com_rx_cb(uint8_t
   c); trigger_control.h void mcu_limits_changed_cb(); void
   mcu_controls_changed_cb(); void mcu_probe_changed_cb(uint8_t probe);
...
// naked ISR to reduce impact since doen't need to change any register (just an
// interrupt mask and pin outputs)
ISR(RTC_COMPB_vect, ISR_NOBLOCK) { mcu_clear_servos(); }
#endif
...
ISR(ITP_COMPA_vect, ISR_BLOCK) {
  ...
  ITP_TIFR = 2; // clears pending OCIEA overflow ISR (this prevents abnomal
                // short step pulses)
  ITP_TIMSK |= (1 << ITP_OCIEA);
}

ISR(ITP_COMPB_vect, ISR_BLOCK) { mcu_step_reset_cb(); }
...
// 		mcu_step_reset_cb();
// 	}
// 	// this is necessary to allow minimum CPU time to run the itp_run to
// feed the step isr 	ITP_TCNT = 0; 	MEM_BARRIER; 	resetstep = !resetstep;
// 	MEM_BARRIER;
// 	ITP_TIMSK |= (1 << ITP_OCIEA);
// }
...
void mcu_uart_init() {
  ...
  UCSRA_REG |=
      (1 << U2X_BIT); // baud doubler on for high baud rates, i.e. 115200
#endif
...
void mcu_uart2_init() {
  ...
  UCSRA_REG_2 &=
      ~(1 << U2X_BIT_2); // baud doubler off  - Only needed on Uno XXX
#else
  UBRR_value = (F_CPU / (8UL * BAUDRATE2)) - 1;
  UCSRA_REG_2 |=
      (1 << U2X_BIT_2); // baud doubler on for high baud rates, i.e. 115200
#endif
...
#ifndef FORCE_SOFT_POLLING
#ifdef PCIE0
#if ((PCINT0_LIMITS_MASK | PCINT0_CONTROLS_MASK | PROBE_ISR0 |                 \
      PCINT0_DIN_IO_MASK) != 0)
  SETBIT(PCICR, PCIE0);
#else
...
#endif
#ifdef PCIE1
#if ((PCINT1_LIMITS_MASK | PCINT1_CONTROLS_MASK | PROBE_ISR1 |                 \
      PCINT1_DIN_IO_MASK) != 0)
  SETBIT(PCICR, PCIE1);
#else
...
#endif
#ifdef PCIE2
#if ((PCINT2_LIMITS_MASK | PCINT2_CONTROLS_MASK | PROBE_ISR2 |                 \
      PCINT2_DIN_IO_MASK) != 0)
  SETBIT(PCICR, PCIE2);
#else
...
#endif
#ifdef PCIE3
#if ((PCINT3_LIMITS_MASK | PCINT3_CONTROLS_MASK | PROBE_ISR3 |                 \
      PCINT3_DIN_IO_MASK) != 0)
  SETBIT(PCICR, PCIE3);
#else
...
void mcu_stop_itp_isr(void) {
  ITP_TCCRB = 0;
  ITP_TIFR = 7;
  // do not disable the ISR Mask. this is used by the RTC timer to check if the
  // callback can run ITP_TIMSK &= ~((1 << ITP_OCIEB) | (1 << ITP_OCIEA));
}
...
void mcu_eeprom_putc(uint16_t address, uint8_t value) {
  ...
  do {
  } while (EECR & (1 << EEPE)); // Wait for completion of previous write before
                                // enabling interrupts.
...
static uint8_t mcu_i2c_write(uint8_t data, bool send_start, bool send_stop,
  ...
  if (send_start) {
    ...
    if ((TW_STATUS == TW_BUS_ERROR) ||
        CHECKFLAG(TWCR, ((1 << TWSTA) | (1 << TWSTO)))) {
...
ISR(TWI_vect, ISR_BLOCK) {
  ...
  case TW_ST_DATA_ACK: // byte sent, ack returned
    ...
    if (i >= datalen) {
      datalen = 0;
      TWCR =
          ((1 << TWIE) | (1 << TWINT) | (1 << TWEN)); // on last byte send NACK
      return;
    }
    index = i;
    break;
