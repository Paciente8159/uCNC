...
static esp_err_t spi_ctx_init_bus(spi_ctx_t *ctx, bool dma_enable) {
  ...
  buscfg.max_transfer_sz =
      dma_enable ? SPI_ASYNC_DMA_CHUNK_MAX : SPI_ASYNC_NODMA_CHUNK_MAX;

  err =
      spi_bus_initialize(ctx->host, &buscfg, dma_enable ? SPI_DMA_CH_AUTO : 0);
  if (err != ESP_OK) {
    ctx->bus_initialized = false;
    ctx->bus_dma_enabled = false;
    return err;
...
static esp_err_t spi_ctx_apply_device(spi_ctx_t *ctx, spi_config_t config,
                                      uint32_t frequency) {
  esp_err_t err;
  ...
  if (ctx->dev && (ctx->current_cfg.flags == config.flags) &&
...
static uint16_t spi_ctx_prepare_chunk(spi_ctx_t *ctx) {
  ...
  uint16_t max_chunk = ctx->bulk_dma ? (uint16_t)SPI_ASYNC_DMA_CHUNK_MAX
                                     : (uint16_t)SPI_ASYNC_NODMA_CHUNK_MAX;
...
static bool spi_ctx_bulk_transfer(spi_ctx_t *ctx, const uint8_t *out,
  ...
  if (ctx->bulk_dma) {
    if (!ctx->bulk_queued) {
      ...
      if (ctx->bulk_inplace && (ctx->bulk_in != NULL) &&
          (ctx->bulk_chunk_len != 0U)) {
        memcpy(ctx->bulk_in + ctx->bulk_pos, ctx->rx_stage,
               ctx->bulk_chunk_len);
      }
    ...
    for (i = 0; i < chunk; i++) {
      ...
      if (ctx->bulk_out != NULL) {
        txb = ctx->bulk_inplace ? ctx->tx_stage[i]
                                : ctx->bulk_out[ctx->bulk_pos + i];
      }
...
void mcu_spi_stop(void) {
  if (spi_ctx.bulk_dma && spi_ctx.bulk_queued && spi_ctx.dev) {
    spi_transaction_t *ret_trans = NULL;
    while (spi_device_get_trans_result(spi_ctx.dev, &ret_trans, 0) ==
           ESP_ERR_TIMEOUT) {
      TASK_YIELD();
    }

    if (spi_ctx.bulk_inplace && (spi_ctx.bulk_in != NULL) &&
        (spi_ctx.bulk_chunk_len != 0U)) {
      memcpy(spi_ctx.bulk_in + spi_ctx.bulk_pos, spi_ctx.rx_stage,
             spi_ctx.bulk_chunk_len);
    }
...
uint8_t mcu_spi_xmit(uint8_t data) { return spi_ctx_xmit(&spi_ctx, data); }
...
void mcu_spi2_stop(void) {
  if (spi2_ctx.bulk_dma && spi2_ctx.bulk_queued && spi2_ctx.dev) {
    spi_transaction_t *ret_trans = NULL;
    while (spi_device_get_trans_result(spi2_ctx.dev, &ret_trans, 0) ==
           ESP_ERR_TIMEOUT) {
      TASK_YIELD();
    }

    if (spi2_ctx.bulk_inplace && (spi2_ctx.bulk_in != NULL) &&
        (spi2_ctx.bulk_chunk_len != 0U)) {
      memcpy(spi2_ctx.bulk_in + spi2_ctx.bulk_pos, spi2_ctx.rx_stage,
             spi2_ctx.bulk_chunk_len);
    }
...
uint8_t mcu_spi2_xmit(uint8_t data) { return spi_ctx_xmit(&spi2_ctx, data); }
