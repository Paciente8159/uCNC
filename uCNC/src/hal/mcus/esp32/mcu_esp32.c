...
MCU_CALLBACK void mcu_gpio_isr(void *type) {
  // read the address and not the pointer value because we are passing a literal
  // integer reading the pointer value would try to read an invalid memory
  // address
  switch ((int)type) {
  case 0:
...
void mcu_core0_tasks_init(void *arg) {
  ...
  mcu_uart2_init();
  mcu_uart2_start();
  // xTaskCreatePinnedToCore(mcu_core0_dotasks, "core0Task", 8192, NULL, 7,
  // NULL, 0);
}
...
void __attribute__((weak)) mcu_network_init(void) { esp32_pre_init(); }
 ...
 * */
void mcu_init(void) {
#if (defined(LIMIT_X_ISR) || defined(LIMIT_Y_ISR) || defined(LIMIT_Z_ISR) ||   \
     defined(LIMIT_X2_ISR) || defined(LIMIT_Y2_ISR) ||                         \
     defined(LIMIT_Z2_ISR) || defined(LIMIT_A_ISR) || defined(LIMIT_B_ISR) ||  \
     defined(LIMIT_C_ISR) || defined(PROBE_ISR) || defined(ESTOP_ISR) ||       \
     defined(SAFETY_DOOR_ISR) || defined(FHOLD_ISR) || defined(CS_RES_ISR) ||  \
     defined(DIN0_ISR) || defined(DIN1_ISR) || defined(DIN2_ISR) ||            \
     defined(DIN3_ISR) || defined(DIN4_ISR) || defined(DIN5_ISR) ||            \
     defined(DIN6_ISR) || defined(DIN7_ISR))
  ...
  /* Timer's counter will initially start from value below.
           Also, if auto_reload is set, this value will be automatically reload
     on alarm */
  timer_set_counter_value(ITP_TIMER_TG, ITP_TIMER_IDX, 0x00000000ULL);
  /* Configure the alarm value and the interrupt on alarm. */
  timer_set_alarm_value(ITP_TIMER_TG, ITP_TIMER_IDX,
                        (uint64_t)(getApbFrequency() / (ITP_SAMPLE_RATE)));
  // register PWM isr
  ...
  xTaskCreatePinnedToCore(mcu_rtc_task, "rtcTask", 8192, NULL, 7, NULL,
...
uint32_t mcu_millis() { return (uint32_t)(esp_system_get_time() / 1000); }

uint32_t mcu_micros() { return (uint32_t)esp_system_get_time(); }

uint32_t mcu_free_micros() { return (uint32_t)(esp_system_get_time() % 1000); }
...
void mcu_config_timeout(mcu_timeout_delgate fp, uint32_t timeout) {
  ...
  timer_isr_register(ONESHOT_TIMER_TG, ONESHOT_TIMER_IDX, mcu_oneshot_isr, NULL,
                     0, NULL);
#endif
}
#endif
