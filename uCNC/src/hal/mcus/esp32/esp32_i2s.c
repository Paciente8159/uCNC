...
static void IRAM_ATTR i2s_enter_realtime_mode(void) {
  ...
  // I2S_REG.clkm_conf.clka_en = 0;			// Use PLL/2 as
  // reference I2S_REG.clkm_conf.clkm_div_num = 2; // reset value of 4
  // I2S_REG.clkm_conf.clkm_div_a = 1;		// 0 at reset, what about divide
  // by 0? I2S_REG.clkm_conf.clkm_div_b = 0;		// 0 at reset
...
uint8_t itp_set_step_mode(uint8_t mode) {
  ...
  if (mode) {
    ...
    case ITP_STEP_MODE_DEFAULT:
      ...
      // Clear sync flag
      __atomic_store_n((uint32_t *)&i2s_mode, ITP_STEP_MODE_DEFAULT,
                       __ATOMIC_RELAXED);
      break;
    }
    // mcu_toggle_output(DOUT49);
...
static void i2s_tx_base_config(void) {
  ...
  // 32-bit frames, mono-like usage, MSB-right placement compatible with 74HC595
  // stream
  ...
  i2s_hal_clock_cfg_t clock = {
      160000000, 64000000, 32000000, 2,
      2}; // 1us per word
          // i2s_hal_clock_cfg_t clock = {160000000, 160000000,160000000, 1, 5
          // }; // 2us per word i2s_hal_clock_cfg_t clock = {160000000,
          // 32000000,16000000, 5, 2 }; // 2us per word also
  ...
  i2s_hal.buf_a =
      heap_caps_malloc(DMA_BUFFER_SIZE * sizeof(uint32_t), MALLOC_CAP_DMA);
  i2s_hal.buf_b =
      heap_caps_malloc(DMA_BUFFER_SIZE * sizeof(uint32_t), MALLOC_CAP_DMA);
  ...
  // Hook ISR
  esp_intr_alloc_intrstatus(I2S_ITR_SRC,
                            ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_LEVEL3,
                            (uint32_t)i2s_ll_get_intr_status_reg(&I2S_REG),
                            (I2S_OUT_EOF_INT_ST | I2S_TX_REMPTY_INT_ST),
