...
extern "C" void esp32_wifi_config(bool force) {
  ...
  // 	if (wifi_settings.wifi_mode != 2)
  // 	{
  // 		strncpy((char *)wifi_cfg.sta.ssid, (const char
  // *)wifi_settings.ssid, sizeof(wifi_cfg.sta.ssid)); 		strncpy((char
  // *)wifi_cfg.sta.password, (const char *)wifi_settings.pass,
  // sizeof(wifi_cfg.sta.password)); 		wifi_cfg.sta.threshold.authmode =
  // WIFI_AUTH_WPA2_PSK; 		wifi_cfg.sta.pmf_cfg.capable = true;
  ...
  // 	if (wifi_settings.wifi_mode != 1)
  // 	{
  // 		strncpy((char *)wifi_cfg.ap.ssid, BOARD_NAME,
  // sizeof(wifi_cfg.ap.ssid)); 		wifi_cfg.ap.ssid_len = strlen(BOARD_NAME);
  // 		wifi_cfg.ap.channel = 1;
  // 		wifi_cfg.ap.max_connection = 4;
  // 		wifi_cfg.ap.authmode = WIFI_AUTH_WPA_WPA2_PSK;
  // 		strncpy((char *)wifi_cfg.ap.password, (const char
  // *)wifi_settings.pass, sizeof(wifi_cfg.ap.password)); 		if (strlen((const char
  // *)wifi_settings.pass) == 0) 			wifi_cfg.ap.authmode = WIFI_AUTH_OPEN;
  // 	}
...
uint8_t esp32_wifi_scan(void) {
  ...
  for (int netid = 0; netid < ap_count; netid++) {
    proto_info("%d) %s\tSignal:  %ddBm", netid, WiFi.SSID(netid).c_str(),
               WiFi.RSSI(netid));
  }
...
void flash_fs_close(fs_file_t *fp) { fileptr_t(fp->file_ptr).close(); }
...
void esp32_restart(void) { ESP.restart(); }

static flash_udpate_t esp32_flashupdate = {.get_flash_size =
                                               esp32_get_flash_size,
                                           .flash_begin = esp32_flash_begin,
                                           .flash_write = esp32_flash_write,
                                           .flash_end = esp32_flash_end,
                                           .device_restart = esp32_restart};
}
#endif
...
void esp32_pre_init(void) {
  ...
  // register the shared (lwip) socket device as the device default network
  // device
