...
#define __FREERTOS_BIN_SEMPH_TAKE__(mutex, timeout)                            \
  (((xPortInIsrContext()) ? (xSemaphoreTakeFromISR(mutex, NULL))               \
                          : (xSemaphoreTake(mutex, timeout))) == pdTRUE)
#define __FREERTOS_BIN_SEMPH_GIVE__(mutex)                                     \
  ((xPortInIsrContext()) ? (xSemaphoreGiveFromISR(mutex, NULL))                \
...
#define BIN_SEMPH_TIMEDLOCK(name, timeout_ms)                                  \
  __FREERTOS_BIN_SEMPH_TAKE__(name##_semph_lock, timeout_ms)
...
#define ATOMIC_COMPARE_EXCHANGE_N(dst, cmp, des, sucmode, failmode)            \
  __atomic_compare_exchange_n((dst), (cmp), (des), false, sucmode, failmode)
