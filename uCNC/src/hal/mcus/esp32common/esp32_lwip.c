...
static int esp32_fail_client(int index, int reason) {
  ...
  if (reason != SOCKET_DEVICE_CLOSED && reason != SOCKET_DEVICE_ERROR &&
...
static int esp32_consume_error_hint(int index) {
  ...
  fd = esp32_client_fds[index];
  if (lwip_getsockopt(fd, SOL_SOCKET, SO_ERROR, &native_error, &length) < 0) {
    int error = errno;
...
esp32_socket_listen(const socket_device_endpoint_t *endpoint, uint8_t backlog) {
  ...
  {
    int reuse = 1;
    (void)lwip_setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &reuse,
  ...
  if (lwip_bind(fd, (struct sockaddr *)&address, (socklen_t)sizeof(address)) <
      0) {
    (void)lwip_close(fd);
    return SOCKET_DEVICE_INVALID_HANDLE;
  }
...
static int esp32_socket_recv(socket_device_handle_t handle, void *destination,
                             size_t capacity) {
  size_t attempt;
  ssize_t received;
  int index;
...
static int esp32_socket_send(socket_device_handle_t handle, const void *source,
                             size_t length) {
  size_t attempt;
  ssize_t sent;
  int index;
  ...
  return esp32_fail_client(index, sent == 0 ? SOCKET_DEVICE_ERROR
...
static bool esp32_poll_one_accept(const fd_set *read_set) {
  ...
  for (checked = 0U; checked < (uint16_t)ESP32_MAX_LISTENERS; ++checked) {
    ...
    esp32_next_listener =
        (uint16_t)((esp32_next_listener + 1U) % (uint16_t)ESP32_MAX_LISTENERS);
...
static bool esp32_poll_one_client(const fd_set *read_set,
  ...
  for (checked = 0U; checked < (uint16_t)ESP32_MAX_CLIENTS; ++checked) {
    ...
    esp32_next_client =
        (uint16_t)((esp32_next_client + 1U) % (uint16_t)ESP32_MAX_CLIENTS);
    fd = esp32_client_fds[index];
    if (fd < 0) {
      continue;
    }
...
socket_device_t wifi_socket = {.init = esp32_socket_init,
