/*
        Name: esp32_uart.c
        Description: Implements the µCNC uart shim for ESP32, ESP32S3 and
   ESP32C3.
...
void mcu_uart_init(void) {
  const uart_config_t uartconfig = {.baud_rate = BAUDRATE,
  ...
  uart_set_pin(UART_PORT, TX_BIT, RX_BIT, UART_PIN_NO_CHANGE,
               UART_PIN_NO_CHANGE);
}

void mcu_uart_start(void) {
  uart_driver_install(UART_PORT, MAX((RX_BUFFER_SIZE * 2), (UART_FIFO_LEN + 1)),
                      MAX((UART_TX_BUFFER_SIZE * 2), (UART_FIFO_LEN + 1)), 0,
                      NULL, 0);
}
...
void mcu_uart2_init() {
  const uart_config_t uartconfig = {.baud_rate = BAUDRATE2,
                                    .data_bits = UART_DATA_8_BITS,
                                    .parity = UART_PARITY_DISABLE,
                                    .stop_bits = UART_STOP_BITS_1,
                                    .flow_ctrl = UART_HW_FLOWCTRL_DISABLE,
  ...
  uart_set_pin(UART2_PORT, TX2_BIT, RX2_BIT, UART2_PIN_NO_CHANGE,
               UART2_PIN_NO_CHANGE);
}

void mcu_uart2_start() {
  uart_driver_install(
      UART2_PORT, MAX((RX2_BUFFER_SIZE * 2), (UART_FIFO_LEN + 1)),
      MAX((UART2_TX_BUFFER_SIZE * 2), (UART_FIFO_LEN + 1)), 0, NULL, 0);
}
