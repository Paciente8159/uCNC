...
#include <Wire.h>
TwoWire arduino_i2c =
    TwoWire(ARDUINO_PIN_NAME(I2C_DATA), ARDUINO_PIN_NAME(I2C_CLK));
...
SPIClass arduino_spi =
    SPIClass(ARDUINO_PIN_NAME(SPI_SDO), ARDUINO_PIN_NAME(SPI_SDI),
             ARDUINO_PIN_NAME(SPI_CLK), NC);
...
uint8_t mcu_spi_xmit(uint8_t c) { return arduino_spi.transfer(c, false); }
...
void mcu_spi_stop(void) { arduino_spi.endTransaction(); }
