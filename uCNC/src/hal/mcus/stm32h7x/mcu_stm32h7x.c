/*
   ...
   interact with the MCU. This provides a opac intenterface between the µCNC and
   the MCU unit used to power the µCNC.
...
// #define FLASH_EEPROM_PER_SECTION (FLASH_SECTOR_SIZE /
// FLASH_EEPROM_SIZE_WORD_ALIGNED) #define FLASH_EEPROM_END (FLASH_EEPROM_START
// + (FLASH_EEPROM_PER_SECTION * FLASH_EEPROM_SIZE_WORD_ALIGNED) - 1)
...
void MCU_SERIAL_ISR(void) {
  ...
  if ((COM_UART->ISR & USART_ISR_TXE_TXFNF) &&
...
void MCU_SERIAL2_ISR(void) {
  ...
  if ((COM2_UART->ISR & USART_ISR_TXE_TXFNF) &&
...
#define LIMITS_EXTIBITMASK                                                     \
  (LIMIT_X_EXTIBITMASK | LIMIT_Y_EXTIBITMASK | LIMIT_Z_EXTIBITMASK |           \
   LIMIT_X2_EXTIBITMASK | LIMIT_Y2_EXTIBITMASK | LIMIT_Z2_EXTIBITMASK |        \
   LIMIT_A_EXTIBITMASK | LIMIT_B_EXTIBITMASK | LIMIT_C_EXTIBITMASK)
#define CONTROLS_EXTIBITMASK                                                   \
  (ESTOP_EXTIBITMASK | SAFETY_DOOR_EXTIBITMASK | FHOLD_EXTIBITMASK |           \
   CS_RES_EXTIBITMASK)
#define DIN_IO_EXTIBITMASK                                                     \
  (DIN0_EXTIBITMASK | DIN1_EXTIBITMASK | DIN2_EXTIBITMASK | DIN3_EXTIBITMASK | \
   DIN4_EXTIBITMASK | DIN5_EXTIBITMASK | DIN6_EXTIBITMASK | DIN7_EXTIBITMASK)
#define ALL_EXTIBITMASK                                                        \
  (LIMITS_EXTIBITMASK | CONTROLS_EXTIBITMASK | PROBE_EXTIBITMASK |             \
   DIN_IO_EXTIBITMASK)
...
void EXTI0_IRQHandler(void) { mcu_input_isr(); }
#endif
#if (ALL_EXTIBITMASK & 0x0002)
void EXTI1_IRQHandler(void) { mcu_input_isr(); }
#endif
#if (ALL_EXTIBITMASK & 0x0004)
void EXTI2_IRQHandler(void) { mcu_input_isr(); }
#endif
#if (ALL_EXTIBITMASK & 0x0008)
void EXTI3_IRQHandler(void) { mcu_input_isr(); }
#endif
#if (ALL_EXTIBITMASK & 0x0010)
void EXTI4_IRQHandler(void) { mcu_input_isr(); }
#endif
#if (ALL_EXTIBITMASK & 0x03E0)
void EXTI9_5_IRQHandler(void) { mcu_input_isr(); }
#endif
#if (ALL_EXTIBITMASK & 0xFC00)
void EXTI15_10_IRQHandler(void) { mcu_input_isr(); }
#endif
#endif
...
#define APB1_PRESC                                                             \
  ((F_CPU > 90000000UL)                                                        \
       ? RCC_CFGR_PPRE1_DIV4                                                   \
       : ((F_CPU > 45000000UL) ? RCC_CFGR_PPRE1_DIV2 : RCC_CFGR_PPRE1_DIV1))
#define APB2_PRESC                                                             \
  ((F_CPU > 90000000UL) ? RCC_CFGR_PPRE2_DIV2 : RCC_CFGR_PPRE2_DIV1)
#endif
...
void mcu_clocks_init() {
  ...
  // to run at other speeds different configuration must be applied but the
  // limit for fast AHB is 180Mhz, APB is 90Mhz and slow APB is 45Mhz
  SETFLAG(RCC->PLLCFGR,
          (RCC_PLLCFGR_PLLSRC_HSE | (PLL_M << RCC_PLLCFGR_PLLM_Pos) |
           (PLL_N << RCC_PLLCFGR_PLLN_Pos) |
           (PLL_P << RCC_PLLCFGR_PLLP_Pos) /*main clock /4*/ |
...
void mcu_uart_init(void) {
  ...
  COM_UART->CR1 |=
      (USART_CR1_RE | USART_CR1_TE | USART_CR1_UE); // enable TE, RE and UART
#endif
}
...
uint8_t mcu_usb_available(void) { return BUFFER_READ_AVAILABLE(usb_rx); }

void mcu_usb_clear(void) { BUFFER_CLEAR(usb_rx); }
...
uint32_t mcu_micros() { return ((mcu_runtime_ms * 1000) + mcu_free_micros()); }
...
static void mcu_eeprom_init(void) {
  ...
  // for (eeprom_offset = 0; eeprom_offset < FLASH_SECTOR_SIZE; eeprom_offset +=
  // FLASH_EEPROM_SIZE_WORD_ALIGNED)
  // {
  // 	if (*((volatile uint32_t *)(FLASH_EEPROM_START + eeprom_offset)) ==
  // 0xFFFFFFFF)
  // 	{
  // 		break;
  // 	}
...
static void mcu_eeprom_erase(void) {
  ...
  // }
  // FLASH->CR = 0;
  // // Ensure PG bit is low FLASH->CR |= FLASH_CR_SER | (((FLASH_SECTORS - 1)
  // << FLASH_CR_SNB_Pos) & FLASH_CR_MER_Msk); // set the SER bit FLASH->CR |=
  // FLASH_CR_STRT;
  // // set the start bit while (FLASH->SR & FLASH_SR_BSY) 	; // wait while busy
  // FLASH->CR = 0;
}
...
void mcu_eeprom_putc(uint16_t address, uint8_t value) {
  ...
  // // flash default value is 0xFF. If programming can change value from 1 to 0
  // but not the other way around
  // // if a bit is changed from 0 back to 1 then it will need to rewrite values
  // in a new page
...
void mcu_eeprom_flush() {
  ...
  // 	volatile uint32_t *eeprom = ((volatile uint32_t *)(FLASH_EEPROM_START +
  // stm32_flash_current_offset)); 	uint32_t *ptr = ((uint32_t
  // *)&stm32_eeprom_buffer[0]); 	uint32_t counter =
  // (uint32_t)FLASH_EEPROM_SIZE_WORD; 	while (counter--)
  // 	{
  // 		while (FLASH->SR & FLASH_SR_BSY)
  // 			; // wait while busy
  // 		mcu_disable_global_isr();
  ...
  // 			; // wait while busy
  // 		mcu_enable_global_isr();
  // 		if (FLASH->SR & (FLASH_SR_PGAERR | FLASH_SR_PGPERR |
  // FLASH_SR_PGSERR)) 			proto_error(42); // STATUS_SETTING_WRITE_FAIL 		if
  // (FLASH->SR & FLASH_SR_WRPERR) 			proto_error(43); //
  // STATUS_SETTING_PROTECTED_FAIL 		FLASH->CR = 0;
  // // Ensure PG bit is low 		FLASH->SR = 0; 		eeprom++; 		ptr++;
  // 	}
  // 	stm32_flash_modified = EEPROM_CLEAN;
...
void mcu_spi_config(spi_config_t config, uint32_t frequency) {
  uint8_t div = (frequency >= 2000000UL)
                    ? (uint8_t)(SPI_CLOCK / frequency)
                    : (uint8_t)(SPI_CLOCK_SLOW / frequency);
  ...
  SPI_REG->CFG2 = SPI_CFG2_SSM | SPI_CFG2_SSOE /*| SPI_CFG2_SP_0*/ |
                  SPI_CFG2_MASTER |
                  (((uint32_t)(config.mode & 0x3)) << SPI_CFG2_CPHA_Pos);
  SPI_REG->CFG1 &= ~(SPI_CFG1_DSIZE | SPI_CFG1_MBR | SPI_CFG1_FTHLV);
  SPI_REG->CFG1 |= (SPI_CFG1_DSIZE_2 | SPI_CFG1_DSIZE_1 | SPI_CFG1_DSIZE_0) |
                   (((uint32_t)speed) << SPI_CFG1_MBR_Pos);
  SPI_REG->CR1 |= SPI_CR1_SPE;
  while (!(SPI_REG->CR1 & SPI_CR1_SPE))
    ;
...
// 		--spi_transfer_tx_len;
// 	}
// 	if (spi_transfer_tx_len == 0 && spi_transfer_rx_len == 0 && (SPI_REG->SR
// & SPI_SR_EOT))
// 	{
// 		SPI_REG->IER &= ~(SPI_IER_EOTIE | SPI_IER_TXPIE |
// SPI_IER_RXPIE); 		spi_port_state = SPI_TRANSMIT_COMPLETE;
// 	}
// 	NVIC_ClearPendingIRQ(SPI_IRQ);
// }
 ...
 */
// bool mcu_spi_bulk_transfer(const uint8_t *tx_data, uint8_t *rx_data, uint16_t
// datalen)
// {
// 	// if (!spi_enable_dma)
// 	{
...
// 			SPI_REG->IER = (SPI_IER_EOTIE | SPI_IER_TXPIE |
// SPI_IER_RXPIE); 			SPI_REG->CR2 &= ~SPI_CR2_TSIZE; 			SPI_REG->CR2 |=
// (datalen<<SPI_CR2_TSIZE_Pos); 			SPI_REG->IFCR = 0xFFFFFFFFUL; 			SPI_REG->CR1 |=
// SPI_CR1_CSTART;
// 		}
...
// 	// if ((SPI_DMA_TX_STREAM->CR & DMA_SxCR_EN) || ((SPI_DMA_RX_STREAM->CR
// & DMA_SxCR_EN) && rx_data))
// 	// 	return true;
...
// 	// 		(SPI_DMA_TX_CHANNEL << DMA_SxCR_CHSEL_Pos) | // Select
// correct channel
// 	// 		(0b01 << DMA_SxCR_PL_Pos) |
...
// 	// 			(SPI_DMA_RX_CHANNEL << DMA_SxCR_CHSEL_Pos) | //
...
// 	// 	SPI_DMA_RX_STREAM->FCR &= ~DMA_SxFCR_DMDIS; // Enable direct
// mode
...
void mcu_spi2_config(spi_config_t config, uint32_t frequency) {
  uint8_t div = (frequency >= 2000000UL)
                    ? (uint8_t)(SPI2_CLOCK / frequency)
                    : (uint8_t)(SPI2_CLOCK_SLOW / frequency);
  ...
  SPI2_REG->IER = 0;
  // clear speed and mode
  SPI2_REG->CFG2 = SPI2_CFG2_SSM | SPI2_CFG2_SSOE /*| SPI2_CFG2_SP_0*/ |
                   SPI2_CFG2_MASTER |
                   (((uint32_t)(config.mode & 0x3)) << SPI2_CFG2_CPHA_Pos);
  SPI2_REG->CFG1 &= ~(SPI2_CFG1_DSIZE | SPI2_CFG1_MBR | SPI2_CFG1_FTHLV);
  SPI2_REG->CFG1 |=
      (SPI2_CFG1_DSIZE_2 | SPI2_CFG1_DSIZE_1 | SPI2_CFG1_DSIZE_0) |
      (((uint32_t)speed) << SPI2_CFG1_MBR_Pos);
  SPI2_REG->CR1 |= SPI2_CR1_SPE;
  while (!(SPI2_REG->CR1 & SPI2_CR1_SPE))
    ;
...
uint8_t mcu_i2c_send(uint8_t address, uint8_t *data, uint8_t datalen,
  ...
  if (release) {
    ...
    __TIMEOUT_ASSERT__(t) { return I2C_NOTOK; }
  }
...
uint8_t mcu_i2c_receive(uint8_t address, uint8_t *data, uint8_t datalen,
  ...
  /*Enable I2C*/
  I2C_REG->CR1 |= I2C_CR1_PE;
  I2C_REG->CR2 = I2C_CR2_HEAD10R | I2C_CR2_RD_WRN |
                 STM_VAL2REG(datalen, I2C_CR2_NBYTES) | (address << 1);
  I2C_REG->CR2 |= I2C_CR2_START;
...
void mcu_i2c_config(uint32_t frequency) {
  ...
  uint8_t scll = 0,
          sclh = scll =
              (uint8_t)CLAMP(0, (i2c_osc / (frequency * 2) - 1),
                             0xFFFF); // half time clock up and clock down
  float risetime = 8333333.3f, falltime = 8333333.3f;
  if (frequency <= 100000UL) // standart mode (max fall time 1000ns(1Mhz) and
                             // max rise time 300ns(3,3MHZ))
  {
    risetime = 1000000.0f;
    falltime = 3333333.3f;
  ...
  uint8_t scldel =
      (uint8_t)CLAMP(0, (ceilf((float)i2c_osc / risetime) - 1), 0x0F);
  uint8_t sdadel =
      (uint8_t)CLAMP(0, (ceilf((float)i2c_osc / falltime) - 1), 0x0F);

  sclh -= (sclh >= sdadel) ? sdadel : 0;
  // presc = 0; scll = 0x9; sclh=0x3; sdadel = 0x1; scldel = 0x3;
  uint32_t tmr = (STM_VAL2REG(presc, I2C_TIMINGR_PRESC) |
                  STM_VAL2REG(scll, I2C_TIMINGR_SCLL) |
                  STM_VAL2REG(sclh, I2C_TIMINGR_SCLH) |
                  STM_VAL2REG(scldel, I2C_TIMINGR_SCLDEL) |
                  STM_VAL2REG(sdadel, I2C_TIMINGR_SDADEL));
