...
#define RP2040_SOCKET_RX_QUEUE_DEPTH 4
	...
	static uint32_t g_fd_generations[RP2040_SOCKET_MAX_HANDLES];
	 ...
	 */
	static unsigned int rp2040_service_depth;
	static char rp2040_rx_dispatch_buffer[SOCKET_MAX_DATA_SIZE + 1];
	...
	static size_t rp2040_socket_copy_rx(
		fd_entry_t *e,
		char *buffer)
	{
		if (!e ||
	...
	static void rp2040_socket_service(void)
	{
		...
		bool allow_rx_and_accept = (rp2040_service_depth == 0);
		rp2040_service_depth++;
			 ...
			 i++)
		{
			...
			if (e->rx_count)
			{
				...
				size_t len =
					rp2040_socket_copy_rx(
						e,
						rp2040_rx_dispatch_buffer);
				...
				if (len > 0)
				{
					...
					{
						consumed = rp2040_socket_events->data(
							client,
							rp2040_rx_dispatch_buffer,
							len);
					}
