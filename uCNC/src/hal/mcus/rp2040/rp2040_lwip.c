/*
	...
	static backend state. rp2040_socket_poll() later emits normalized events in
...
#define RP2040_SOCKET_RX_QUEUE_DEPTH 4U
#endif

#if RP2040_SOCKET_RX_QUEUE_DEPTH == 0
#error "RP2040_SOCKET_RX_QUEUE_DEPTH must be greater than zero"
#endif
...
static err_t rp2040_recv_cb(void *arg,
						  ...
						  struct pbuf *p,
						  err_t err);
static err_t rp2040_sent_cb(void *arg, struct tcp_pcb *tpcb, u16_t len);
static err_t rp2040_tcp_poll_cb(void *arg, struct tcp_pcb *tpcb);
...
static rp2040_listener_t *rp2040_find_listener(socket_device_handle_t handle)
{
	...
	if (!rp2040_listeners[slot].in_use ||
...
static rp2040_client_t *rp2040_find_client(socket_device_handle_t handle)
{
	...
	if (client_index >= SOCKET_MAX_CONNECTIONS ||
		!rp2040_clients[client_index].in_use ||
...
static rp2040_listener_t *rp2040_alloc_listener(size_t *index_out)
{
	...
	{
		if (!rp2040_listeners[i].in_use)
		{
			uint16_t generation =
				rp2040_next_generation(rp2040_listeners[i].generation);

			memset(&rp2040_listeners[i], 0, sizeof(rp2040_listeners[i]));
			rp2040_listeners[i].generation = generation;
			rp2040_listeners[i].in_use = true;
			*index_out = i;
...
static rp2040_client_t *rp2040_alloc_client(size_t *index_out)
{
	...
	{
		if (!rp2040_clients[i].in_use)
		{
			uint16_t generation =
				rp2040_next_generation(rp2040_clients[i].generation);

			memset(&rp2040_clients[i], 0, sizeof(rp2040_clients[i]));
			rp2040_clients[i].generation = generation;
			rp2040_clients[i].handle =
				rp2040_client_handle(i, generation);
			rp2040_clients[i].listener_handle =
				SOCKET_DEVICE_INVALID_HANDLE;
			rp2040_clients[i].token = SOCKET_DEVICE_INVALID_TOKEN;
			rp2040_clients[i].close_reason = SOCKET_DEVICE_INVALID;
			rp2040_clients[i].in_use = true;
			*index_out = i;
			return &rp2040_clients[i];
		}
	}
...
/* Must be called with lwIP core protection held. */
static void rp2040_clear_rx(rp2040_client_t *client)
{
...
static void rp2040_backlog_accepted(rp2040_listener_t *listener)
{
...
#else
#define rp2040_backlog_accepted(listener) ((void)(listener))
#endif
...
static err_t rp2040_accept_cb(void *arg, struct tcp_pcb *newpcb, err_t err)
{
	...
	if (!client)
	{
		...
		rp2040_backlog_accepted(listener);
		tcp_abort(newpcb);
		return ERR_ABRT;
	}
						  ...
						  err_t err)
{
	...
	if (client->rx_count >= RP2040_SOCKET_RX_QUEUE_DEPTH)
	{
...
static void rp2040_socket_poll(uint16_t budget)
{
	...
	while (emitted < budget)
	{
			 ...
			 ++checked)
		{
			...
			rp2040_client_t *client = &rp2040_clients[index];
		...
		case RP2040_EVENT_WRITABLE:
			rp2040_events->writable(token);
			break;
