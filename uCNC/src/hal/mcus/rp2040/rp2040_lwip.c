...
static err_t rp2040_recv_cb(void *arg, struct tcp_pcb *tpcb, struct pbuf *p,
                            err_t err);
...
static bool rp2040_decode_handle(socket_device_handle_t handle,
                                 uint16_t *native_slot, uint16_t *generation) {
...
static rp2040_client_t *rp2040_find_client(socket_device_handle_t handle) {
  ...
  if (!rp2040_decode_handle(handle, &slot, &generation) || slot < MAX_SOCKETS) {
    return NULL;
  }
...
static rp2040_client_t *rp2040_alloc_client(size_t *index_out) {
  ...
  for (i = 0U; i < SOCKET_MAX_CONNECTIONS; ++i) {
    ...
    if ((client->flags & RP2040_CLIENT_IN_USE) == 0U) {
      uint16_t generation = rp2040_next_generation(client->generation);
...
/* Fatal/native state normalization. Context-specific ERR_MEM handling is
 * separate. */
...
rp2040_socket_listen(const socket_device_endpoint_t *endpoint,
                     uint8_t backlog) {
  ...
  if (endpoint->address == IP_ANY) {
    ...
    IP_ADDR4(&bind_address, (uint8_t)(endpoint->address >> 24),
             (uint8_t)(endpoint->address >> 16),
             (uint8_t)(endpoint->address >> 8), (uint8_t)endpoint->address);
  }
...
static int rp2040_socket_recv(socket_device_handle_t handle, void *destination,
                              size_t capacity) {
  rp2040_client_t *client;
  struct pbuf *p;
  ...
  if (p) {
    ...
    copied = pbuf_copy_partial(p, output, (u16_t)wanted, client->rx_offset);
    if (copied == 0U) {
      close_token = client->token;
      close_reason = SOCKET_DEVICE_ERROR;
      rp2040_release_client(client);
  ...
  if ((client->flags & RP2040_CLIENT_CLOSE_PENDING) != 0U || !client->pcb) {
    close_token = client->token;
    close_reason = (client->flags & RP2040_CLIENT_CLOSE_PENDING) != 0U
...
static int rp2040_socket_send(socket_device_handle_t handle, const void *source,
                              size_t length) {
  rp2040_client_t *client;
  size_t attempt;
  u16_t available;
   ...
   */
  err = tcp_write(client->pcb, (const uint8_t *)source, (u16_t)attempt,
                  TCP_WRITE_FLAG_COPY);
...
static void rp2040_socket_poll(uint16_t budget) {
  ...
  while (emitted < budget) {
    ...
    for (checked = 0U; checked < SOCKET_MAX_CONNECTIONS; ++checked) {
      ...
      rp2040_poll_cursor =
          (uint16_t)((rp2040_poll_cursor + 1U) % SOCKET_MAX_CONNECTIONS);
      ...
      if ((candidate->flags & RP2040_CLIENT_ACCEPT_PENDING) != 0U) {
        ...
        client_handle = rp2040_client_handle(index, candidate->generation);
      ...
      if (candidate->rx &&
        ...
        candidate->flags &= (uint8_t)~RP2040_CLIENT_READABLE_PENDING;
    ...
    case RP2040_EVENT_ACCEPTED: {
      ...
      if (selected_index < SOCKET_MAX_CONNECTIONS) {
        rp2040_client_t *selected = &rp2040_clients[selected_index];
        ...
        if ((selected->flags & RP2040_CLIENT_IN_USE) != 0U &&
          ...
          if (accepted_token == SOCKET_DEVICE_INVALID_TOKEN) {
            ...
            if (selected->rx) {
              selected->flags |= RP2040_CLIENT_READABLE_PENDING;
            }
          }
        }
      }
...
socket_device_t wifi_socket = {.init = rp2040_socket_init,
