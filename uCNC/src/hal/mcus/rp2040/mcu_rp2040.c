...
void mcu_din_isr(void) { mcu_inputs_changed_cb(); }

void mcu_probe_isr(void) { mcu_probe_changed_cb(); }

void mcu_limits_isr(void) { mcu_limits_changed_cb(); }

void mcu_controls_isr(void) { mcu_controls_changed_cb(); }
...
void mcu_alarm_isr(void) {
  ...
  uint32_t time = 0;
  if (mcu_alarms) {
    while (time = (uint32_t)timer_hw->timerawl, MEM_BARRIER,
           mcu_alarms->timeout <= time) {
  ...
  ATOMIC_CODEBLOCK {
    ...
    if (!ptr) {
      ...
      while (ptr) {
        ...
        if (ptr->timeout < target && ptr->timeout < (uint32_t)target &&
            ptr == mcu_alarms) {
 ...
 * Multicore code
 * **/
void rp2040_core0_loop() { rp2040_uart_process(); }
...
#if SERVOS_MASK > 0
  mcu_servos[servo - SERVO_PINS_OFFSET] =
      (((2000UL * value) >> 8) + 500); // quick aproximation should be divided
                                       // by 255 but it's a faste quick approach
#else
  (void)servo;
  (void)value;
#endif
...
bool mcu_get_global_isr(void) { return rp2040_global_isr_enabled; }
#endif
...
void mcu_spi_config(spi_config_t config, uint32_t frequency) {
  ...
  spi_set_format(SPI_HW, 8, ((config.mode >> 1) & 0x01), (config.mode & 0x01),
                 SPI_MSB_FIRST);
  mcu_spi_init();
}
...
bool mcu_spi_bulk_transfer(const uint8_t *out, uint8_t *in, uint16_t len) {
  ...
  if (rp2040_spi_config.enable_dma) {
    if (!transmitting) {
      ...
      // address to increment every element (in this case 1 byte = DMA_SIZE_8)
...
void mcu_spi2_config(spi_config_t config, uint32_t frequency) {
  ...
  spi_set_format(SPI2_HW, 8, ((config.mode >> 1) & 0x01), (config.mode & 0x01),
                 SPI_MSB_FIRST);
  mcu_spi2_init();
}
