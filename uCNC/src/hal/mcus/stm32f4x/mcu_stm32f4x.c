...
#define FLASH_EEPROM_PER_SECTION                                               \
  (FLASH_SECTOR_SIZE / FLASH_EEPROM_SIZE_WORD_ALIGNED)
#define FLASH_EEPROM_END                                                       \
  (FLASH_EEPROM_START +                                                        \
...
void mcu_i2c_init() {
  ...
  I2C_REG->CCR |= (I2C_FREQ <= 100000UL)
                      ? ((I2C_SPEEDRANGE * 5) & 0x0FFF)
                      : (((I2C_SPEEDRANGE * 5 / 6) & 0x0FFF) | I2C_CCR_FS);
...
static void mcu_eeprom_init(void) {
  ...
  for (eeprom_offset = 0; eeprom_offset < FLASH_SECTOR_SIZE;
  ...
  volatile uint32_t *eeprom =
      ((volatile uint32_t *)(FLASH_EEPROM_START + eeprom_offset));
  while (counter--) {
...
static void mcu_eeprom_erase(void) {
  ...
  FLASH->CR |= FLASH_CR_SER | (((FLASH_SECTORS - 1) << FLASH_CR_SNB_Pos) &
                               FLASH_CR_MER_Msk); // set the SER bit
...
void mcu_eeprom_putc(uint16_t address, uint8_t value) {
  ...
  // but not the other way around if a bit is changed from 0 back to 1 then it
  // will need to rewrite values in a new page flash read and writing is done in
  // negated form
...
void mcu_eeprom_flush() {
  ...
  if (stm32_flash_modified) {
    ...
    volatile uint32_t *eeprom = ((
        volatile uint32_t *)(FLASH_EEPROM_START + stm32_flash_current_offset));
    uint32_t *ptr = ((uint32_t *)&stm32_eeprom_buffer[0]);
    uint32_t counter = (uint32_t)FLASH_EEPROM_SIZE_WORD;
    while (counter--) {
      while (FLASH->SR & FLASH_SR_BSY)
...
bool mcu_spi_bulk_transfer(const uint8_t *tx_data, uint8_t *rx_data,
  ...
  if ((SPI_DMA_TX_STREAM->CR & DMA_SxCR_EN) ||
      ((SPI_DMA_RX_STREAM->CR & DMA_SxCR_EN) && rx_data))
    return true;
...
bool mcu_spi2_bulk_transfer(const uint8_t *tx_data, uint8_t *rx_data,
  ...
  // Wait until streams are available
  if ((SPI2_DMA_TX_STREAM->CR & DMA_SxCR_EN) ||
      ((SPI2_DMA_RX_STREAM->CR & DMA_SxCR_EN) && rx_data))
    return true;
...
static uint8_t mcu_i2c_write(uint8_t data, bool send_start, bool send_stop,
  ...
  if (send_start) {
    if ((I2C_REG->SR1 & I2C_SR1_ARLO) ||
        ((I2C_REG->CR1 & I2C_CR1_START) && (I2C_REG->CR1 & I2C_CR1_STOP))) {
      // Save values
    ...
    __TIMEOUT_MS__(timeout) {
      if ((I2C_REG->SR1 & I2C_SR1_SB) && (I2C_REG->SR2 & I2C_SR2_MSL) &&
          (I2C_REG->SR2 & I2C_SR2_BUSY)) {
        break;
      }
      if (I2C_REG->SR1 & I2C_SR1_ARLO) {
        stop = false;
...
void mcu_i2c_config(uint32_t frequency) {
  ...
  I2C_REG->CR2 |= I2C_SPEEDRANGE;
  I2C_REG->TRISE = (I2C_SPEEDRANGE + 1);
  I2C_REG->CCR |= (frequency <= 100000UL)
                      ? ((I2C_SPEEDRANGE * 5) & 0x0FFF)
                      : (((I2C_SPEEDRANGE * 5 / 6) & 0x0FFF) | I2C_CCR_FS);
#else
  // set address
