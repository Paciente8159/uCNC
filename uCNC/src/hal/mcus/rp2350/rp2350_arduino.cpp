/*
        ...
        Description: Contains all Arduino RP2350 C++ to C functions ports needed
   by µCNC.
...
bool mcu_custom_grbl_cmd(void *args) {
  ...
  if (!strncmp((const char *)(cmd_params->cmd), "WIFI", 4)) {
    if (!strcmp((const char *)&(cmd_params->cmd)[4], "ON")) {
      WiFi.disconnect();
#ifdef USE_STATIC_IP
      if (!WiFi.config(IPAddress(STATIC_IP_IP), IPAddress(STATIC_IP_GW),
                       IPAddress(STATIC_IP_SUB))) {
      ...
      settings_save(wifi_settings_offset, (uint8_t *)&wifi_settings,
    ...
    if (!strcmp((const char *)&(cmd_params->cmd)[4], "SCAN")) {
      ...
      for (int netid = 0; netid < numSsid; netid++) {
        proto_info("%d) %s\tSignal:  %ddBm", netid, WiFi.SSID(netid),
                   WiFi.RSSI(netid));
      }
      *(cmd_params->error) = STATUS_OK;
      return EVENT_HANDLED;
    ...
    if (!strcmp((const char *)&(cmd_params->cmd)[4], "RESET")) {
      settings_erase(wifi_settings_offset, (uint8_t *)&wifi_settings,
                     sizeof(wifi_settings_t));
...
bool flash_fs_remove(const char *path) { return FLASH_FS.remove(path); }
...
bool flash_fs_next_file(fs_file_t *fp, fs_file_info_t *finfo) {
  ...
  strncpy(finfo->full_name, f.name(),
          (FS_PATH_NAME_MAX_LEN - strlen(f.name())));
...
fs_file_t *flash_fs_open(const char *path, const char *mode) {
  ...
  if (fp) {
    ...
    if (fp->file_ptr) {
      *(static_cast<File *>(fp->file_ptr)) =
          FLASH_FS.open(path, sanitized_mode);
      if (*(static_cast<File *>(fp->file_ptr))) {
        ...
        strncat(fp->file_info.full_name, ((File *)fp->file_ptr)->name(),
                FS_PATH_NAME_MAX_LEN - 3);
        fp->file_info.is_dir = ((File *)fp->file_ptr)->isDirectory();
        fp->file_info.size = ((File *)fp->file_ptr)->size();
        fp->file_info.timestamp =
            (uint32_t)((File *)fp->file_ptr)->getLastWrite();
        fp->fs_ptr = &flash_fs;
        return fp;
      }
...
bool flash_fs_mkdir(const char *path) { return FLASH_FS.mkdir(path); }

bool flash_fs_rmdir(const char *path) { return FLASH_FS.rmdir(path); }
...
size_t rpico_get_flash_size(void) { return (size_t)(0xEFFFFFFF); }
...
void rpico_restart(void) { rp2040.reboot(); }

static flash_udpate_t rpico_flashupdate = {.get_flash_size =
                                               rpico_get_flash_size,
                                           .flash_begin = rpico_flash_begin,
                                           .flash_write = rpico_flash_write,
                                           .flash_end = rpico_flash_end,
                                           .device_restart = rpico_restart};
}
#endif
...
void rp2350_wifi_bt_init(void) {
  ...
  wifi_settings_offset =
      settings_register_external_setting(sizeof(wifi_settings_t));
  if (settings_load(wifi_settings_offset, (uint8_t *)&wifi_settings,
                    sizeof(wifi_settings_t))) {
    wifi_settings = {0};
...
uint8_t mcu_bt_available(void) { return BUFFER_READ_AVAILABLE(bt_rx); }

void mcu_bt_clear(void) { BUFFER_CLEAR(bt_rx); }
...
#include <EEPROM.h>
extern "C" {
void rp2350_eeprom_init(int size) { EEPROM.begin(size); }

uint8_t rp2350_eeprom_read(uint16_t address) { return EEPROM.read(address); }
...
uint8_t mcu_spi_xmit(uint8_t data) { return COM_SPI.transfer(data); }
...
void mcu_spi_stop(void) { COM_SPI.endTransaction(); }
...
uint8_t mcu_spi2_xmit(uint8_t data) { return COM_SPI2.transfer(data); }
...
void mcu_spi2_stop(void) { COM_SPI2.endTransaction(); }
...
void mcu_i2c_init(void) { mcu_i2c_config(I2C_FREQ); }
