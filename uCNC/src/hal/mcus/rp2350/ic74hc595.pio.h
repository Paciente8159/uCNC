...
static inline pio_sm_config ic74hc595_program_get_default_config(uint offset) {
  ...
  sm_config_set_wrap(&c, offset + ic74hc595_wrap_target,
                     offset + ic74hc595_wrap);
  sm_config_set_sideset(&c, 2, true, false);
  return c;
}

#include "hardware/clocks.h"
static inline void ic74hc595_program_init(PIO pio, uint sm, uint offset,
                                          uint pin_data, uint pin_clk,
                                          uint pin_latch, uint freq) {
  pio_sm_config c = ic74hc595_program_get_default_config(offset);
  sm_config_set_out_pins(&c, pin_data,
                         1); // define one pin to respond to the out instruction
  sm_config_set_set_pins(
      &c, pin_clk, 2); // define all pins to respond to the set instruction
  sm_config_set_sideset_pins(
      &c, pin_clk); // define clock as the side set base pin (and only)
  // Only support MSB-first in this example code (shift to left, no auto
  // push/pull, threshold=nbits)
