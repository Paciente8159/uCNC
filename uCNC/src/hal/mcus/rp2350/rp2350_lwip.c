...
static err_t rp2350_recv_cb(void *arg, struct tcp_pcb *tpcb, struct pbuf *p,
                            err_t err);
...
static rp2350_client_t *rp2350_find_client(socket_device_handle_t handle) {
  ...
  if (!rp2350_decode_handle(handle, &slot, &generation) || slot < MAX_SOCKETS) {
    return NULL;
  }
...
static rp2350_client_t *rp2350_alloc_client(size_t *index_out) {
  ...
  for (i = 0U; i < SOCKET_MAX_CONNECTIONS; ++i) {
    ...
    if ((client->flags & RP2350_CLIENT_IN_USE) == 0U) {
      uint16_t generation = rp2350_next_generation(client->generation);
...
rp2350_socket_listen(const socket_device_endpoint_t *endpoint,
                     uint8_t backlog) {
...
static int rp2350_socket_recv(socket_device_handle_t handle, void *destination,
                              size_t capacity) {
  rp2350_client_t *client;
  struct pbuf *p;
  ...
  if ((client->flags & RP2350_CLIENT_CLOSE_PENDING) != 0U || !client->pcb) {
    close_token = client->token;
    close_reason = (client->flags & RP2350_CLIENT_CLOSE_PENDING) != 0U
...
static int rp2350_socket_send(socket_device_handle_t handle, const void *source,
                              size_t length) {
  rp2350_client_t *client;
  size_t attempt;
  u16_t available;
...
static void rp2350_socket_poll(uint16_t budget) {
  ...
  while (emitted < budget) {
    ...
    for (checked = 0U; checked < SOCKET_MAX_CONNECTIONS; ++checked) {
      ...
      rp2350_poll_cursor =
          (uint16_t)((rp2350_poll_cursor + 1U) % SOCKET_MAX_CONNECTIONS);
      ...
      if ((candidate->flags & RP2350_CLIENT_ACCEPT_PENDING) != 0U) {
        ...
        client_handle = rp2350_client_handle(index, candidate->generation);
      ...
      if (candidate->rx &&
        ...
        candidate->flags &= (uint8_t)~RP2350_CLIENT_READABLE_PENDING;
    ...
    case RP2350_EVENT_ACCEPTED: {
      ...
      if (selected_index < SOCKET_MAX_CONNECTIONS) {
        rp2350_client_t *selected = &rp2350_clients[selected_index];
        ...
        if ((selected->flags & RP2350_CLIENT_IN_USE) != 0U &&
          ...
          if (accepted_token == SOCKET_DEVICE_INVALID_TOKEN) {
            ...
            if (selected->rx) {
              selected->flags |= RP2350_CLIENT_READABLE_PENDING;
            }
          }
        }
      }
...
socket_device_t wifi_socket = {.init = rp2350_socket_init,
