...
// generic telnet stream implementation for all MCUs that have some sort of
// implementation this can also be used by modules that implement sockets
...
uint8_t mcu_telnet_available(void) { return BUFFER_READ_AVAILABLE(telnet_rx); }
...
void mcu_telnet_clear(void) { BUFFER_CLEAR(telnet_tx); }
...
// most MCU can perform some sort of loop within 4 to 6 CPU cycles + a small
// function call overhead this is intended to use with very small delays serves
// as a base for 50ns and 100ns delays. Other values can also be generated by
// running a callibration routine
...
void __attribute__((weak)) mcu_config_analog(uint8_t pin) { (void)pin; }
#endif
#if !defined(mcu_config_input_isr)
void __attribute__((weak)) mcu_config_input_isr(uint8_t pin) { (void)pin; }
#endif
 ...
 * MCU.
 * */
uint8_t __attribute__((weak)) mcu_eeprom_getc(uint16_t address) { return 0; }
...
void __attribute__((weak)) mcu_spi_init(void) {
  spi_config_t c = {0};
  mcu_spi_config(c, SPI_FREQ);
}

void __attribute__((weak)) mcu_spi_config(spi_config_t config,
                                          uint32_t frequency) {
  (void)config;
  (void)frequency;
}

void __attribute__((weak)) mcu_spi_start(spi_config_t config,
                                         uint32_t frequency) {
...
bool __attribute__((weak)) mcu_spi_bulk_transfer(const uint8_t *out,
...
spi_port_t
    __attribute__((used)) mcu_spi_port = {.isbusy = false,
                                          .start = mcu_spi_start,
                                          .xmit = mcu_spi_xmit,
...
void __attribute__((weak)) mcu_spi2_init(void) {
  spi_config_t c = {0};
  mcu_spi2_config(c, SPI2_FREQ);
}

void __attribute__((weak)) mcu_spi2_config(spi_config_t config,
                                           uint32_t frequency) {
  (void)config;
  (void)frequency;
}

void __attribute__((weak)) mcu_spi2_start(spi_config_t config,
                                          uint32_t frequency) {
  // reapply port settings if port is shared
...
bool __attribute__((weak)) mcu_spi2_bulk_transfer(const uint8_t *out,
                                                  uint8_t *in, uint16_t len) {
...
spi_port_t
    __attribute__((used)) mcu_spi2_port = {.isbusy = false,
