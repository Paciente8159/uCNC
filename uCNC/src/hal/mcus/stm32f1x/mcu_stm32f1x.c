...
#ifdef MCU_HAS_USB
void USB_HP_CAN1_TX_IRQHandler(void) { tusb_cdc_isr_handler(); }

void USB_LP_CAN1_RX0_IRQHandler(void) { tusb_cdc_isr_handler(); }

void USBWakeUp_IRQHandler(void) { tusb_cdc_isr_handler(); }
#endif
...
void mcu_clocks_init() {
   ...
   */
  RCC->CFGR = (uint32_t)(RCC_CFGR_HPRE_DIV1 | APB1_PRESC | APB2_PRESC |
                         RCC_CFGR_PLLSRC | (PLL_MUL << RCC_CFGR_PLLMULL_Pos));
  /* Enable PLL */
  RCC->CR |= RCC_CR_PLLON;
...
void mcu_usb_init(void) {
  ...
  NVIC_SetPriority(USB_HP_CAN1_TX_IRQn, NVIC_USB_IRQ_Pri);
  NVIC_ClearPendingIRQ(USB_HP_CAN1_TX_IRQn);
  NVIC_EnableIRQ(USB_HP_CAN1_TX_IRQn);
  NVIC_SetPriority(USB_LP_CAN1_RX0_IRQn, NVIC_USB_IRQ_Pri);
  NVIC_ClearPendingIRQ(USB_LP_CAN1_RX0_IRQn);
  NVIC_EnableIRQ(USB_LP_CAN1_RX0_IRQn);
  NVIC_SetPriority(USBWakeUp_IRQn, NVIC_USB_IRQ_Pri);
  NVIC_ClearPendingIRQ(USBWakeUp_IRQn);
  NVIC_EnableIRQ(USBWakeUp_IRQn);

  // Enable USB interrupts and enable usb
  USB->CNTR |=
      (USB_CNTR_WKUPM | USB_CNTR_SOFM | USB_CNTR_ESOFM | USB_CNTR_CTRM);
  RCC->APB1ENR |= RCC_APB1ENR_USBEN;
  tusb_cdc_init();
#endif
}
...
void mcu_freq_to_clocks(float frequency, uint16_t *ticks,
  ...
  uint32_t totalticks = (uint32_t)((float)(ITP_TIMER_CLOCK) / frequency);
...
float mcu_clocks_to_freq(uint16_t ticks, uint16_t prescaller) {
  return ((float)ITP_TIMER_CLOCK / (float)(((uint32_t)ticks) << (prescaller)));
}
...
static uint16_t mcu_access_flash_page(uint16_t address) {
  ...
  if (stm32_flash_current_page != address_page) {
    ...
    volatile uint32_t *eeprom =
        ((volatile uint32_t *)(FLASH_EEPROM + address_page));
    while (counter--) {
...
void mcu_eeprom_flush() {
  ...
  if (stm32_flash_modified) {
    ...
    volatile uint16_t *eeprom =
        ((volatile uint16_t *)(FLASH_EEPROM + stm32_flash_current_page));
...
bool mcu_spi_bulk_transfer(const uint8_t *tx_data, uint8_t *rx_data,
  ...
  if ((SPI_DMA_TX_CHANNEL->CCR & DMA_CCR_EN) ||
      ((SPI_DMA_RX_CHANNEL->CCR & DMA_CCR_EN) && rx_data))
    return true;
  ...
  SPI_DMA_TX_CHANNEL->CCR = (0b01 << DMA_CCR_PL_Pos) | // Priority medium
  ...
  if (rx_data) {
    ...
    SPI_DMA_RX_CHANNEL->CCR = (0b01 << DMA_CCR_PL_Pos) | // Priority medium
                              DMA_CCR_MINC;              // Increment memory
...
bool mcu_spi2_bulk_transfer(const uint8_t *tx_data, uint8_t *rx_data,
  ...
  // Wait until streams are available
  if ((SPI2_DMA_TX_CHANNEL->CCR & DMA_CCR_EN) ||
      ((SPI2_DMA_RX_CHANNEL->CCR & DMA_CCR_EN) && rx_data))
    return true;
  ...
  SPI2_DMA_TX_CHANNEL->CCR = (0b01 << DMA_CCR_PL_Pos) | // Priority medium
                             DMA_CCR_MINC |             // Increment memory
                             DMA_CCR_DIR;               // Memory to peripheral
  ...
  if (rx_data) {
    ...
    SPI2_DMA_RX_CHANNEL->CCR = (0b01 << DMA_CCR_PL_Pos) | // Priority medium
                               DMA_CCR_MINC;              // Increment memory
...
void mcu_i2c_config(uint32_t frequency) {
  ...
  I2C_REG->TRISE |= (frequency <= 100000UL)
                        ? (I2C_SPEEDRANGE + 1)
                        : (((I2C_SPEEDRANGE * 300UL) / 1000UL) + 1);
  I2C_REG->CCR &= ~(I2C_CCR_FS | I2C_CCR_DUTY | I2C_CCR_CCR);
  uint32_t ccr = 0;
  if ((frequency <= 100000UL)) {
    // standart speed
    ccr = MAX(4, ((((HAL_RCC_GetPCLK1Freq() - 1U) / (frequency * 2)) + 1UL) &
                  I2C_CCR_CCR));
  } else {
    // fast speed
    ccr = MAX(1, ((((HAL_RCC_GetPCLK1Freq() - 1U) / (frequency * 3)) + 1UL) &
                  I2C_CCR_CCR)) |
          I2C_CCR_FS;
  }
  I2C_REG->CCR |= ccr;
