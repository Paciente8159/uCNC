...
void MCU_COM_ISR(void) {
  ...
  if (irqstatus == UART_IIR_INTID_RLS) {
    ...
    // Receive Line Status
    if (linestatus & (UART_LSR_OE | UART_LSR_PE | UART_LSR_FE | UART_LSR_RXFE |
                      UART_LSR_BI)) {
      // There are errors or break interrupt
      // Read LSR will clear the interrupt
      /*uint8_t dummy = */ (COM_INREG &
                            UART_RBR_MASKBIT); // Dummy read on RX to clear
...
#ifdef MCU_HAS_UART
  PINSEL_CFG_Type tx = {TX_PORT, TX_BIT, UART_ALT_FUNC, PINSEL_PINMODE_PULLUP,
                        PINSEL_PINMODE_NORMAL};
  PINSEL_ConfigPin(&tx);
  PINSEL_CFG_Type rx = {RX_PORT, RX_BIT, UART_ALT_FUNC, PINSEL_PINMODE_PULLUP,
                        PINSEL_PINMODE_NORMAL};
  PINSEL_ConfigPin(&rx);

  CLKPWR_SetPCLKDiv(COM_PCLK, CLKPWR_PCLKSEL_CCLK_DIV_4);

  UART_CFG_Type conf = {BAUDRATE, UART_PARITY_NONE, UART_DATABIT_8,
                        UART_STOPBIT_1};
...
#ifdef MCU_HAS_UART2
  PINSEL_CFG_Type tx = {TX2_PORT, TX2_BIT, UART2_ALT_FUNC,
                        PINSEL_PINMODE_PULLUP, PINSEL_PINMODE_NORMAL};
  PINSEL_ConfigPin(&tx);
  PINSEL_CFG_Type rx = {RX2_PORT, RX2_BIT, UART2_ALT_FUNC,
                        PINSEL_PINMODE_PULLUP, PINSEL_PINMODE_NORMAL};
  PINSEL_ConfigPin(&rx);

  CLKPWR_SetPCLKDiv(COM2_PCLK, CLKPWR_PCLKSEL_CCLK_DIV_4);

  UART_CFG_Type conf = {BAUDRATE2, UART_PARITY_NONE, UART_DATABIT_8,
                        UART_STOPBIT_1};
...
void mcu_usb_init(void) {
  ...
  LPC_PINCON->PINSEL1 |=
      ((1 << 26) | (1 << 28)); /* PINSEL1 26.27, 28.29  = 01 */

  // todo: VBUS not used by smoothieboard (though spec requires it for self
  // powered devices), pin used for beeper todo: Goodlink used for servo4?
  // LPC_PINCON->PINSEL3 &= ~((3<< 4)|(3<<28));   /* P1.18 GoodLink, P1.30 VBUS
  // */ LPC_PINCON->PINSEL3 |=  ((1<< 4)|(2<<28));   /* PINSEL3 4.5 = 01, 28.29
  // = 10 */
...
void mcu_start_itp_isr(uint16_t ticks, uint16_t prescaller) {
  ...
  ITP_TIMER_REG->PR = 0; // for higher resolution use PR = 0 that means that the
                         // timer will tick at (F_CPU/4)
 ...
 * */
uint32_t mcu_millis() { return (uint32_t)_millis; }

/**
 * provides a delay in us (micro seconds)
 * the maximum allowed delay is 255 us
 * */
uint32_t mcu_micros() { return ((mcu_millis() * 1000) + mcu_free_micros()); }
...
static void mcu_prep_spidma_transfer(void *buf, uint16_t length, bool is_rx) {
  ...
  // Transfer type
  GPDMACfg.TransferType =
      (is_rx) ? GPDMA_TRANSFERTYPE_P2M : GPDMA_TRANSFERTYPE_M2P;
  ...
  GPDMACfg.MemoryIncrease =
      (is_rx) ? GPDMA_DMACCxControl_DI : GPDMA_DMACCxControl_SI;
...
static bool mcu_assert_spidma_transfer(bool is_rx) {
  ...
  if (!GPDMA_IntGetStatus(GPDMA_STAT_INTTC,
                          ((is_rx) ? SPI_DMA_RX_CH : SPI_DMA_TX_CH)) &&
      !GPDMA_IntGetStatus(GPDMA_STAT_INTERR,
                          ((is_rx) ? SPI_DMA_RX_CH : SPI_DMA_TX_CH))) {
    return true;
  }

  // Clear err and int
  GPDMA_ClearIntPending(GPDMA_STATCLR_INTTC,
                        ((is_rx) ? SPI_DMA_RX_CH : SPI_DMA_TX_CH));
  GPDMA_ClearIntPending(GPDMA_STATCLR_INTERR,
                        ((is_rx) ? SPI_DMA_RX_CH : SPI_DMA_TX_CH));
...
bool mcu_spi_bulk_transfer(const uint8_t *out, uint8_t *in, uint16_t len) {
  ...
  if (!is_running) {
    is_running = true;
    if (spi_dma_enabled &&
        !in) // DMA only seems to work correctly in send only transmissions
    {
      // if (in)
      // {
...
static bool mcu_assert_spi2dma_transfer(bool is_rx) {
  // Wait for data transfer
  if (!GPDMA_IntGetStatus(GPDMA_STAT_INTTC,
                          ((is_rx) ? SPI2_DMA_RX_CH : SPI2_DMA_TX_CH)) &&
      !GPDMA_IntGetStatus(GPDMA_STAT_INTERR,
                          ((is_rx) ? SPI2_DMA_RX_CH : SPI2_DMA_TX_CH))) {
    return true;
  }
...
static uint8_t mcu_i2c_write(uint8_t data, bool send_start, bool send_stop,
  ...
  if (send_start) {
    // manual 19.9.7.3
    if (!I2C_REG->I2STAT || (I2C_REG->I2CONSET & I2C_I2CONSET_STA) ||
        (I2C_REG->I2CONSET & I2C_I2CONSET_STO)) {
      I2C_REG->I2CONCLR = I2C_I2CONCLR_SIC;
      I2C_REG->I2CONSET = I2C_I2CONSET_STO;
      // Wait for complete
...
void mcu_i2c_config(uint32_t frequency) {
  I2C_DeInit(I2C_REG);
  PINSEL_CFG_Type scl = {I2C_CLK_PORT, I2C_CLK_BIT, I2C_ALT_FUNC,
                         PINSEL_PINMODE_TRISTATE, PINSEL_PINMODE_OPENDRAIN};
  PINSEL_ConfigPin(&scl);
  PINSEL_CFG_Type sda = {I2C_DATA_PORT, I2C_DATA_BIT, I2C_ALT_FUNC,
                         PINSEL_PINMODE_TRISTATE, PINSEL_PINMODE_OPENDRAIN};
