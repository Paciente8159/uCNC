...
void *uart_thread_fn(void *arg) {
  ...
  while (!g_uart.stop) {
    if (!g_uart.connected) {
      HANDLE h = CreateFileA(g_uart.port_name, GENERIC_READ | GENERIC_WRITE, 0,
...
void *ioserver(void *args) {
  ...
  for (;;) {
    ...
    hPipe =
        CreateNamedPipe(lpszPipename, PIPE_ACCESS_DUPLEX,
                        PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT,
                        PIPE_UNLIMITED_INSTANCES, sizeof(VIRTUAL_MAP),
                        sizeof(VIRTUAL_MAP), 0, NULL);
...
int start_timer(int mSec, void (*timer_func_handler)(void)) {
  ...
  if (CreateTimerQueueTimer(&win_timer, NULL,
                            (WAITORTIMERCALLBACK)timer_sig_handler, NULL, mSec,
                            mSec, WT_EXECUTEINTIMERTHREAD) == 0) {
...
bool flash_fs_finfo(const char *path, fs_file_info_t *finfo) {
  ...
  finfo->is_dir =
      (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? true : false;
...
void get_current_dir(char *cwd, size_t len) { GetCurrentDirectoryA(1024, cwd); }
...
#define WINDOWS_CLIENT_READABLE_BYTES ((WINDOWS_SOCKET_MAX_CLIENTS + 7U) / 8U)
...
static socket_device_handle_t windows_make_handle(bool client, uint16_t slot,
                                                  uint16_t generation) {
 ...
 */
static bool windows_decode_handle(socket_device_handle_t handle, bool *client,
  ...
  decoded_slot =
      (uint16_t)((low >> WINDOWS_HANDLE_SLOT_SHIFT) & WINDOWS_HANDLE_SLOT_MASK);
  decoded_generation = (uint16_t)(value >> WINDOWS_HANDLE_GENERATION_SHIFT);

  if (decoded_generation == 0U ||
      windows_make_handle(decoded_client, decoded_slot, decoded_generation) !=
          handle) {
    return false;
  }
...
static int windows_socket_map_close_error(int error) {
  return error == WSAENOTSOCK || error == WSAEINVAL ? SOCKET_DEVICE_INVALID
                                                    : SOCKET_DEVICE_ERROR;
}
...
static int windows_socket_device_init(const socket_device_events_t *events) {
  ...
  windows_state.flags =
      (uint8_t)(windows_state.flags | WINDOWS_STATE_ACCEPT_FIRST);
  windows_socket_events = events;
  return SOCKET_DEVICE_OK;
}

static socket_device_handle_t
windows_socket_listen(const socket_device_endpoint_t *endpoint,
                      uint8_t backlog) {
...
static int windows_socket_recv(socket_device_handle_t client, void *destination,
                               size_t capacity) {
...
static void windows_poll_accept(const fd_set *readfds, uint16_t budget,
                                uint16_t *emitted) {
  uint16_t checked;
  uint16_t start;
  ...
  for (checked = 0U; checked < WINDOWS_SOCKET_MAX_LISTENERS; ++checked) {
    ...
    client_handle =
        windows_make_handle(true, (uint16_t)client_slot, client_generation);
...
static void windows_poll_clients(const fd_set *readfds, uint16_t budget,
                                 uint16_t *emitted) {
  uint16_t checked;
  uint16_t start;
  ...
  for (checked = 0U; checked < WINDOWS_SOCKET_MAX_CLIENTS && *emitted < budget;
       ++checked) {
    uint16_t slot = (uint16_t)((start + checked) % WINDOWS_SOCKET_MAX_CLIENTS);
    SOCKET native_socket = windows_client_sockets[slot];
    socket_device_token_t token = windows_client_tokens[slot];
...
socket_device_t wifi_socket = {.init = windows_socket_device_init,
