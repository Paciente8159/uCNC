/* linux_port.c
 *
 * Linux-compatible implementations of the functions from the Windows virtual
 * MCU file. Maintains the same function names and declarations.
 *
...
bool serial_configure(HANDLE h) {
  ...
  /* Mirror original WindowsSerial: 9600 8N1, DTR enabled (DTR not directly
   * controllable here) */
...
void *uart_thread_fn(void *arg) {
  ...
  while (!g_uart.stop) {
    if (!g_uart.connected) {
      /* Open serial port (port_name expected like "/dev/ttyUSB0" or
       * "/dev/ttyS0") */
...
bool uart_connected(void) { return g_uart.connected; }
...
/* start_timer: create a periodic timer that calls timer_func_handler every mSec
 * milliseconds */
...
fs_file_t *flash_fs_open(const char *path, const char *mode) {
  ...
  if (!finfo.is_dir) {
    ...
    if (fp->file_ptr) {
      ...
      strncat(fp->file_info.full_name, finfo.full_name,
              FS_PATH_NAME_MAX_LEN - 3);
...
/*
 * POSIX sockets fd_set stores a fixed number of int values. readfds contains
 * all listeners plus clients whose readable hint is not already latched by
 * uCNC. Fail at compile time instead of letting FD_SET silently exceed
 * capacity.
 */
...
#define LINUX_CLIENT_READABLE_BYTES ((LINUX_SOCKET_MAX_CLIENTS + 7U) / 8U)

/*
 * Structure-of-arrays storage avoids per-record alignment padding on 64-bit
 * hosts. Keeping the arrays in one aggregate also prevents linker alignment
 * gaps between separate static objects. Native TCP payload stays entirely in
 * POSIX sockets buffers; there is no backend RX copy, TX queue, TX retry
 * offset, or writable-interest state.
 */
...
static socket_device_handle_t linux_make_handle(bool client, uint16_t slot,
                                                uint16_t generation) {
 ...
 * comparing rejects malformed values and, on 64-bit hosts, values with
 * unexpected upper bits without relying on a shift as wide as uintptr_t on
 * 32-bit hosts.
 */
static bool linux_decode_handle(socket_device_handle_t handle, bool *client,
  ...
  decoded_slot =
      (uint16_t)((low >> LINUX_HANDLE_SLOT_SHIFT) & LINUX_HANDLE_SLOT_MASK);
  decoded_generation = (uint16_t)(value >> LINUX_HANDLE_GENERATION_SHIFT);

  if (decoded_generation == 0U ||
      linux_make_handle(decoded_client, decoded_slot, decoded_generation) !=
          handle) {
    return false;
  }
...
static int linux_resolve_client(socket_device_handle_t handle) {
  ...
  if (!linux_decode_handle(handle, &client, &slot, &generation) || !client ||
      slot >= LINUX_SOCKET_MAX_CLIENTS || linux_client_sockets[slot] == -1 ||
...
static socket_device_handle_t
linux_socket_listen(const socket_device_endpoint_t *endpoint, uint8_t backlog) {
...
static int linux_socket_recv(socket_device_handle_t client, void *destination,
                             size_t capacity) {
...
static int linux_socket_send(socket_device_handle_t client, const void *source,
                             size_t length) {
  int slot = linux_resolve_client(client);
  int native_socket;
  int native_length;
  ...
  /*
   * Exactly one non-blocking native send attempt. POSIX sockets copies/owns
   * bytes reported as sent before return, so the caller's source pointer is
   * never retained. No backend queue, retry offset, or writable event is
   * needed.
   */
...
static int linux_socket_close(socket_device_handle_t handle) {
  ...
  if (!client) {
    ...
    if (slot >= LINUX_SOCKET_MAX_CLIENTS || linux_client_sockets[slot] == -1 ||
        linux_client_generations[slot] != generation) {
      return SOCKET_DEVICE_INVALID;
    }
...
static void linux_poll_accept(const fd_set *readfds, uint16_t budget,
                              uint16_t *emitted) {
  uint16_t checked;
  uint16_t start;
  ...
  for (checked = 0U; checked < LINUX_SOCKET_MAX_LISTENERS; ++checked) {
    ...
    if (listener_socket == -1 || !FD_ISSET(listener_socket, readfds)) {
      continue;
    }
    ...
    if (client_slot < 0 || linux_set_nonblocking(client_socket) == -1) {
      (void)close(client_socket);
      return;
    }
    ...
    client_handle =
        linux_make_handle(true, (uint16_t)client_slot, client_generation);
  ...
  linux_listener_cursor = (uint16_t)((start + 1U) % LINUX_SOCKET_MAX_LISTENERS);
}

static void linux_poll_clients(const fd_set *readfds, uint16_t budget,
                               uint16_t *emitted) {
  uint16_t checked;
  uint16_t start;
  ...
  for (checked = 0U; checked < LINUX_SOCKET_MAX_CLIENTS && *emitted < budget;
       ++checked) {
    uint16_t slot = (uint16_t)((start + checked) % LINUX_SOCKET_MAX_CLIENTS);
    int native_socket = linux_client_sockets[slot];
    socket_device_token_t token = linux_client_tokens[slot];

    if (native_socket == -1 || token == SOCKET_DEVICE_INVALID_TOKEN ||
    ...
    linux_client_cursor = (uint16_t)((slot + 1U) % LINUX_SOCKET_MAX_CLIENTS);
  ...
  if (!emitted_client_event) {
    linux_client_cursor = (uint16_t)((start + 1U) % LINUX_SOCKET_MAX_CLIENTS);
  }
}
...
socket_device_t wifi_socket = {.init = linux_socket_device_init,
