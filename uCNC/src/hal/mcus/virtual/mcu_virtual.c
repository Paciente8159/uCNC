...
/* ----- UART (Windows COM) - non-blocking connect/service ----------------
         UART maps to a Windows serial port (UART_PORT_NAME). We connect and
service it from a background thread. RX is polled into a ring buffer; TX drains
a ring buffer. If the port is unavailable, the thread keeps retrying without
...
void mcu_uart_flush(void) {
  while (!BUFFER_EMPTY(uart_tx)) {
    ...
    if (!uart_connected()) {
      /* leave remaining bytes enqueued? We already read into tmp; if not
         connected, we can drop or re-insert. For simplicity, send attempt will
         fail silently. */
      return;
    }
    serial_write(tmp, r);
...
uint8_t mcu_unit_test_available(void) {
  return BUFFER_READ_AVAILABLE(unit_test_rx);
}
...
bool mcu_unit_test_wait_for_output(uint64_t cursor, uint32_t timeout_ms) {
  ...
  struct timespec deadline = {.tv_sec =
                                  now.tv_sec + (time_t)(timeout_ms / 1000U),
                              .tv_nsec = (long)now.tv_usec * 1000L +
  ...
  while (unit_test_tx_total <= cursor) {
    if (pthread_cond_timedwait(&unit_test_tx_changed, &unit_test_tx_mutex,
                               &deadline)) {
      break;
    }
  }
...
size_t mcu_unit_test_buffer_read_since(uint64_t cursor, char *destination,
  ...
  if (cursor > unit_test_tx_base) {
    ...
    offset =
        relative < unit_test_tx_length ? (size_t)relative : unit_test_tx_length;
  }
...
DECL_GRBL_STREAM(unit_test_grbl_stream, mcu_unit_test_getc,
                 mcu_unit_test_available, mcu_unit_test_clear,
                 mcu_unit_test_putc, mcu_unit_test_flush);
...
bool test_io_condition(test_io_id_t input) {
  ...
  if (sig->timed_enabled) {
    bool elapsed =
        (uint32_t)(mcu_millis() - sig->timed_start_ms) >= sig->timed_delay_ms;
...
void test_io_reset(void) { memset(g_test_io, 0, sizeof(g_test_io)); }
...
uint32_t mcu_micros(void) { return (uint32_t)tickcount; }

uint32_t mcu_millis(void) { return (uint32_t)(tickcount / 1000); }
 ...
 * */
void mcu_start_timeout() { oneshot_alarm = mcu_micros() + oneshot_timeout; }
...
void mcu_add_event(uint32_t delay_us, void (*callback)(void *args),
                   void *args) {
...
void ticksimul(void) { mcu_unit_test_advance_time(EMULATION_MS_TICK * 1000U); }
#else
void ticksimul(void) {
  ...
  } while (!__atomic_compare_exchange_n(&running, &test, true, false,
...
static flash_udpate_t virtual_flashupdate = {.get_flash_size =
                                                 virtual_get_flash_size,
                                             .flash_begin = virtual_flash_begin,
                                             .flash_write = virtual_flash_write,
                                             .flash_end = virtual_flash_end,
                                             .device_restart = virtual_restart};
...
uint32_t mcu_free_micros(void) {
  return mcu_micros() % 1000U; /* fix recursive bug */
}
...
// 	// HTML form for firmware upload (simplified from
...
// 			"<form method='POST' action='" OTA_URI "'
// enctype='multipart/form-data'>" 			"Firmware:<br><input type='file'
// name='firmware'>"
// 			"<input type='submit' value='Update'>"
// 			"</form></body></html>";
// 		http_send_header(client_idx, "Cache-Control", "no-cache",
// false); 		http_send_header(client_idx, "Cache-Control", "max-age=300", false);
...
// received_bytes, up.filelen); 			const char suc[] = "Update Success!
// Rebooting..."; 			http_send_str(client_idx, 200, (char *)fmt, (char *)suc);
// 			http_send(client_idx, 200, (char *)fmt, NULL, 0);
// 		}
