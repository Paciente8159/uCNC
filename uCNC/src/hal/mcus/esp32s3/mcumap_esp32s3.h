/*
        ...
        Description: Contains all MCU and PIN definitions for Arduino ESP32-Sx
   to run µCNC.
...
#define mcu_get_input(X)                                                       \
  ((__indirect__(X, BIT) & 0x20)                                               \
       ? (GPIO.in1.val & (1UL << (__indirect__(X, BIT) & 0x1f)))               \
       : (GPIO.in & (1UL << (__indirect__(X, BIT) & 0x1f))))
