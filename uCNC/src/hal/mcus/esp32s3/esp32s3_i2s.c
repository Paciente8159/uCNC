...
#define BCK_OUT_IDX                                                            \
  __helper__(I2S, I2S_PORT,                                                    \
             O_BCK_OUT_IDX) // resolves to I2S<I2S_PORT>O_BCK_OUT_IDX
#define WS_OUT_IDX                                                             \
  __helper__(I2S, I2S_PORT,                                                    \
             O_WS_OUT_IDX) // resolves to I2S<I2S_PORT>O_WS_OUT_IDX
#define DATA_OUT_IDX                                                           \
  __helper__(I2S, I2S_PORT,                                                    \
             O_SD_OUT_IDX) // resolves to I2S<I2S_PORT>O_SD_OUT_IDX
#define PERIPH_I2S                                                             \
  __helper__(PERIPH_I2S, I2S_PORT, _MODULE) // resolves to I2S<I2S_PORT>_MODULE
#define I2S_ITR_SRC                                                            \
  __helper__(ETS_I2S, I2S_PORT,                                                \
             _INTR_SOURCE) // resolves to I2S<I2S_PORT>_INTR_SOURCE
#define GDMA_I2S_PERIPH                                                        \
  (SOC_GDMA_TRIG_PERIPH_I2S0 +                                                 \
   I2S_PORT) // resolves to SOC_GDMA_TRIG_PERIPH_I2S0 or
...
#define ITP_TICK_US                                                            \
  (1000000 /                                                                   \
   (I2S_SAMPLE_RATE << 1)) // calculates the time it takes to send a sample
...
static void IRAM_ATTR i2s_dma_event_handler(void *arg) {
  ...
  dma_descriptor_t *finished = (dma_descriptor_t *)gdma_ll_tx_get_eof_desc_addr(
      &GDMA, i2s_hal.gdma_channel);
  ...
  case ITP_STEP_MODE_DEFAULT:
    ...
    finished->next =
        (dma_descriptor_t *)((finished == &desc[1]) ? &desc[0] : &desc[1]);
...
static void i2s_enter_mode_glitch_free(uint32_t mode) {
  ...
  case ITP_STEP_MODE_DEFAULT:
    ...
    gdma_ll_tx_clear_interrupt_status(
        &GDMA, i2s_hal.gdma_channel,
        gdma_ll_tx_get_interrupt_status(&GDMA, i2s_hal.gdma_channel));
    gdma_ll_tx_enable_interrupt(
        &GDMA, i2s_hal.gdma_channel,
        GDMA_LL_EVENT_TX_DONE | GDMA_LL_EVENT_TX_TOTAL_EOF, true);
    break;
...
uint8_t itp_set_step_mode(uint8_t mode) {
  ...
  if (mode != last_mode) {
    ...
    __atomic_store_n((uint32_t *)&i2s_mode, (ITP_STEP_MODE_REALTIME),
                     __ATOMIC_RELAXED);
#else
    __atomic_store_n((uint32_t *)&i2s_mode, (ITP_STEP_MODE_SYNC | mode),
                     __ATOMIC_RELAXED);
#endif
...
static void i2s_tx_base_config(void) {
  ...
  gdma_ll_tx_reset_channel(&GDMA, i2s_hal.gdma_channel);
  gdma_ll_tx_enable_interrupt(&GDMA, i2s_hal.gdma_channel,
                              GDMA_LL_TX_EVENT_MASK, false);
  gdma_ll_tx_clear_interrupt_status(&GDMA, i2s_hal.gdma_channel,
                                    GDMA_LL_TX_EVENT_MASK);
  gdma_ll_tx_connect_to_periph(&GDMA, i2s_hal.gdma_channel,
                               GDMA_TRIG_PERIPH_I2S, GDMA_I2S_PERIPH);
  gdma_ll_tx_set_eof_mode(&GDMA, i2s_hal.gdma_channel, 0);

  i2s_hal.ctx.dev = &I2S_REG;
  i2s_hal_config_t config = {.mode = I2S_MODE_MASTER | I2S_MODE_TX,
  ...
  ESP_LOGV("i2s", "clock %u, %u, %u", mclk_set.mclk_div, mclk_set.a,
           mclk_set.b);
#endif
  ...
  i2s_ll_tx_clk_set_src(&I2S_REG,
                        I2S_CLK_D2CLK); // Set I2S_CLK_D2CLK as default
  ...
  esp_intr_alloc(
      gdma_periph_signals.groups[0].pairs[i2s_hal.gdma_channel].tx_irq_id,
      ESP_INTR_FLAG_IRAM, i2s_dma_event_handler, NULL, &i2s_hal.intr_handle);
}
