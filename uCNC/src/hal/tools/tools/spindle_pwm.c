/*
        Name: spindle_pwm.c
        Description: Defines a spindle tool using PWM0-speed and DOUT0-dir for
   µCNC. Defines a coolant output using DOUT1 and DOUT2.
...
/**
 * This configures a simple spindle control with a pwm assigned to PWM0 and dir
 * invert assigned to DOUT0 This spindle also has a coolant pin assigned to
 * DOUT1
 *
 * */
...
DECL_EXTENDED_SETTING(SPINDLE_PWM_PID_SETTING_ID, spindle_pwm_pid.k, float, 3,
                      proto_gcode_setting_line_flt);
#if (HZ_TO_MS(SPINDLE_PWM_PID_SAMPLE_RATE_HZ) == 0)
#error                                                                         \
    "Period of SPINDLE_PWM_PID_SAMPLE_RATE_HZ is zero (not enough integer precision)"
#endif
#endif
...
// force pwm mode
  ...
  settings_load(EXTENDED_SETTING_ADDRESS(SPINDLE_PWM_PID_SETTING_ID),
                (uint8_t *)spindle_pwm_pid.k, sizeof(spindle_pwm_pid.k));
...
static int16_t range_speed(int16_t value, uint8_t conv) {
  ...
  if (!conv) {
    ...
    value =
        (int16_t)roundf((1.0f / 255.0f) * value * g_settings.spindle_max_rpm);
  }
  return value;
}
...
static void pid_update(void) {
  ...
  if (output != 0) {
    if (pid_compute(&spindle_pwm_pid, &output, output, get_speed(),
                    HZ_TO_MS(SPINDLE_PWM_PID_SAMPLE_RATE_HZ))) {
...
const tool_t spindle_pwm = {.startup_code = &startup_code,
                            .shutdown_code = NULL,
#if defined(ENABLE_TOOL_PID_CONTROLLER) && !defined(DISABLE_SPINDLE_PWM_PID)
