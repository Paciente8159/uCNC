/*
        ...
        Description: Defines a spindle tool using a brushless motor and a ESC
   controller
...
/**
 * Configures a simple spindle control with digital pins for tool forward (`M3`)
 * and reverse (`M4`). There is also provision for digital pins for coolant mist
 * (`M7`) and flood (`M8`).
 *
 * */
...
static int16_t range_speed(int16_t value, uint8_t conv) {
  ...
  if (!conv) {
    value = (int16_t)((SPINDLE_BESC_RANGE) *
                          (((float)value) / g_settings.spindle_max_rpm) +
                      SPINDLE_BESC_LOW);
  } else {
    value = (int16_t)roundf((1.0f / (float)SPINDLE_BESC_RANGE) *
                            (value - SPINDLE_BESC_LOW) *
...
const tool_t spindle_besc = {.startup_code = &startup_code,
