/*
        ...
        Description: Defines a embroidery tool where the needle mechanism is
   controlled with a stepper motor for µCNC.
...
DECL_EXTENDED_SETTING(EMBRODERY_STEPPER_STP_PER_REVS, &embd_steps_per_rev,
                      uint32_t, 1, proto_gcode_setting_line_int);
#define EMBRODERY_STEPPER_STP_NEEDLE_DOWN 311
DECL_EXTENDED_SETTING(EMBRODERY_STEPPER_STP_NEEDLE_DOWN,
                      &embd_steps_needle_down, uint32_t, 1,
                      proto_gcode_setting_line_int);
#else
#ifndef EMBRODERY_STEPPER_STP_PER_REVS
#define EMBRODERY_STEPPER_STP_PER_REVS                                         \
  3200 // if settings are disable set the steps per revolution directly
#endif
#ifndef EMBRODERY_STEPPER_STP_NEEDLE_DOWN
#define EMBRODERY_STEPPER_STP_NEEDLE_DOWN                                      \
  2600 // if settings are disable set the steps at which the needle enters the
       // fabric and not motion can occur directly
#endif
#endif

bool needle_is_down() { return (embd_steps_count >= embd_down_steps); }
  ...
  if (current_us > embd_steps_target_us) {
    ...
    if (current_us <= embd_steps_target_us) {
      ...
      // mcu_set_output(DOUT1);
             ...
             (embd_steps_per_rev - embd_steps_count) <= steps) {
    // mcu_clear_output(DOUT1);
    uint32_t steps_mult = (steps << 2);
    next = (uint32_t)((uint64_t)current_us * (steps_mult + 1)) /
           (steps_mult - 1); // apply the reverse equation
...
#else
#warning                                                                       \
    "ENABLE_RT_SYNC_MOTIONS not enabled embroidery tool will not work correctly"
#endif
      return; // tool stopped. prevent rearm timer
    }
    mcu_config_timeout(&embd_isr_cb,
                       ((uint32_t)embd_steps_target_us >> INT_MATH_SHIFT));
...
// 	// #else
// 	// #warning "Parser extensions are not enabled. M126, M127 and M128 code
...
// force laser mode
  ...
  settings_load(EXTENDED_SETTING_ADDRESS(EMBRODERY_STEPPER_STP_PER_REVS),
                (uint8_t *)&embd_steps_per_rev, sizeof(embd_steps_per_rev));
  EXTENDED_SETTING_INIT(EMBRODERY_STEPPER_STP_NEEDLE_DOWN,
                        embd_steps_needle_down);
  settings_load(EXTENDED_SETTING_ADDRESS(EMBRODERY_STEPPER_STP_NEEDLE_DOWN),
                (uint8_t *)&embd_steps_needle_down,
                sizeof(embd_steps_needle_down));
#else
  ...
  embd_down_steps = embd_steps_needle_down << 1;
    ...
    !defined(RT_STEP_PREVENT_HAS_CUSTOM_CONDITION) &&                          \
    defined(ENABLE_RT_SYNC_MOTIONS)
  itp_rt_step_prevent_cb = &needle_is_down;
#elif defined(RT_STEP_PREVENT_HAS_CUSTOM_CONDITION)
#warning                                                                       \
    "RT_STEP_PREVENT_CONDITION has a custom detection condition and and needle down condition may not be detected!!"
#else
#warning                                                                       \
    "RT_STEP_PREVENT_CONDITION and ENABLE_RT_SYNC_MOTIONS is not set and needle down condition will not be detected!!"
#endif
  embd_accel = 5;
...
static void set_speed(int16_t value) {
  ...
  if (value != previous_rpm) {
    uint32_t target_us =
        (value)
            ? (uint32_t)(1000000.0f / (value * embd_max_steps * MIN_SEC_MULT))
            : 0;
...
const tool_t embroidery_stepper = {.startup_code = &startup_code,
