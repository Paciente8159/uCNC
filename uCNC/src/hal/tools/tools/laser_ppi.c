/*
        ...
        Description: Defines a laser PPI tool using PWM0 pin for µCNC.
                                 Defines a coolant output using DOUT2 and DOUT3
   (can be used for air assist).
...
static void laser_ppi_config_parameters(void) {
  ...
  if (tool_get_mode() & (PPI_MODE | PPI_VARPOWER_MODE)) {
    ...
    if (!g_settings.step_per_mm[STEPPER_COUNT - 1]) {
      g_settings.step_per_mm[STEPPER_COUNT - 1] =
          g_settings.laser_ppi * MM_INCH_MULT;
    }
    g_settings.max_feed_rate[STEPPER_COUNT - 1] =
        (60000000.0f / (g_settings.laser_ppi_uswidth *
                        g_settings.step_per_mm[STEPPER_COUNT - 1]));
  ...
  if (ptr->word == 'M') {
    ...
    case 126:
      if (ptr->cmd->group_extended != 0) {
        // there is a collision of custom gcode commands (only one per line can
        // be processed)
...
bool laser_ppi_mcodes_exec(void *args) {
  ...
  case M126:
    ...
    case 1:
      ...
      break;
    case 2:
      tool_set_mode(PPI_VARPOWER_MODE);
      break;
    case 3:
      tool_set_mode((PPI_MODE | PPI_VARPOWER_MODE));
      break;
    }
    laser_ppi_config_parameters();
    return EVENT_HANDLED;
...
#else
#warning                                                                       \
    "Parser extensions are not enabled. M126, M127 and M128 code extensions will not work."
#endif
}
...
const tool_t laser_ppi = {.startup_code = &startup_code,
