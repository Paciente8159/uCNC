/*
        ...
        Description: Defines a laser tool using PWM0 for µCNC.
                                 Defines a coolant output using DOUT1 (can be
   used for air assist).
...
// this sets the minimum power (laser will never fully turn off during engraving
// and prevents power up delays)
...
static int16_t range_speed(int16_t value, uint8_t conv) {
  ...
  if (!conv) {
    ...
    value = (int16_t)roundf((1.0f / (float)(255.0f - LASER_PWM_MIN_VALUE)) *
