...
/**
 *
 * VFD Commands are an array of 6 (if first byte is 0 for dummy commands) or
 * 7(otherwise) bytes that have the following information
 *
 * {tx command length, rx command length, MODBUS command type, start address
 * high byte, start address low byte, value high byte, value low byte}
 *
 * A typical MODBUS command is usually 8 <vfd address (1byte) + MODBUS command
 * type (1byte) + start address (2byte) + value (2byte) + CRC (2byte)> A tx
 * command length of 0 will mute the command and rx length should be ommited
 *
 * Some VFD do not use the standard MODBUS message format and some times the
 * command length is different (like the Huanyang Type1)
 *
 * */
#if (VFD_CONTROLLER == VFD_HUANYANG_TYPE1)
#define VFD_SETRPM_CMD {7, 6, MODBUS_FORCE_SINGLE_COIL, 0x02, 0x00, 0x00, 0x00}
#define VFD_GETRPM_CMD                                                         \
  {8, 8, MODBUS_READ_INPUT_REGISTERS, 0x03, 0x01, 0x00, 0x00}
// sets fixed freq 400.00Hz -> 40000
#define VFD_RPM_HZ_CMD {8, 8, MODBUS_READ_COIL_STATUS, 0x03, 0x05, 0x00, 0x00}
#define VFD_CW_CMD {6, 6, MODBUS_READ_HOLDING_REGISTERS, 0x01, 0x01, 0x00, 0x00}
#define VFD_CCW_CMD                                                            \
...
#define VFD_CW_CMD {8, 8, MODBUS_PRESET_SINGLE_REGISTER, 0x20, 0x00, 0x00, 0x01}
#define VFD_CCW_CMD                                                            \
...
#define VFD_CW_CMD {8, 8, MODBUS_PRESET_SINGLE_REGISTER, 0x20, 0x00, 0x00, 0x12}
#define VFD_CCW_CMD                                                            \
...
#define VFD_RUN_CMD {8, 8, MODBUS_FORCE_SINGLE_COIL, 0x00, 0x00, 0xFF, 0x00}
#define VFD_CW_CMD {8, 8, MODBUS_FORCE_SINGLE_COIL, 0x00, 0x02, 0xFF, 0x00}
#define VFD_CCW_CMD {8, 8, MODBUS_FORCE_SINGLE_COIL, 0x00, 0x02, 0x00, 0x00}
#define VFD_STOP_CMD {8, 8, MODBUS_FORCE_SINGLE_COIL, 0x00, 0x00, 0x00, 0x00}
#define VFD_IN_MULT g_settings.spindle_max_rpm
#define VFD_IN_DIV vfd_state.rpm_hz
#define VFD_OUT_MULT vfd_state.rpm_hz
#define VFD_OUT_DIV g_settings.spindle_max_rpm
...
static bool modvfd_command(uint8_t *cmd, modbus_response_t *response) {
  ...
  proto_feedback("VFD COMMUNICATION FAILED");
...
static uint16_t vfd_rpm_hz(void) {
  ...
  if (modvfd_command(cmd, &response)) {
    return ((float)((((uint16_t)response.data[data_len - 2]) << 8) |
...
static uint16_t vfd_get_rpm(bool truerpm) {
  ...
  if (truerpm) {
    ...
    if (modvfd_command(cmd, &response)) {
      return (
          uint16_t)(((float)((((uint16_t)response.data[data_len - 2]) << 8) |
                             response.data[data_len - 1])) *
                    VFD_IN_MULT / VFD_IN_DIV);
    }
    return 0;
  }
...
static bool vfd_update_rpm(void) {
  ...
  uint16_t hz =
      (uint16_t)lroundf((float)ABS(vfd_state.rpm) * VFD_OUT_MULT / VFD_OUT_DIV);
...
static uint16_t get_speed(void) { return vfd_get_rpm(GET_SPINDLE_TRUE_RPM); }

const tool_t vfd_modbus = {.startup_code = &startup_code,
