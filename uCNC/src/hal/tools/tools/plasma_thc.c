...
// overridable
// user can implement the plasma thc up condition based on analog voltage
// reading using analog the controller analog inputs and PID controllers
...
// overridable
// user can implement the plasma thc down condition based on analog voltage
// reading using analog the controller analog inputs and PID controllers
...
// overridable
// user can implement the plasma thc arc ok condition based on analog voltage
// reading using analog the controller analog inputs and PID controllers
...
// overridable
// user can implement the plasma thc VAD (velocity anti-dive) condition on it's
// on conditions or external signals
  ...
  while (ret--) {
    ...
    if (mc_probe(pos, 0, &block) == STATUS_PROBE_SUCCESS) {
      ...
      if (mc_probe(pos, 1, &block) == STATUS_PROBE_SUCCESS) {
        ...
        block.feed = FLT_MAX;
        mc_line(pos, &block);
        // turn torch on and wait before confirm the arc on signal and form
        // puddle
...
bool m103_exec(void *args) {
  ...
  if (ptr->cmd->group_extended == M103) {
    if (CHECKFLAG(ptr->cmd->words,
                  (GCODE_WORD_I | GCODE_WORD_J | GCODE_WORD_K | GCODE_WORD_R |
                   GCODE_WORD_P | GCODE_WORD_D)) !=
        (GCODE_WORD_I | GCODE_WORD_J | GCODE_WORD_K | GCODE_WORD_R |
         GCODE_WORD_P | GCODE_WORD_D)) {
    ...
    plasma_start_params.retries =
        (CHECKFLAG(ptr->cmd->words, (GCODE_WORD_L))) ? ptr->words->l : 1;
...
static void pid_update(void) {
  ...
  if (CHECKFLAG(state, PLASMA_THC_ENABLED) &&
      CHECKFLAG(state, PLASMA_THC_ACTIVE)) {
    // arc lost
    ...
    if (!plasma_thc_vad_active()) {
      if (plasma_thc_up() && !plasma_step_error) {
        ...
        float max_feed_ratio =
            ceilf(feed / g_settings.max_feed_rate[AXIS_TOOL]);
...
const tool_t plasma_thc = {.startup_code = &startup_code,
