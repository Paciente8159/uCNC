...
#include <stdint.h>
	...
	uint8_t tool_change(uint8_t tool);					   // executes a tool change µCNC. This runs the shutdown code of the current tool and then runs the startup code of the next tool
	...
	uint8_t tool_get_mode(void);						   // gets the current tool mode (can be different from $32)
	void tool_set_mode(uint8_t mode);					   // sets a temporary mode (while the tool is active). This will be reset on tool change.
