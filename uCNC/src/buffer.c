/*
        ...
        Description: Some useful circular buffers functions and macros.
                These are small (255 slots max) sized buffer optimized for
   speed.
...
static FORCEINLINE void set_flag(ring_buffer_t *b, buffer_index_t idx) {
  ATOMIC_FETCH_OR(
      &b->flags[idx >> buf_index_byteoffset],
      (buffer_index_t)((buffer_index_t)1u << (idx & buf_index_bitoffset)),
      __ATOMIC_RELEASE);
}

static FORCEINLINE bool test_flag(ring_buffer_t *b, buffer_index_t idx) {
  buffer_index_t byte =
      ATOMIC_LOAD_N(&b->flags[idx >> buf_index_byteoffset], __ATOMIC_ACQUIRE);
  return (byte & (buffer_index_t)((buffer_index_t)1u
                                  << (idx & buf_index_bitoffset))) != 0;
}

// Clear can be release or relaxed; release is conservative if
// producers/consumers ever gate on flags.
...
static FORCEINLINE buffer_index_t index_wrap_around(buffer_index_t val,
                                                    buffer_index_t cap) {
...
uint8_t buffer_write_available(ring_buffer_t *buffer) {
  ...
  return (head >= tail) ? (buffer->size - (head - tail) - 1)
                        : (tail - head - 1);
}
...
bool buffer_empty(ring_buffer_t *buffer) {
  // return ATOMIC_COMPARE_EXCHANGE_N(&buffer->tail, &buffer->head,
  // buffer->tail, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
  uint8_t tail = (uint8_t)ATOMIC_LOAD_N(&buffer->tail, __ATOMIC_ACQUIRE);
  uint8_t head = (uint8_t)ATOMIC_LOAD_N(&buffer->head, __ATOMIC_ACQUIRE);
  return tail == head;
}
...
void buffer_peek(ring_buffer_t *buffer, void *ptr) {
  ...
  void *src =
      (void *)(buffer->data + ((size_t)tail * (size_t)buffer->elem_size));
...
bool buffer_try_enqueue(ring_buffer_t *buffer, void *ptr) {
  for (;;) {
    ...
    if (!ATOMIC_COMPARE_EXCHANGE_N(&buffer->head, &head, next_slot,
    ...
    void *dst =
        (void *)(buffer->data + ((size_t)head * (size_t)buffer->elem_size));
...
bool buffer_try_dequeue(ring_buffer_t *buffer, void *ptr) {
  ...
  for (;;) {
    ...
    if (!ATOMIC_COMPARE_EXCHANGE_N(&buffer->tail, &tail, next_avail,
                                   __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
      ATOMIC_SPIN();
      continue; // lost race
    }
...
void buffer_write(ring_buffer_t *buffer, void *ptr, uint8_t len,
                  uint8_t *written) {
  uint8_t count = 0;
  ...
  for (uint8_t i = 0; i < len; i++) {
    ...
    for (;;) {
      ...
      void *dst =
          (void *)(buffer->data + ((size_t)head * (size_t)buffer->elem_size));
      memcpy(dst, &src[(size_t)i * (size_t)buffer->elem_size],
             buffer->elem_size);
...
void buffer_read(ring_buffer_t *buffer, void *ptr, uint8_t len, uint8_t *read) {
  ...
  for (uint8_t i = 0; i < len; i++) {
    ...
    for (;;) {
      ...
      void *src =
          (void *)(buffer->data + ((size_t)tail * (size_t)buffer->elem_size));
      memcpy(&dst[(size_t)i * (size_t)buffer->elem_size], src,
             buffer->elem_size);
...
void buffer_clear(ring_buffer_t *buffer) {
  ...
  buffer_index_t flag_elems =
      (buffer->size + buf_index_bitoffset) >> buf_index_byteoffset;
