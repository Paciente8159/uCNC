...
// performs direct float manipulation and bit shifting. At the very end spectrum
// of the float (to infinity and to 0) makes aproximation either to 0 or
// infinity div2 takes about 13 clock cycles on AVR instead of 144 if multiply
// by 0.5f (x11 faster) div4 takes about 9 clock cycles on AVR instead of 144 if
// multiply by 0.25f (x16 faster) mul2 takes about 11 clock cycles on AVR
// instead of 144 if multiply by 0.25f (x13 faster) mul4 takes about 7 clock
// cycles on AVR instead of 144 if multiply by 0.25f (x20 faster)
typedef union {
  float f;
  int32_t i;
} flt_t;
...
// fast_flt_sqrt takes about 19 clock cycles on AVR instead of +/-482 if using
// normal sqrt (x25 faster). The error of this shortcut should be under 4~5%.
...
// fast_flt_invsqrt takes about 18 clock cycles on AVR instead of +/-960 if
// using normal 1/sqrt (x53 faster). The error of this shortcut should be under
// 4~5%.
...
// fast_flt_pow2 takes about 25 clock cycles on AVR instead of 144 if using
// normal pow or muliply by itself (x~5.5 faster). The error of this shortcut
// should be under 4~5%.
...
// mul10 takes about 26 clock cycles on AVR instead of 77 on 32bit integer
// multiply by 10 (x~3 faster). Can be customized for each MCU
...
#define __TIMEOUT_US__(timeout)                                                \
  for (uint32_t elap_us_##timeout,                                             \
       curr_us_##timeout = mcu_free_micros();                                  \
       timeout > 0; elap_us_##timeout = mcu_free_micros() - curr_us_##timeout, \
       timeout -= MIN(timeout, ((elap_us_##timeout < 1000)                     \
                                    ? elap_us_##timeout                        \
                                    : 1000 + elap_us_##timeout)),              \
       curr_us_##timeout = mcu_free_micros())
#define __TIMEOUT_MS__(timeout)                                                \
  timeout = (((uint32_t)timeout) < (UINT32_MAX / 1000)) ? (timeout * 1000)     \
                                                        : UINT32_MAX;          \
