/**
        Name: planner.c
        Description: Chain planner for linear motions and
   acceleration/deacceleration profiles. It uses a similar algorithm to Grbl.
...
// fires every time the current running planner block remaing steps are being
// read and before a chunk of the motion is enqueued for step generation
// (partial/full)
...
/*
        ...
        The planner is responsible for calculating the entry and exit speeds of
   the transitions The trajectory planner does the following actions:
                1. Calculates the direction change of the new movement
                2. Adjusts maximum entry feed according to the angle of the
   junction point
                3. Recalculates all chained segments

        For profiling the motion 4 feeds are calculated
                1. The target feed
                2. The rapid motion feed given the direction (maximum allowed
   feed with overrides)
                3. The entry feed (initialy set to 0)
                4. The maximum entry feed given the juntion angle between
   planner blocks
*/
void planner_add_line(motion_data_t *block_data) {
  ...
  planner_data[index].planner_flags.reg =
      block_data->motion_flags.reg; // copies the motion flags relative to
                                    // coolant spindle running and feed_override
  ...
  memcpy(planner_data[index].steps, block_data->steps,
         sizeof(planner_data[index].steps));

  // calculates the normalized vector with the amount of motion in any linear
  // actuator also calculates the maximum feedrate and acceleration for each
  // linear actuator
#ifdef ENABLE_LINACT_PLANNER
  ...
  for (uint8_t i = STEPPER_COUNT; i != 0;) {
    ...
    if (planner_data[index].steps[i] != 0) {
      ...
      if (!planner_buffer_is_empty()) {
        ...
        if (last_dir_vect[i] ==
            0) // checks if actuator is starting from a full stop
        {
  ...
  if (!planner_buffer_is_empty()) {
    prev =
        planner_buffer_prev(index); // BUFFER_PTR(planner_buffer, prev_index);
#ifdef ENABLE_LINACT_COLD_START
  ...
  // if more than one move stored cals juntion speeds and recalculates speed
  // profiles
  if (cos_theta != 0 &&
      !CHECKFLAG(block_data->motion_mode,
                 PLANNER_MOTION_EXACT_STOP |
                     MOTIONCONTROL_MODE_BACKLASH_COMPENSATION)) {
    if (cos_theta != 1.0f) {
      // calculates the junction angle with previous
      if (cos_theta > 0) {
        // uses the half angle identity conversion to convert from cos(theta) to
        // tan(theta/2) where:
        //	tan(theta/2) = sqrt((1-cos(theta)/(1+cos(theta))
        // to simplify the calculations it multiplies by
        // sqrt((1+cos(theta)/(1+cos(theta)) transforming the equation to
        // sqrt((1^2-cos(theta)^2))/(1+cos(theta)) this way the output will be
        // between 0<tan(theta/2)<inf but if theta is 0<theta<90 the
        // tan(theta/2) will be 0<tan(theta/2)<1 all angles greater than 1 that
        // can be excluded
      ...
      // sets the maximum allowed speed at junction (if angle doesn't force a
      // full stop)
      float factor =
          ((!CHECKFLAG(block_data->motion_mode, PLANNER_MOTION_CONTINUOUS))
               ? 0
               : g_settings.g64_angle_factor);
      ...
      if (angle_factor < 1.0f) {
        ...
        // the maximum feed is the minimal feed between the previous feed given
        // the angle and the current feed
  ...
  planner_add_block();
  DBGLOG(
      "[PLANNER] block added idx=%hu blocks=%hu cos_theta=%.3f entry_max=%.3f",
      index, planner_data_blocks, cos_theta,
      planner_data[index].entry_max_feed_sqr);
}
...
static void planner_add_block(void) {
  ...
  ATOMIC_CODEBLOCK { planner_data_blocks++; }
}

// this gets called inside the main loop and the step ISR
// it's safe without atomic because this only runs in the main loop after the
// step ISR is stopped so it should be OK
...
bool planner_buffer_is_empty(void) { return (!planner_data_blocks); }
...
float planner_get_block_exit_speed_sqr(void) {
  ...
  if (planner_data[next].planner_flags.bit.ovr_bypass == 0) {
    ...
    // if rapid overrides are active the feed must not exceed the rapid motion
    // feed
    if (g_planner_state.rapid_feed_override != 100) {
      rapid_feed_sqr *=
          fast_flt_pow2((float)g_planner_state.rapid_feed_override);
...
float planner_get_block_top_speed(float exit_speed_sqr) {
  ...
  */
  ...
  float junction_speed_sqr =
      planner_data[index].acceleration *
      (float)(planner_data[index].steps[planner_data[index].main_stepper]);
...
void planner_itp_pre_output(void) {
  // unnecessary check (this is only called if there is a planner block in the
  // buffer) planner_block_t* args = (planner_data_blocks != 0) ?
  // &planner_data[planner_data_read] : NULL;
...
int16_t planner_get_spindle_speed(float scale) {
  if (g_planner_state.state_flags.bit.spindle_running ^ spindle_override) {
    ...
    if ((tool_get_mode() & PWM_VARPOWER_MODE) &&
        neg) // scales pwm power only if invert is active (M4)
    {
      scaled_spindle *=
          scale; // scale calculated in laser mode (otherwise scale is always 1)
    }
    if ((g_planner_state.state_flags.bit.ovr_bypass == 0) &&
        (g_planner_state.spindle_speed_override != 100)) {
      scaled_spindle = 0.01f * (float)g_planner_state.spindle_speed_override *
                       scaled_spindle;
    }
    scaled_spindle = CLAMP(g_settings.spindle_min_rpm, scaled_spindle,
...
static void planner_recalculate(void) {
  ...
  DBGLOG("[PLANNER] recalc first=%hu last=%hu blocks=%hu", first, last,
         planner_data_blocks);

  // starts in the last added block
  // calculates the maximum entry speed of the block so that it can do a full
  // stop in the end
  ...
  while (!planner_data[block].planner_flags.bit.optimal && block != first) {
    if ((planner_data[block].entry_feed_sqr >=
         planner_data[block].entry_max_feed_sqr) ||
        planner_data[block].planner_flags.bit.optimal) {
      // found optimal
      break;
    }
    speedchange =
        ((float)(planner_data[block].steps[planner_data[block].main_stepper]
                 << 1)) *
        planner_data[block].acceleration;
    speedchange += (block != last) ? planner_data[next].entry_feed_sqr : 0;
    planner_data[block].entry_feed_sqr =
        MIN(planner_data[block].entry_max_feed_sqr, speedchange);
  ...
  while (block != last) {
    ...
    if (planner_data[block].entry_feed_sqr <
        planner_data[next].entry_feed_sqr) {
      speedchange =
          ((float)(planner_data[block].steps[planner_data[block].main_stepper]
                   << 1)) *
          planner_data[block].acceleration;
      ...
      if (speedchange < planner_data[next].entry_feed_sqr) {
        // lowers next entry speed (aka exit speed) to the maximum reachable
        // speed from current block optimization achieved for this movement
...
void planner_sync_tools(motion_data_t *block_data) {
  ...
  g_planner_state.state_flags.reg |=
      (block_data->motion_flags.reg & TOOL_STATE_COPY_FLAG_MASK);
#endif
}
...
void planner_spindle_ovr_toggle(void) {
  if (cnc_get_exec_state(EXEC_MOTIONS | EXEC_DOOR) ==
      EXEC_HOLD) // only available if a TRUE hold is active
  {
    uint8_t newstate =
