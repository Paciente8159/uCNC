...
// command send command and CRC
// each slave receives the message and calculates the message CRC including it's own address at the end
// master queries each slave with CRC with the additional slave address and slave responds if CRC matches or not with the received command
...
// master send command to get response
// slave replies
// master checks is command code is in the fisrt byte of the response and checks if the last byte of the data matches the crc
uint8_t master_get_response(uint8_t address, uint8_t command, uint8_t *data, uint8_t datalen, uint32_t timeout)
{
	...
	if (mcu_i2c_receive(address, buffer, datalen + 2, timeout) != I2C_OK || buffer[0] != command)
	{
		return MULTIBOARD_CONTROL_RESPONSE_ERROR;
	}
	...
	{
		crc = crc7(buffer[i], crc);
	}

	// crc missmatch
	if (buffer[datalen + 1] != crc)
	{
		return MULTIBOARD_CONTROL_RESPONSE_ERROR;
	}
