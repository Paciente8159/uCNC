/*
        ...
        Description: Parses Grbl system commands and RS274NGC (GCode) commands
                The RS274NGC parser tries to follow the standard document
   version 3 as close as possible. The parsing is done in 3 steps:
                        - tokenization; Converts the command string to a
   structure with GCode parameters
                        - Validation; Validates the command by checking all the
   parameters (part 3.5 - 3.7 of the document)
                        - Execution; Executes the command by the orther set in
   part 3.8 of the document.
...
FORCEINLINE static uint8_t parser_gcode_word(uint8_t code, uint8_t mantissa,
                                             parser_state_t *new_state,
                                             parser_cmd_explicit_t *cmd);
FORCEINLINE static uint8_t parser_mcode_word(uint8_t code, uint8_t mantissa,
                                             parser_state_t *new_state,
                                             parser_cmd_explicit_t *cmd);
FORCEINLINE static uint8_t parser_letter_word(uint8_t c, float value,
                                              uint8_t mantissa,
...
uint8_t parser_exec_command_block(parser_state_t *new_state,
                                  parser_words_t *words,
                                  parser_cmd_explicit_t *cmd);
#endif
...
WEAK_EVENT_HANDLER(gcode_parse) { DEFAULT_EVENT_HANDLER(gcode_parse); }

// event_gcode_exec_handler
WEAK_EVENT_HANDLER(gcode_exec) { DEFAULT_EVENT_HANDLER(gcode_exec); }
...
WEAK_EVENT_HANDLER(parse_token) { DEFAULT_EVENT_HANDLER(parse_token); }
...
WEAK_EVENT_HANDLER(parser_reset) { DEFAULT_EVENT_HANDLER(parser_reset); }
#endif

#if (defined(ENABLE_PARSER_MODULES) ||                                         \
     defined(BOARD_HAS_CUSTOM_SYSTEM_COMMANDS))
// event_grbl_cmd_handler
WEAK_EVENT_HANDLER(grbl_cmd) { DEFAULT_EVENT_HANDLER(grbl_cmd); }
...
static uint8_t parser_process_command(parser_cmd_explicit_t *cmd) {
  ...
  if (error == GRBL_JOG_CMD) {
    cmd->is_jog = 1;
             ...
             cnc_has_alarm()) // if any other than idle, run or hold discards
                              // the command
  {
  ...
  if (cnc_get_exec_state(EXEC_JOG_LOCKED) &&
      !cmd->is_jog) // error if trying to do a normal move with jog active
  {
...
#if TOOL_COUNT > 0
  modalgroups[8] = ((parser_state.groups.spindle_turning == M5)
                        ? 5
                        : (2 + parser_state.groups.spindle_turning));
...
void parser_get_coordsys(uint8_t system_num, float *axis) {
  switch (system_num) {
  case 255:
    memcpy(axis, parser_parameters.last_probe_position,
           sizeof(parser_parameters.last_probe_position));
    break;
  case 254:
...
#endif
  case 92:
    memcpy(axis, parser_parameters.g92_offset,
           sizeof(parser_parameters.g92_offset));
    break;
  default:
...
void parser_parameters_reset(void) {
  ...
  for (uint8_t i = 0; i < TOTAL_COORDINATE_SYSTEMS; i++) {
    settings_erase(SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +
                       (i * PARSER_PARAM_ADDR_OFFSET),
                   (uint8_t *)&parser_parameters.coord_system_offset,
                   PARSER_PARAM_SIZE);
  }
...
#ifndef DISABLE_COORDINATES_SYSTEM_RAM
  for (uint8_t i = 0; i < TOTAL_COORDINATE_SYSTEMS; i++) {
    settings_save(SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +
                      (i * PARSER_PARAM_ADDR_OFFSET),
                  (uint8_t *)&coordinate_systems[i], PARSER_PARAM_SIZE);
  }
#else
  // can only store the current loaded working system
  settings_save(
      SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +
          (parser_parameters.coord_system_index * PARSER_PARAM_ADDR_OFFSET),
      (uint8_t *)&parser_parameters.coord_system_offset, PARSER_PARAM_SIZE);
#endif
...
bool parser_get_wco(float *axis) {
  if (!parser_wco_counter) {
    for (uint8_t i = AXIS_COUNT; i != 0;) {
      i--;
      axis[i] = parser_parameters.g92_offset[i] +
                parser_parameters.coord_system_offset[i];
    }
...
void parser_sync_probe(void) { io_get_steps_pos(rt_probe_step_pos); }
...
void parser_update_probe_pos(void) {
  kinematics_steps_to_coordinates(rt_probe_step_pos,
                                  parser_parameters.last_probe_position);
}
...
static uint8_t parser_grbl_command(void) {
  ...
  case 1:
    ...
    case 'J':
      ...
      if (cnc_get_exec_state(EXEC_ALLACTIVE) &&
          !cnc_get_exec_state(EXEC_JOG)) // Jog only allowed in IDLE or JOG mode
      {
        parser_discard_command();
        return STATUS_IDLE_ERROR;
      }
...
#endif
    case 'R':
      if (grbl_cmd_str[1] == 'S' && grbl_cmd_str[2] == 'T' && c == '=' &&
          grbl_cmd_len == 3) {
        grbl_cmd_str[3] = '=';
        grbl_cmd_len++;
        c = grbl_stream_getc();
 ...
 *
 * STEP 1
 * Fetches the next line from the mcu communication buffer and preprocesses the
 * string In the preprocess these steps are executed
...
static uint8_t parser_fetch_command(parser_state_t *new_state,
  ...
  for (;;) {
...
#ifdef ENABLE_RS274NGC_EXPRESSIONS
    case '#':
      if ((value < 1) || (value > RS274NGC_MAX_USER_VARS) ||
          ((int)floorf(value) != value)) {
      ...
      if (new_state->modified_params_count >=
          RS274NGC_MAX_PARAMS_SET_PER_LINE) {
        return STATUS_MAXIMUM_PARAMS_PER_BLOCK_EXCEEDED;
      }
      new_state->modified_params[new_state->modified_params_count].id =
          (uint16_t)value;
      new_state->modified_params[new_state->modified_params_count].value =
          assign_val;
    ...
    case 'O':
      ...
      if (error != STATUS_OK) {
        // an error on a subrotine will cause the top subrotine to close and the
        // stack to collapse
...
#ifdef ENABLE_PARSER_MODULES
    if ((error == STATUS_GCODE_UNSUPPORTED_COMMAND ||
         error == STATUS_GCODE_UNUSED_WORDS)) {
      gcode_parse_args_t args = {word,      code,  &error, value,
                                 new_state, words, cmd};
...
static uint8_t parser_validate_command(parser_state_t *new_state,
  ...
  if (cnc_get_exec_state(EXEC_JOG)) {
    if (cmd->groups &
        ~(GCODE_GROUP_DISTANCE | GCODE_GROUP_UNITS | GCODE_GROUP_NONMODAL)) {
...
// P is not between 1 and N of coord systems
  ...
  if (has_axis || CHECKFLAG(cmd->groups, GCODE_GROUP_MOTION)) {
    ...
    case G39:
      ...
      if (!new_state->groups.motion_mantissa) {
        ...
        if ((cmd->words &
             (GCODE_WORD_I | GCODE_WORD_J | GCODE_WORD_Z | GCODE_WORD_R)) !=
            (GCODE_WORD_I | GCODE_WORD_J | GCODE_WORD_Z | GCODE_WORD_R)) {
          return STATUS_GCODE_VALUE_WORD_MISSING;
        }
    ...
    default: // G81..G89 canned cycles (partially implemented)
      ...
      if (new_state->groups.motion == G82 || new_state->groups.motion == G86 ||
          new_state->groups.motion == G88 || new_state->groups.motion == G89) {
  ...
  if ((new_state->groups.tlo_mode == G43) &&
      CHECKFLAG(cmd->groups, GCODE_GROUP_TOOLLENGTH)) {
    if (!CHECKFLAG(cmd->words, GCODE_WORD_Z) ||
        CHECKFLAG(cmd->words, (GCODE_ALL_AXIS & ~GCODE_WORD_Z))) {
      return STATUS_GCODE_G43_DYNAMIC_AXIS_ERROR;
    }

    // since G43.1 (and currently G43) support and uses word Z it can't be in
    // the same line as a MOTION group command this may be reviewed in the
    // future
...
// Words S and T must be positive
  ...
  // checks if an extended command was called with any other command at the same
  // time extension commands can only be processed individually
...
#if (SERVOS_MASK != 0)
  case M10:
    if (CHECKFLAG(cmd->words, (GCODE_WORD_S | GCODE_WORD_P)) !=
        (GCODE_WORD_S | GCODE_WORD_P)) {
      return STATUS_GCODE_VALUE_WORD_MISSING;
    }
    break;
#endif
 ...
 * All coordinates are converted to machine absolute coordinates before sent to
...
static uint8_t parser_exec_command(parser_state_t *new_state,
  ...
  if (new_state->groups.stopping &&
      !CHECKFLAG(cmd->groups, GCODE_GROUP_STOPPING)) {
  ...
  // a special case (negative extended command) is reserved for additional
  // motion commands
  if (cmd->group_extended > 0) {
    ...
    case M10:
      if (words->p < 6) {
        io_set_pinvalue(words->p + SERVO_PINS_OFFSET,
                        (uint8_t)CLAMP(words->s, 0, 255));
      }
      break;
#endif
...
// 4. set spindle speed
  ...
  block_data.motion_flags.bit.spindle_running =
      new_state->groups.spindle_turning;
  update_tools = ((parser_state.spindle != new_state->spindle) |
                  (parser_state.groups.spindle_turning !=
                   new_state->groups.spindle_turning));

  // spindle speed or direction was changed (force a safety dwell to let the
  // spindle change speed and continue)
  if (update_tools && (tool_get_mode() == SPINDLE_MODE)) {
    ...
    float set_speed =
        (new_state->groups.spindle_turning) ? new_state->spindle : 0;
    do {
  ...
  // 9. overrides
  update_tools |= (parser_state.groups.feed_speed_ovr_bypass !=
                   new_state->groups.feed_speed_ovr_bypass);
  block_data.motion_flags.bit.ovr_bypass =
      new_state->groups.feed_speed_ovr_bypass ? 1 : 0;

  // 10. dwell (or if any other nonmodal command except G53 requires a sync
  // motion)
  if (new_state->groups.nonmodal != 0 && new_state->groups.nonmodal != G53) {
    itp_sync();
    if (new_state->groups.nonmodal == G4) {
      // calc dwell in milliseconds
      block_data.dwell = MAX(block_data.dwell,
                             (uint16_t)lroundf(MIN(words->p * 1000.0f, 65535)));
  ...
  if (new_state->groups.units ==
      G20) // all internal state variables must be converted to mm
  {
    for (uint8_t i = AXIS_COUNT; i != 0;) {
      i--;
    ...
    if (CHECKFLAG(cmd->words, GCODE_WORD_F) &&
        (new_state->groups.feedrate_mode != G93)) {
...
// 13. cutter radius compensation on or off (G40, G41, G42) (not implemented
// yet)
// 14. cutter length compensation on or off (G43.1, G49)
  ...
  if (CHECKFLAG(cmd->groups, GCODE_GROUP_TOOLLENGTH)) {
    ...
    if (new_state->groups.tlo_mode == G43) {
      ...
      words->xyzabc[AXIS_TOOL] =
          0; // resets parameter so it it doen't do anything else
    }
    parser_wco_counter = 0;
  }
#endif
// 15. coordinate system selection (G54, G55, G56, G57, G58, G59, G59.1, G59.2,
// G59.3) (OK nothing to be done)
  ...
  if (CHECKFLAG(cmd->groups, GCODE_GROUP_COORDSYS)) {
    ...
    parser_coordinate_system_load(parser_parameters.coord_system_index,
                                  parser_parameters.coord_system_offset);
    parser_wco_counter = 0;
  }
#endif
// 16. set path control mode (G61, G61.1, G64)
  ...
  if (CHECKFLAG(cmd->words, GCODE_WORD_X)) {
    target[AXIS_X] = (abspos) ? words->xyzabc[AXIS_X]
                              : (words->xyzabc[AXIS_X] + target[AXIS_X]);
  }
#endif
#ifdef AXIS_Y
  if (CHECKFLAG(cmd->words, GCODE_WORD_Y)) {
    target[AXIS_Y] = (abspos) ? words->xyzabc[AXIS_Y]
                              : (words->xyzabc[AXIS_Y] + target[AXIS_Y]);
  }
#endif
#ifdef AXIS_Z
  if (CHECKFLAG(cmd->words, GCODE_WORD_Z)) {
    target[AXIS_Z] = (abspos) ? words->xyzabc[AXIS_Z]
                              : (words->xyzabc[AXIS_Z] + target[AXIS_Z]);
  }
#endif
#ifdef AXIS_A
  if (CHECKFLAG(cmd->words, GCODE_WORD_A)) {
#ifndef AXIS_A_FORCE_RELATIVE_MODE
    target[AXIS_A] = (abspos) ? words->xyzabc[AXIS_A]
                              : (words->xyzabc[AXIS_A] + target[AXIS_A]);
#else
  ...
  if (CHECKFLAG(cmd->words, GCODE_WORD_B)) {
    ...
    target[AXIS_B] = (abspos) ? words->xyzabc[AXIS_B]
                              : (words->xyzabc[AXIS_B] + target[AXIS_B]);
#else
  ...
  if (CHECKFLAG(cmd->words, GCODE_WORD_C)) {
    ...
    target[AXIS_C] = (abspos) ? words->xyzabc[AXIS_C]
                              : (words->xyzabc[AXIS_C] + target[AXIS_C]);
#else
  ...
  // 19. home (G28, G30) or change coordinate system data (G10) or set target
  // offsets (G92, G92.1, G92.2, G92.3)
  //	or also modifies target if G53 is active. These are executed after
  //calculating intemediate targets (G28 ad G30)
...
#ifndef DISABLE_G10_SUPPORT
  case G10: // G10
    index = ((uint8_t)words->p) ? words->p
                                : (parser_parameters.coord_system_index + 1);
    switch (index) {
#ifndef DISABLE_HOME_SUPPORT
    case 28:
      index = G28HOME;
  ...
  case G92_1: // G92.1
    ...
    // continue
  case G92_2: // G92.2
    memset(parser_parameters.g92_offset, 0,
           sizeof(parser_parameters.g92_offset));
    parser_wco_counter = 0;
    new_state->groups.nonmodal =
        0; // this command is compatible with motion commands
    break;
  case G92_3: // G92.3
    memcpy(parser_parameters.g92_offset, g92permanentoffset,
           sizeof(g92permanentoffset));
    parser_wco_counter = 0;
    new_state->groups.nonmodal =
        0; // this command is compatible with motion commands
    break;
    ...
    if ((new_state->groups.distance_mode == G90)) {
      for (uint8_t i = AXIS_COUNT; i != 0;) {
        ...
        if (CHECKFLAG(cmd->words, (1 << i))) {
          target[i] += parser_parameters.coord_system_offset[i] +
                       parser_parameters.g92_offset[i];
        }
      }
#ifdef AXIS_TOOL
  ...
  if (index <= G92OFFSET) {
    for (uint8_t i = AXIS_COUNT; i != 0;) {
      i--;
      if (CHECKFLAG(cmd->words, (1 << i))) {
        coords[i] = (relative_target)
                        ? -(target[i] - parser_last_pos[i] + relative_offset[i])
                        : target[i];
      }
    }
#ifdef AXIS_TOOL
  ...
  // G80 does no motion
  // G81 to G89 is executed in a separate function and uses G53,G0,G1 and G4 has
  // building blocks only if any target word was used
  if (new_state->groups.nonmodal == 0 &&
      CHECKFLAG(cmd->words, GCODE_ALL_AXIS)) {
#ifdef ENABLE_G39_H_MAPPING
    if (new_state->groups.height_map_active) {
    ...
    DBGLOG("[PARSER] motion G%hu.%hu target X=%.3f Y=%.3f feed=%.3f",
           new_state->groups.motion, new_state->groups.motion_mantissa,
           target[AXIS_X], target[AXIS_Y], block_data.feed);
#if AXIS_COUNT > 2
    ...
    case G3:
      if (block_data.feed == 0) {
        ...
        } else // offset mode
        {
          ...
          radius = sqrtf(center_offset_a * center_offset_a +
                         center_offset_b * center_offset_b);
          // calculates the distance between the center point and the target
          // points and compares with the center offset distance
          float x1 = x - center_offset_a;
          float y1 = y - center_offset_b;
          float r1 = sqrt(x1 * x1 + y1 * y1);
          if (fabs(radius - r1) >
              0.002) // error must not exceed 0.002mm according to the NIST
                     // RS274NGC standard
          {
            return STATUS_GCODE_INVALID_TARGET;
          }
        ...
        error = mc_arc(target, center_offset_a, center_offset_b, radius, a, b,
                       (new_state->groups.motion == 2), &block_data);
        DBGLOG("[PARSER] arc r=%.3f center=(%.3f,%.3f) err=%hu", radius,
               center_offset_a, center_offset_b, error);
      }
      break;
#endif
#ifndef DISABLE_PROBING_SUPPORT
 ...
 *
 *
 * Parse the next gcode line available in the buffer and send it to the motion
 * controller
 *
 *
 */
static uint8_t parser_gcode_command(parser_cmd_explicit_t *cmd) {
  ...
  parser_state_t next_state = {0};
  parser_words_t words = {0};
  // next state will be the same as previous except for nonmodal group (is set
  // with 0)
  ...
  DBGLOG("[PARSER] motion=%hu nonmodal=%hu feed=%.3f", next_state.groups.motion,
         next_state.groups.nonmodal, next_state.feedrate);
...
// executes command
  ...
  if (!cmd->is_jog && !cmd->dry_run) {
    ...
    for (uint8_t i = 0; i < next_state.modified_params_count; i++) {
      parser_set_parameter(next_state.modified_params[i].id,
                           next_state.modified_params[i].value);
    }
#endif
...
static void parser_get_comment(uint8_t start_char) {
  ...
  for (;;) {
    uint8_t c = grbl_stream_peek();
    switch (c) {
      // case '(':	//error under RS274NGC (commented for Grbl
      // compatibility)
    case ')': // OK
      if (start_char == '(') {
    ...
    case EOL: // error under RS274NGC is starts with '(' (it's ignored)
      ...
      break;
    case OVF:
      // return ((c==')') ? true : false); //under RS274NGC (commented for Grbl
      // compatibility)
      return;
    }
...
static uint8_t parser_gcode_word(uint8_t code, uint8_t mantissa,
                                 parser_state_t *new_state,
                                 parser_cmd_explicit_t *cmd) {
...
// motion codes
  ...
  case 89:
    ...
    if (cmd->group_0_1_useaxis) {
      return (cmd->groups == GCODE_GROUP_MOTION)
  ...
  case 42:
    ...
    break;
  case 43: // doesn't support G43 but G43.1 (takes Z coordinate input has
           // offset)
    if (mantissa > 1) {
      return STATUS_GCODE_UNSUPPORTED_COMMAND;
    }
...
static uint8_t parser_mcode_word(uint8_t code, uint8_t mantissa,
                                 parser_state_t *new_state,
                                 parser_cmd_explicit_t *cmd) {
  uint16_t new_group = cmd->groups;
  ...
  case 10:
    if (cmd->group_extended > 0) {
      // there is a collision of custom gcode commands (only one per line can be
      // processed)
      return STATUS_GCODE_MODAL_GROUP_VIOLATION;
    }
    // tells the gcode validation and execution functions this is custom code
    // M42 (ID must be unique)
  ...
  if ((new_group & ~GCODE_GROUP_COOLANT) ==
      (cmd->groups & ~GCODE_GROUP_COOLANT)) {
    return STATUS_GCODE_MODAL_GROUP_VIOLATION;
  }
...
static uint8_t parser_letter_word(uint8_t c, float value, uint8_t mantissa,
                                  parser_words_t *words,
                                  parser_cmd_explicit_t *cmd) {
...
void parser_reset(bool fullreset) {
  ...
  parser_state.groups.stopping =
      0; // resets all stopping commands (M0,M1,M2,M30,M60)
  ...
  memset(parser_parameters.g92_offset, 0,
         sizeof(parser_parameters.g92_offset)); // G92.2
  parser_parameters.tool_length_offset = 0;
#if TOOL_COUNT > 0
...
// loads G92
  ...
  // loads all coordinate systems to check the storage integrity. This includes
  // G28 and G30 if enabled
  for (uint8_t i = 1; i < TOTAL_COORDINATE_SYSTEMS; i++) {
#ifndef DISABLE_COORDINATES_SYSTEM_RAM
    settings_load(SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +
                      (i * PARSER_PARAM_ADDR_OFFSET),
                  (uint8_t *)coordinate_systems[i], PARSER_PARAM_SIZE);
#else
    settings_load(SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +
  ...
  settings_load(SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET,
                ...
                PARSER_PARAM_SIZE);
#ifndef DISABLE_COORDINATES_SYSTEM_RAM
  memcpy(coordinate_systems[0], parser_parameters.coord_system_offset,
         PARSER_PARAM_SIZE);
#endif
}

void parser_sync_position(void) { mc_get_position(parser_last_pos); }
...
#else
    settings_load(SETTINGS_PARSER_PARAMETERS_ADDRESS_OFFSET +
                      (param * PARSER_PARAM_ADDR_OFFSET),
                  (uint8_t *)target, PARSER_PARAM_SIZE);
#endif
    break;
  }
}
...
#endif
    memcpy(parser_parameters.g92_offset, target,
           sizeof(parser_parameters.g92_offset));
...
uint8_t parser_exec_command_block(parser_state_t *new_state,
  ...
  if (new_state->groups.nonmodal != 0 ||
      !CHECKFLAG(cmd->words, GCODE_ALL_AXIS) ||
      new_state->groups.motion < G81 || new_state->groups.motion > G89) {
    sticky_mask = 0;
  ...
  if (new_state->groups.motion == G86 &&
      (new_state->spindle == 0 || new_state->groups.spindle_turning == M5)) {
  ...
  if (CHECKFLAG(cmd->words, GCODE_WORD_R)) {
    ...
    if ((new_state->groups.nonmodal != G53) &&
        (new_state->groups.distance_mode == G90)) {
      if ((new_state->groups.distance_mode == G90)) {
        r += parser_parameters.coord_system_offset[plane_axis] +
             parser_parameters.g92_offset[plane_axis];
#ifdef AXIS_TOOL
  ...
  if (CHECKFLAG(cmd->words, (1U << plane_axis))) {
    ...
    if ((new_state->groups.nonmodal != G53) &&
        (new_state->groups.distance_mode == G90)) {
      if ((new_state->groups.distance_mode == G90)) {
        new_z += parser_parameters.coord_system_offset[plane_axis] +
                 parser_parameters.g92_offset[plane_axis];
#ifdef AXIS_TOOL
        if (CHECKFLAG(plane_axis, (1 << AXIS_TOOL))) {
  ...
  // preliminary motion starts by checking the programmed distance in the
  // working axis and the clearence if distance in inferior to clearence retract
  // to safe distance
    ...
    case G89:
      ...
      do {
        ...
        if (new_state->groups.motion == G83) {
          ...
          error =
              parser_exec_command(&canned_state, &canned_words, &canned_cmd);
          if (error) {
            return error;
          }
          current_z -= words->d;
        ...
        if (new_state->groups.motion == G86) {
          // stops spindle
          canned_state.groups.motion = G80;
          canned_state.groups.spindle_turning =
              new_state->groups.spindle_turning;
          // force spindle update
          canned_cmd.groups |= GCODE_GROUP_SPINDLE;
          error =
...
static float parser_get_named_parameter(int param, int offset, uint8_t pos) {
  ...
  case 601:
    switch (pos) {
    case 0: // motion mode
      return parser_state.groups.motion * 10 +
             parser_state.groups.motion_mantissa;
    case 1: // plane
