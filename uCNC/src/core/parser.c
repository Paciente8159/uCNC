...
uint8_t parser_exec_command(parser_state_t *new_state, parser_words_t *words, parser_cmd_explicit_t *cmd)
{
	...
	if (cmd->group_extended)
	{
		...
		case M127:
			...
			g_settings.max_feed_rate[STEPPER_COUNT - 1] = (60000000.0f / (g_itp_laser_ppi_uswidth + (2000000.0f / g_settings.max_step_rate)))/g_settings.step_per_mm[STEPPER_COUNT - 1];
			break;
		case M128:
...
void parser_reset(void)
{
	...
	g_settings.max_feed_rate[STEPPER_COUNT - 1] = (60000000.0f / (g_itp_laser_ppi_uswidth + (2000000.0f / g_settings.max_step_rate)))/(600*MM_INCH_MULT);
#endif
#endif
