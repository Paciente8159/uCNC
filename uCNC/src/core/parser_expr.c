/*
        Name: parser_expr.c
        Description: Parses RS274NGC expressions and subrotines (O-Codes)
   commands
...
float o_code_stack_context_vars[OCODE_CONTEXT_STACK_DEPTH]
                               [MIN(RS274NGC_MAX_USER_VARS, 30)];
...
static FORCEINLINE uint8_t parser_get_operation(uint8_t stack_depth,
                                                parser_stack_t *stack) {
  bool can_call_unary_func = (stack_depth <= 1)
                                 ? true
                                 : (stack[stack_depth - 1].op <= OP_NEG ||
                                    stack[stack_depth - 1].op == OP_EXPR_START);
  ...
  if (c == '<' && stack_depth && stack[stack_depth - 1].op == OP_PARSER_VAR) {
    ...
    return result;
  } else if (!can_call_unary_func) // if can't do unary checks for possible
                                   // binary op
  {
    char peek = 0;
    char peek2 = 0;
...
uint8_t parser_get_float(float *value) {
  ...
  for (;;) {
    ...
    default:
      while (stack_depth) {
        if (OP_LEVEL(stack[stack_depth - 1].op) < OP_LEVEL(op) ||
            stack[stack_depth - 1].op >= OP_EXPR_START) {
          break;
        }
...
static void o_code_open(uint8_t index) {
  if (o_code_stack[index].op == O_CODE_OP_CALL ||
    ...
    str_sprintf(o_subrotine, "/%c/o%d.nc", OCODE_DRIVE,
                o_code_stack[index].code);
...
bool o_code_end_subrotine(void) {
  ...
  if (index) {
    ...
    // restore user vars
    memcpy(g_parser_num_params,
           o_code_stack_context_vars[o_code_stack[index].context_index],
           OCODE_CONTEXT_SIZE);
...
uint8_t o_code_validate(uint8_t op, uint16_t ocode_id, bool is_new) {
  for (uint8_t index = o_code_stack_index; index != 0;) {
    ...
    if (o_code_stack[index].code == ocode_id) {
      ...
      return ((O_CODE_BASE_TYPE(o_code_stack[index].op) & op) && is_new)
                 ? 0
                 : (index + 1);
    }
...
uint8_t parser_ocode_word(uint16_t code, parser_state_t *new_state,
                          parser_cmd_explicit_t *cmd) {
  uint8_t error __attribute__((__cleanup__(o_code_word_error))) =
  ...
  if (index &&
      ((o_code_stack[index - 1].op == O_CODE_OP_IF) ||
       (o_code_stack[index - 1].op & O_CODE_OP_DISCARD)) &&
      o_code_stack[index - 1].code != ocode_id) {
    // keep discarding
    o_code_discard();
    error = STATUS_OK;
    return error;
  ...
  if (!STRCMP(o_cmd, "CALL")) {
    ...
    memcpy(&o_code_stack_context_vars[i_context], g_parser_num_params,
           OCODE_CONTEXT_SIZE);
    ...
    // workaround to ftell
    o_code_stack[index].pos =
        (o_code_file)
            ? (o_code_file->file_info.size - fs_available(o_code_file))
            : 0;
    o_code_open(index);
   ...
   * If, elseif and else
   */
  if (!STRCMP(o_cmd, "IF") || !STRCMP(o_cmd, "ELSEIF") ||
      !STRCMP(o_cmd, "ELSE") || !STRCMP(o_cmd, "ENDIF")) {
    ...
    default:
      ...
      {
        o_code_stack[index].op =
            O_CODE_OP_IF_FOUND; // signals that the condition was met
      } else {
        if (o_code_stack[index].op ==
            O_CODE_OP_IF_FOUND) // discard all remaining code
        {
  ...
  if (!STRCMP(o_cmd, "CONTINUE") || !STRCMP(o_cmd, "BREAK")) {
    ...
    if (O_CODE_BASE_TYPE(o_code_stack[index].op) == O_CODE_OP_REPEAT) {
      ...
      o_code_stack[index].op =
          (type == 5) ? O_CODE_OP_WHILE_BREAK : O_CODE_OP_WHILE_DISCARD;
    }
   ...
   * do while and while
   */
  if (!STRCMP(o_cmd, "DO") || !STRCMP(o_cmd, "WHILE") ||
      !STRCMP(o_cmd, "ENDWHILE")) {
    uint8_t type = strlen(o_cmd);
    DBGLOG("[EXPR] O%u %s arg=%.3f", ocode_id, o_cmd, op_arg);
    ...
    case 8:
      ...
      if (O_CODE_BASE_TYPE(o_code_stack[index].op) == O_CODE_OP_WHILE &&
          ocode_id == o_code_stack[index].code) {
      ...
      if (o_code_file && o_code_stack[index].op != O_CODE_OP_WHILE_DISCARD) {
        // save position
        if (o_code_file) {
          loop_ret = o_code_file->file_info.size -
                     fs_available(
                         o_code_file); // backtrack 2 chars to catch the newline
        }
        // rewind
        if (!o_code_seek(o_code_stack[index].loop)) {
          error = STATUS_OCODE_ERROR_FILE_NOT_FOUND;
    ...
    case 5:
      if (!index) {
        ...
        index = o_code_stack_index++;
        o_code_stack[index].code = ocode_id;
        o_code_stack[index].pos = loop_ret; // o_code_file->file_info.size -
                                            // fs_available(o_code_file);
        if (op_arg) {
          o_code_stack[index].op = O_CODE_OP_WHILE;
        } else {
          o_code_stack[index].op = O_CODE_OP_WHILE_BREAK;
      ...
      } else if (O_CODE_BASE_TYPE(o_code_stack[index].op) == O_CODE_OP_WHILE &&
                 ocode_id == o_code_stack[index].code) {
        // it's a do while
        switch (o_code_stack[index].op) {
        case O_CODE_OP_WHILE_DISCARD:
      ...
      o_code_stack[index].loop =
          loop_ret; // point to the loop condition eval expression
      break;
    case 2:
      if (index) {
        error = STATUS_OCODE_ERROR_INVALID_NUMBER;
  ...
  if (!STRCMP(o_cmd, "REPEAT") || !STRCMP(o_cmd, "ENDREPEAT")) {
    ...
    if (!o_code_validate(O_CODE_OP_REPEAT, ocode_id,
                         (type == 9) ? false : true)) {
      error = STATUS_OCODE_ERROR_INVALID_OPERATION;
      return error;
    }
    ...
    case 6:
      index = o_code_stack_index++;
      o_code_stack[index].code = ocode_id;
      o_code_stack[index].pos = o_code_file->file_info.size -
                                fs_available(o_code_file) +
                                1; // doesn't care about the re-eval de arg
...
uint8_t parser_get_namedparam_id(float *value) {
  ...
  if (c == '<') {
    ...
    if (valid) {
      for (uint8_t i = 0; i < sizeof(named_params) / sizeof(named_param_t);
           i++) {
        if (!rom_strcmp(namedparam,
                        (const char *)rom_strptr(&(named_params[i].name)))) {
          named_param_t p = {0};
