/*
   ...
   version 3 as close as possible. The parsing is done in 3 steps:
                        - Tockenization; Converts the command string to a
...
#define GCODE_JOG_INVALID_WORDS                                                \
  (GCODE_WORD_I | GCODE_WORD_J | GCODE_WORD_K | GCODE_WORD_D | GCODE_WORD_L |  \
   GCODE_WORD_P | GCODE_WORD_R | GCODE_WORD_T | GCODE_WORD_S)
#define GCODE_ALL_AXIS                                                         \
  (GCODE_WORD_X | GCODE_WORD_Y | GCODE_WORD_Z | GCODE_WORD_A | GCODE_WORD_B |  \
   GCODE_WORD_C)
...
#define SPINDLE_MODE                                                           \
  0 // if the tool is not defined it will return as spindle mode
#define PWM_VARPOWER_MODE                                                      \
  1                // PWM variable mode will scale the tool power with M4 active
