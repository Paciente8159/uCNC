...
uint8_t mc_line(float *target, motion_data_t *block_data)
{
	...
	{
		laser_pulses_per_mm = MM_INCH_MULT * g_settings.step_per_mm[STEPPER_COUNT - 1];
	...
	step_new_pos[STEPPER_COUNT - 1] = (block_data->spindle != 0) ? laser_pulses_per_mm * line_dist : 0;
