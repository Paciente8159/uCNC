/*
        ...
        Description: Contains the building blocks for performing motions/actions
   in µCNC Copyright: Copyright (c) João Martins Author: João Martins Date:
   19/11/2019
...
#define KINEMATICS_MOTION_SEGMENT_INV_SIZE                                     \
...
#define H_MAPING_SEGMENT_INV_SIZE                                              \
  (MAX(((float)H_MAPING_GRID_FACTOR / hmap_x_offset),                          \
       ((float)H_MAPING_GRID_FACTOR / hmap_y_offset)))
...
WEAK_EVENT_HANDLER(mc_line_segment) { DEFAULT_EVENT_HANDLER(mc_line_segment); }
...
bool mc_get_checkmode(void) { return mc_checkmode; }
...
static uint8_t mc_line_segment(int32_t *step_new_pos,
...
// resets accumulator vars of the block
  ...
  if (!mc_checkmode) // check mode (gcode simulation) doesn't send code to
                     // planner
  {
#ifdef ENABLE_BACKLASH_COMPENSATION
...
#endif
      backlash_block_data.feed =
          backlash_block_data
              .max_feed; // max feedrate possible (same as rapid move)
    ...
    DBGLOG("[MC] segment enqueued main=%hu steps=%lu", block_data->main_stepper,
           (unsigned long)block_data->steps[block_data->main_stepper]);
    // dwell should only execute on the first request
    block_data->dwell = 0;
  }
...
// all motions should go through mc_line before entering the final planner
// pipeline after this stage the motion follows a pipeline that performs the
// following steps
...
// 5. if line is to big to be executed correctly by the interpolator, breaks it
// in to smaller line segments
uint8_t mc_line(float *target, motion_data_t *block_data) {
  ...
  block_data->dirbits =
      0; // reset dirbits (this prevents odd behaviour generated by long arcs)
  block_data->main_stepper = 255; // reset the main stepper

  DBGLOG("[MC] line target X=%.3f Y=%.3f feed=%.3f", target[AXIS_X],
         target[AXIS_Y], block_data->feed);

#ifdef ENABLE_G39_H_MAPPING
  // modify the gcode with Hmap
  float target_hmap_offset =
      (CHECKFLAG(block_data->motion_mode, MOTIONCONTROL_MODE_APPLY_HMAP))
  ...
  if (!kinematics_check_boundaries(target)) {
    DBGLOG("[MC] soft limit exceeded");
#ifdef ENABLE_G39_H_MAPPING
    // unmodify target
    target[AXIS_TOOL] -= target_hmap_offset;
#endif

#if !defined(MODIFY_SOFT_LIMIT_TO_ERROR) ||                                    \
    defined(ALLOW_SOFT_LIMIT_JOG_MOTION_CLAMPING)
  ...
  // gets the previous machine position (transformed to calculate the direction
  // vector and traveled distance)
...
// remove the hmap postion of the motion
  ...
  if (!max_steps) {
    ...
    target[AXIS_TOOL] -= target_hmap_offset;
#endif
    DBGLOG("[MC] no motion (max_steps=0)");
    return STATUS_OK;
  }
...
#endif
  {
    ...
    // calculates max junction speed factor in (axis driven). Else the cos_theta
    // is calculated in the planner (linear actuator driven)
    for (uint8_t i = 0; i < AXIS_DIR_VECTORS; i++) {
      ...
      float denorm_param =
          fast_flt_div(g_settings.max_feed_rate[i], normal_vect);
  ...
  ppi_max_feedrate =
      (60000000.0f / (g_settings.laser_ppi_uswidth * ppi_step_rate));
  ...
  if (block_data->motion_flags.bit.spindle_running && block_data->spindle) {
    ...
    if (tool_get_mode() & PPI_MODE) {
      float laser_ppi_scale = fast_flt_div((float)block_data->spindle,
                                           (float)g_settings.spindle_max_rpm);
  ...
  float step_feed =
      (!CHECKFLAG(block_data->motion_mode, MOTIONCONTROL_MODE_INVERSEFEED)
           ? (block_data->feed * inv_dist)
           : block_data->feed);
  float feed_convert_to_steps_per_sec = (float)max_steps;
  // convert accel already in steps/s
  // use max accel if accel is not already set by previous calculations (for
  // example synched motions)
  block_data->max_accel =
      (!block_data->max_accel)
          ? (feed_convert_to_steps_per_sec * max_accel)
          : (block_data->max_accel * inv_dist * feed_convert_to_steps_per_sec);
  ...
  block_data->feed_conversion =
      fast_flt_div(line_dist, feed_convert_to_steps_per_sec);

  DBGLOG("[MC] steps=%lu feed=%.3f max_feed=%.3f accel=%.3f dist=%.3f",
         (unsigned long)max_steps, block_data->feed, block_data->max_feed,
         block_data->max_accel, line_dist);
  ...
  if (CHECKFLAG(block_data->motion_mode, MOTIONCONTROL_MODE_APPLY_HMAP)) {
    line_segments = MAX((uint32_t)ceilf(line_dist * H_MAPING_SEGMENT_INV_SIZE),
                        line_segments);
  }
#endif
#ifdef KINEMATICS_MOTION_BY_SEGMENTS
  line_segments =
      MAX((uint32_t)ceilf(line_dist * KINEMATICS_MOTION_SEGMENT_INV_SIZE),
          line_segments);
#endif
#ifdef BRESENHAM_16BIT
  // checks the amount of steps that this motion translates to
  // if the amount of steps is higher than the limit for the 16bit bresenham
  // algorithm splits the line into smaller segments
  if (max_steps > MAX_STEPS_PER_LINE) {
    line_segments =
        MAX((max_steps >> MAX_STEPS_PER_LINE_BITS) + 1, line_segments);
  }
#endif

// final modifier is a hookable event
// this allows to complete control and override over the amount of segments to
// be produced.
#ifdef ENABLE_MOTION_CONTROL_MODULES
  mc_line_calc_segments_args_t mc_line_calc_segments_args = {
...
#ifdef ENABLE_MOTION_CONTROL_MODULES
    // event_mc_line_segment_handler
    mc_line_segment_pre_args_t mc_line_segment_pre_args = {
        .target = prev_target,
        .target_steps = step_new_pos,
...
#ifdef ENABLE_MOTION_CONTROL_MODULES
  // event_mc_line_segment_handler
  mc_line_segment_pre_args_t args = {
      .target = target, .target_steps = step_new_pos, .block_data = block_data};
  ...
  // restores feed and clears max acceleration to enable recalculation on next
  // motion
  block_data->feed = feed;
  block_data->max_accel = 0;
  DBGLOG("[MC] line done err=%hu", error);
  return error;
}

#ifndef DISABLE_ARC_SUPPORT
// applies an algorithm similar to grbl with slight changes
uint8_t mc_arc(float *target, float center_offset_a, float center_offset_b,
               float radius, uint8_t axis_0, uint8_t axis_1, bool isclockwise,
               motion_data_t *block_data) {
  ...
  float pt0_a =
      -center_offset_a; // Radius vector from center to current location
  float pt0_b = -center_offset_b;
  float pt1_a = target[axis_0] -
                ptcenter_a; // Radius vector from center to current location
  ...
  uint16_t segment_count =
      floor(fabs(radiusangle) / sqrt(g_settings.arc_tolerance *
                                     (diameter - g_settings.arc_tolerance)));
  float arc_per_sgm =
      (segment_count != 0) ? arc_angle / segment_count : arc_angle;
  ...
  if (CHECKFLAG(block_data->motion_mode, MOTIONCONTROL_MODE_INVERSEFEED)) {
    // split the required time to complete the motion with the number of
    // segments
    block_data->feed *= segment_count;
  }

  // calculates an aproximation to sine and cosine of the angle segment
  // improves the error for the cosine by calculating an extra term of the
  // taylor series at the expence of an extra multiplication and addition
...
uint8_t mc_dwell(motion_data_t *block_data) {
  DBGLOG("[MC] dwell %hu ms", block_data->dwell);
  if (!mc_checkmode) // check mode (gcode simulation) doesn't send code to
                     // planner
  {
...
uint8_t mc_update_tools(motion_data_t *block_data) {
  ...
  if (!mc_checkmode) // check mode (gcode simulation) doesn't send code to
                     // planner
  {
    DBGLOG("[MC] update tools spindle=%hu",
           (block_data) ? block_data->spindle : 0);
    if (itp_sync() != STATUS_OK) {
      return STATUS_CRITICAL_FAIL;
    }
...
bool mc_home_motion(uint8_t axis_mask, bool is_origin_search, bool fast_mode) {
  ...
  block_data.feed = (fast_mode) ? g_settings.homing_fast_feed_rate
                                : g_settings.homing_slow_feed_rate;
  ...
  for (uint8_t i = 0; i < AXIS_COUNT; i++) {
    ...
    if (imask & axis_mask) {
      ...
      if (g_settings.homing_dir_invert_mask & axis_mask) {
        target[i] -= (is_origin_search) ? (g_settings.max_distance[i] * -1.5f)
                                        : g_settings.homing_offset * 5.0f;
      } else {
        target[i] += (is_origin_search) ? (g_settings.max_distance[i] * -1.5f)
                                        : g_settings.homing_offset * 5.0f;
      }
...
bool mc_home_motion_pulloff(uint8_t axis_mask, bool fast_mode) {
  ...
  mc_line(target, &block_data);
  itp_sync();
  if (cnc_get_exec_state(~EXEC_HOMING) != EXEC_IDLE) {
    // pulloff failed
    return false;
  }
...
uint8_t mc_home_axis(uint8_t axis_mask, uint8_t axis_limit) {
  ...
  uint8_t restore_step_mode __attribute__((__cleanup__(mc_restore_step_mode))) =
      itp_set_step_mode(ITP_STEP_MODE_REALTIME);

#ifdef ENABLE_MOTION_CONTROL_MODULES
  homing_status_t homing_status __attribute__((
      __cleanup__(mc_home_axis_finalize))) = {axis_mask, axis_limit, STATUS_OK};
#endif
...
// locks limits to accept axis limit mask only or else throw error
  ...
  if (cnc_get_exec_state(EXEC_HOLD | EXEC_ALARM) ||
      CHECKFLAG(io_get_raw_limits(), LIMITS_MASK)) {
  ...
  bool homing_failed = !mc_home_motion(axis_mask, true, true);
    ...
    io_disable_limits();        // temporary limits disable
  ...
  if (!mc_home_motion(axis_mask, false, false)) {
    return STATUS_CRITICAL_FAIL;
  }
...
uint8_t mc_probe(float *target, uint8_t flags, motion_data_t *block_data) {
  ...
  if (block_data->main_stepper == 255) {
    return STATUS_GCODE_INVALID_TARGET; // target is the current position
                                        // (invalid)
  }
  ...
  cnc_delay_ms(
      g_settings.debounce_ms); // adds a delay before reading io pin (debounce)
...
uint8_t mc_incremental_jog(float *target_offset, motion_data_t *block_data) {
  ...
  if ((state & ~EXEC_JOG) ||
      cnc_has_alarm()) // if any other than idle or jog discards the command
  {
...
void mc_flush_pending_motion(void) { mc_flush_pending = true; }

#ifdef ENABLE_G39_H_MAPPING

void mc_print_hmap(void) {
  proto_info("HMAP start corner: %f;%f", hmap_x, hmap_y);
  proto_info("HMAP end corner: %f;%f", hmap_x + hmap_x_offset,
             hmap_y + hmap_y_offset);
...
static float mc_apply_hmap(float *target) {
  ...
  a2 = hmap_offsets[H_MAPING_GRID_FACTOR * height_col + height_row +
                    H_MAPING_GRID_FACTOR];
  a3 = hmap_offsets[H_MAPING_GRID_FACTOR * height_col + height_row +
                    H_MAPING_GRID_FACTOR + 1] +
       a0 - a1 - a2;
  a1 -= a0;
  a2 -= a0;
...
uint8_t mc_build_hmap(float *target, float *offset, float retract_h,
                      motion_data_t *block_data) {
  // // generate dummy map
  // // store coordinates
...
void mc_clear_hmap(void) { memset(hmap_offsets, 0, sizeof(hmap_offsets)); }
#endif
