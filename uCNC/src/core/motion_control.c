...
void mc_spindle_index_cb_handler(void)
{
...
uint8_t mc_spindle_sync_line(float *target, float mm_per_rev, motion_data_t *block_data)
{
	...
	if (mc_update_tools(block_data) != STATUS_OK)
	{
		return STATUS_CRITICAL_FAIL;
	}

	if (!block_data->motion_flags.bit.spindle_running)
	{
	...
	uint16_t programmed_speed = block_data->spindle;
	...
	float average_rpm = block_data->spindle;
	...
	float line_dist = 0;
	float dir_vect[AXIS_COUNT];
	memcpy(dir_vect, target, sizeof(dir_vect));
	...
	rpm_to_stepfeed_constant = mm_per_rev * total_steps * MIN_SEC_MULT / line_dist;
	...
	float total_revs = line_dist / mm_per_rev;
	float feed = mm_per_rev * average_rpm;
	block_data->feed = feed;
	block_data->motion_flags.bit.synched = 1;
	...
	if (mc_line(target, block_data) != STATUS_OK)
	{
		return STATUS_CRITICAL_FAIL;
	}
	...
	encoder_attach_index_cb(&mc_spindle_index_cb_handler);
#endif
