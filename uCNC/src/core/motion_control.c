...
// resets accumulator vars of the block
	...
	if (!mc_checkmode) // check mode (gcode simulation) doesn't send code to planner
	{
		...
		DBGLOG("[MC] segment enqueued main=%hu steps=%lu", block_data->main_stepper, (unsigned long)block_data->steps[block_data->main_stepper]);
		// dwell should only execute on the first request
		block_data->dwell = 0;
	}
...
uint8_t mc_line(float *target, motion_data_t *block_data)
{
	...
	DBGLOG("[MC] line target X=%.3f Y=%.3f feed=%.3f", target[AXIS_X], target[AXIS_Y], block_data->feed);
...
// remove the hmap postion of the motion
	...
	DBGLOG("[MC] steps=%lu feed=%.3f max_feed=%.3f accel=%.3f dist=%.3f", (unsigned long)max_steps, block_data->feed, block_data->max_feed, block_data->max_accel, line_dist);
...
uint8_t mc_update_tools(motion_data_t *block_data)
{
	...
	if (!mc_checkmode) // check mode (gcode simulation) doesn't send code to planner
	{
		DBGLOG("[MC] update tools spindle=%hu", (block_data) ? block_data->spindle : 0);
		if (itp_sync() != STATUS_OK)
		{
			return STATUS_CRITICAL_FAIL;
		}
...
bool mc_home_motion(uint8_t axis_mask, bool is_origin_search, bool fast_mode)
{
	...
	block_data.feed = (fast_mode) ? g_settings.homing_fast_feed_rate : g_settings.homing_slow_feed_rate;
