/*
        ...
        Description: Simulates and MCU that runs on a Windows PC. This is mainly
   used to test/simulate µCNC.
...
#define printpin(X)                                                            \
  if (stimuli) {                                                               \
    fprintf(stimuli, "%d%c\n",                                                 \
            ((virtualmap.outputs & (1 << (X - DOUT_PINS_OFFSET))) ? 1 : 0),    \
            33 + X);                                                           \
    fflush(stimuli);                                                           \
  }
...
unsigned long stopCycleCounter(void) { return (getTickCounter() - perf_start); }
...
void virtual_delay_us(uint16_t delay) {
  ...
  do {
    elapsed =
        ((double)(getTickCounter()) - (double)(start)) / (double)(getCPUFreq());
    elapsed *= 1000000;
  } while (elapsed < delay);
}
...
void ticksimul(void) {
  ...
  bool test = false;
  if (!__atomic_compare_exchange_n(&running, &test, true, false,
  ...
  for (int i = 0; i < (int)timestep; i++) {
    ...
    if ((prev_special ^ virtualmap.special_outputs) ||
...
bool flash_fs_finfo(const char *path, fs_file_info_t *finfo) {
  ...
  finfo->full_name[FS_PATH_NAME_MAX_LEN - 1] =
  ...
  if (findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
    ...
    // File size (64-bit), we take only the lower 32 bits as per fs_file_info_t
    // definition
  ...
  // (UTC) Unix timestamp is in seconds since January 1, 1970 (UTC) Convert by
  // subtracting the number of 100-nanosecond intervals between these dates
  // (11644473600 seconds between 1601 and 1970) and dividing by 10,000,000 to
  // convert to seconds.
...
uint32_t mcu_free_micros(void) { return (uint32_t)(mcu_micros() % 1000); }
