...
static void pg_expand_min_y(pg_struct *pg, pg_word_t min_y,
                            uint8_t pge_idx) PG_NOINLINE;
...
/* assumes y2 > y1 */
static void pge_Init(struct pg_edge_struct *pge, pg_word_t x1, pg_word_t y1,
                     pg_word_t x2, pg_word_t y2) {
  pg_word_t dx = x2 - x1;
  pg_word_t width;
...
static uint8_t pg_prepare(pg_struct *pg) {
  ...
  if (pg->list[pg->pge[PG_LEFT].curr_idx].x !=
      pg->list[pg->pge[PG_RIGHT].curr_idx].x) {
...
static void pg_ClearPolygonXY(pg_struct *pg) { pg->cnt = 0; }
...
void u8g2_ClearPolygonXY(void) { pg_ClearPolygonXY(&u8g2_pg); }

void u8g2_AddPolygonXY(U8X8_UNUSED u8g2_t *u8g2, int16_t x, int16_t y) {
  pg_AddPolygonXY(&u8g2_pg, x, y);
}

void u8g2_DrawPolygon(u8g2_t *u8g2) { pg_DrawPolygon(&u8g2_pg, u8g2); }
