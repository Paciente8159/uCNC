...
[`README.md`](uCNC/src/hal/mcus/README.md) (the user-facing guide at `uCNC/src/hal/mcus/README.md`) contains the
canonical pin numbering table; this spec references it as the single source of
truth for pin numbers. Terminology is pinned in `.claude/skills/ucnc-hal-mcu/CONTEXT.md`;
...
core (cnc.c, interpolator.c, ...)
        |
        v
IO HAL  (uCNC/src/hal/io_hal.h)   <- resolves friendly names (STEP0, DOUT0, ...) to
...
MCU HAL (mcumap_<arch>.h + mcu_<arch>.c)   <- THIS SPEC
        |
        v
Board HAL (uCNC/src/hal/boards/*/boardmap_*.h)  <- maps friendly names to physical pins
```
...
* The **IO HAL** (`io_hal.h`) consumes exclusively the MCU HAL surface defined in
  `uCNC/src/hal/mcus/mcu.h` and the values of `DIO<n>` macros set by the mcumap.
...
2. `uCNC/src/cnc_hal_config_helper.h:30-37` defines the pin assert helpers used
   ...
   ```
3. `uCNC/src/hal/boards/boarddefs.h` includes the selected `BOARDMAP`, which defines
...
4. `boarddefs.h` then includes `uCNC/src/hal/mcus/mcudefs.h`, which includes
...
| # | Where | What |
|---|---|---|
| 1 | `uCNC/src/hal/mcus/mcus.h` | Add `#define MCU_<ARCH> <id>` (see grouping: AVR=1, STM32=10..13, SAMD21=20, LPC176X=30, ESP8266=40, ESP32=50..52, RP=60..61, virtual=1000/2000; pick the next free number in the matching family). |
| 2 | `uCNC/src/hal/mcus/mcudefs.h` | Add an `#if (MCU == MCU_<ARCH>) ... #include "<arch>/mcumap_<arch>.h" ... #endif` block. Set `CFG_TUSB_MCU`/`CFG_TUSB_OS` here if the MCU uses tinyUSB. |
| 3 | `uCNC/src/hal/boards/boards_helper.h` | Optional: register a `BOARD_<NAME>` and the matching `BOARDMAP` string. |
| 4 | `uCNC/src/hal/boards/<family>/boardmap_<board>.h` | New or existing board map defining pins and `#define MCU MCU_<ARCH>`. |
...
| 6 | `uCNC/src/hal/mcus/<arch>/README.md` | Document the backend following the README skeleton (§2.1). This is required for new backends; older backends are being migrated incrementally. |
...
- [ ] `uCNC/src/hal/mcus/mcus.h`: `MCU_<ARCH>` id added.
- [ ] `uCNC/src/hal/mcus/<arch>/mcumap_<arch>.h` follows the expansion patterns in
...
* Interface & defaults: `uCNC/src/hal/mcus/mcu.h`, `uCNC/src/hal/mcus/mcu.c`
  (weak defaults, generic IO init), `uCNC/src/hal/mcus/mcudefs.h`, `mcus.h`.
* User guide + canonical pin table: `uCNC/src/hal/mcus/README.md`.
...
* Glossary & decisions: `.claude/skills/ucnc-hal-mcu/CONTEXT.md`, `docs/adr/0001-pin-numbering-space.md`,
