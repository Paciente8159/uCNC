...
bool extend_settings_load(void *args) {
  ...
  // if a read error occurs it can be represent signaled to force the user to
  // reconfigure all the machine settings the settings error global variable
  // must be signaled like this:
...
DECL_MODULE(dummy) {
#ifdef ENABLE_SETTINGS_MODULES
  dummy_settings_address =
      settings_register_external_setting(sizeof(dummy_setting));
  ...
  // since settings are loaded before the modules are initialized, it's
  // recommended to manually trigger the load event since this will not fire
  // again naturally
...
DECL_EXTENDED_SETTING(DUMMY_SETTING_ID, &dummy_settings_address, uint32_t, 1,
