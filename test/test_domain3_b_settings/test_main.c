...
static void test_all_standard_settings_write_and_read(void) {
  static const setting_case_t cases[] = {
      {0, "30"},    {1, "25"},    {2, "1"},     {3, "2"},      {4, "1"},
      {5, "1"},     {6, "1"},     {10, "1"},    {11, "0.020"}, {12, "0.003"},
      {13, "1"},    {20, "0"},    {21, "0"},    {22, "0"},     {23, "1"},
      {24, "30"},   {25, "300"},  {26, "50"},   {27, "2"},     {30, "1000"},
      {31, "0"},    {100, "201"}, {101, "202"}, {102, "203"},  {110, "401"},
      {111, "402"}, {112, "403"}, {120, "11"},  {121, "12"},   {122, "13"},
      {130, "100"}, {131, "101"}, {132, "102"}};
  d3_idle();
  char failures[4096] = "";
  for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); ++i) {
    ...
    if (!d3_command(command, "ok\r\n", D3_TIMEOUT_MS)) {
      ...
      snprintf(failures + used, sizeof(failures) - used, "%s => %s; ", command,
    ...
    if (!strstr(grbl_test_transcript, needle)) {
      size_t used = strlen(failures);
      snprintf(failures + used, sizeof(failures) - used, "%s not reported; ",
               needle);
    }
  }
  TEST_IGNORE_MESSAGE("uCNC standard setting diverge from Grbl");
  TEST_ASSERT_EQUAL_STRING_MESSAGE("", failures,
                                   "standard setting coverage failures");
}
...
static void test_setting_validation_and_dependencies(void) {
  ...
  TEST_ASSERT_NOT_NULL_MESSAGE(strstr(grbl_test_transcript, "$20=0"),
                               "Expected $20=0 after $22=0 but got $20=1");
}
...
D3_UNITY_MAIN(RUN_TEST(test_all_standard_settings_write_and_read);
