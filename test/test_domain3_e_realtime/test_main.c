...
static void test_reset_idle_and_run(void) {
  ...
  grbl_test_assert_wait_for("ALARM:3");
...
static void test_safety_door_realtime_suspends_and_requires_resume(void) {
  ...
  d3_start_long_motion();
  d3_realtime(0x84);
  d3_expect_state("<Door", D3_MOTION_TIMEOUT_MS);
  d3_realtime('~');
  TEST_ASSERT_FALSE_MESSAGE(
      d3_wait_state("<Run", 500U),
      "cycle start resumed while door was logically open");
  d3_cancel_motion();
}
...
static void test_physical_hold_and_cycle_start_inputs(void) {
  d3_idle();
  d3_start_long_motion();
  test_io_set(TEST_IO_FHOLD, true);
  d3_expect_state("<Hold", D3_TIMEOUT_MS);
  test_io_set(TEST_IO_FHOLD, false);
  d3_expect_state("<Hold:0", D3_MOTION_TIMEOUT_MS);
...
D3_UNITY_MAIN(RUN_TEST(test_hold_resume_full_transition);
