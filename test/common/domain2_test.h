...
static __attribute__((unused)) bool d2_parse_status(const char *text,
                                                    d2_sample_t *sample) {
  ...
  if (!separator || separator - begin < 2 ||
      separator - begin >= (ptrdiff_t)sizeof(sample->state)) {
    return false;
  }
  ...
  if (!position || sscanf(position + 5, "%f,%f,%f", &sample->x, &sample->y,
                          &sample->z) != 3) {
    return false;
  }
  sample->feed = 0.0f;
...
d2_assert_position(const d2_sample_t *position, float x, float y, float z,
                   float tolerance) {
  TEST_ASSERT_FLOAT_WITHIN_MESSAGE(tolerance, x, position->x,
                                   "unexpected machine X");
  TEST_ASSERT_FLOAT_WITHIN_MESSAGE(tolerance, y, position->y,
                                   "unexpected machine Y");
  TEST_ASSERT_FLOAT_WITHIN_MESSAGE(tolerance, z, position->z,
                                   "unexpected machine Z");
}
...
static __attribute__((unused)) bool d2_same_position(const d2_sample_t *a,
                                                     const d2_sample_t *b) {
  return fabsf(a->x - b->x) < 0.0005f && fabsf(a->y - b->y) < 0.0005f &&
         fabsf(a->z - b->z) < 0.0005f;
}

static __attribute__((unused)) void d2_trace_motion(const char *command,
                                                    d2_trace_t *trace) {
  memset(trace, 0, sizeof(*trace));
  d2_sample_t initial = {0};
  TEST_ASSERT_TRUE_MESSAGE(d2_query_status(&initial),
                           "could not capture position before motion");
  ...
  do {
    ...
    if (d2_query_status(&current)) {
      ...
      if (!have_previous || !d2_same_position(&previous, &current) ||
          strcmp(previous.state, current.state)) {
      ...
      if (!strcmp(current.state, "Idle") &&
          (trace->saw_run || position_changed)) {
        trace->saw_idle = true;
        return;
      }
    }
...
static __attribute__((unused)) void
d2_assert_trace_endpoint(const d2_trace_t *trace, float x, float y, float z) {
  TEST_ASSERT_TRUE_MESSAGE(trace->saw_idle, "trace never reached Idle");
  TEST_ASSERT_GREATER_THAN_MESSAGE(0U, trace->count,
                                   "trace contains no status samples");
  d2_assert_position(&trace->samples[trace->count - 1U], x, y, z,
                     D2_POSITION_TOLERANCE);
}

static __attribute__((unused)) void
d2_assert_line(const d2_trace_t *trace, d2_sample_t start, d2_sample_t end) {
  float vx = end.x - start.x, vy = end.y - start.y, vz = end.z - start.z;
  float length2 = vx * vx + vy * vy + vz * vz;
  TEST_ASSERT_TRUE_MESSAGE(length2 > 0.0f,
                           "line validator requires a non-zero move");
  ...
  for (size_t i = 0; i < trace->count; ++i) {
    ...
    float distance =
        sqrtf((trace->samples[i].x - px) * (trace->samples[i].x - px) +
              (trace->samples[i].y - py) * (trace->samples[i].y - py) +
              (trace->samples[i].z - pz) * (trace->samples[i].z - pz));
    TEST_ASSERT_TRUE_MESSAGE(
        distance <= D2_PATH_TOLERANCE,
        "linear interpolation departed from commanded line");
    TEST_ASSERT_TRUE_MESSAGE(projection >= prior - 0.02f,
                             "linear motion reversed progress");
    TEST_ASSERT_TRUE_MESSAGE(projection >= -0.02f && projection <= 1.02f,
                             "linear motion overshot segment");
    prior = projection;
  }
}

static __attribute__((unused)) void
d2_assert_arc(const d2_trace_t *trace, int axis_u, int axis_v, int linear_axis,
              float center_u, float center_v, float radius, bool clockwise,
              float linear_start, float linear_end) {
  float previous_angle = 0.0f;
  float total_angle = 0.0f;
  bool have_angle = false;
  for (size_t i = 0; i < trace->count; ++i) {
    const float p[3] = {trace->samples[i].x, trace->samples[i].y,
                        trace->samples[i].z};
    float du = p[axis_u] - center_u, dv = p[axis_v] - center_v;
    float r = sqrtf(du * du + dv * dv);
    TEST_ASSERT_FLOAT_WITHIN_MESSAGE(D2_PATH_TOLERANCE, radius, r,
                                     "arc radial error exceeds tolerance");
    ...
    if (have_angle) {
      ...
      if (fabsf(delta) > 0.001f) {
        TEST_ASSERT_TRUE_MESSAGE(clockwise ? delta <= 0.03f : delta >= -0.03f,
                                 "arc traveled in wrong angular direction");
        total_angle += delta;
      }
    }
    ...
    if (linear_axis >= 0) {
      ...
      TEST_ASSERT_TRUE_MESSAGE(p[linear_axis] >= lo && p[linear_axis] <= hi,
                               "helix linear axis overshot");
    }
  }
  TEST_ASSERT_TRUE_MESSAGE(clockwise ? total_angle < -0.2f : total_angle > 0.2f,
                           "arc trace lacked directed intermediate motion");
}
