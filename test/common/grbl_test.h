...
static size_t grbl_test_snapshot(void) {
  return mcu_unit_test_buffer_read_since(grbl_test_output_cursor,
                                         grbl_test_transcript,
                                         sizeof(grbl_test_transcript));
}
...
static void grbl_test_assert_wait_for(const char *needle) {
  ...
  TEST_ASSERT_FALSE_MESSAGE(mcu_unit_test_buffer_overflowed(),
                            "Serial test transcript overflowed");
}
...
static void grbl_test_inject(const char *bytes) {
  TEST_ASSERT_TRUE_MESSAGE(mcu_unit_test_inject(bytes),
                           "Unable to inject bytes into Grbl stream");
}

static void grbl_test_command_expect(const char *command,
                                     const char *expected) {
  char line[192];
  int written = snprintf(line, sizeof(line), "%s\n", command);
  TEST_ASSERT_GREATER_THAN_INT_MESSAGE(0, written,
                                       "Unable to format test command");
  TEST_ASSERT_LESS_THAN_INT_MESSAGE((int)sizeof(line), written,
  ...
  if (!grbl_test_wait_for(expected, GRBL_TEST_TIMEOUT_MS)) {
    ...
    snprintf(message, sizeof(message),
             "command `%s` expected `%s`, received: %s", command, expected,
             grbl_test_transcript);
...
grbl_test_command_error(const char *command) {
  grbl_test_command_expect(command, "error:");
}

static __attribute__((unused)) bool
grbl_test_command_matches(const char *command, const char *expected) {
  char line[192];
  int written = snprintf(line, sizeof(line), "%s\n", command);
...
static __attribute__((unused)) void
grbl_test_realtime_expect(const char *bytes, const char *expected) {
  grbl_test_clear_output();
  ...
  if (!grbl_test_wait_for(expected, GRBL_TEST_TIMEOUT_MS)) {
    ...
    grbl_test_snapshot();
    snprintf(message, sizeof(message),
             "realtime input expected `%s`, received: %s", expected,
             grbl_test_transcript);
    TEST_FAIL_MESSAGE(message);
  }
}
...
static __attribute__((unused)) void grbl_test_start(void) {
  TEST_ASSERT_EQUAL_INT_MESSAGE(
      3, AXIS_COUNT, "Conformity fixtures require the default 3-axis build");
...
static __attribute__((unused)) void grbl_test_stop(void) { cnc_stop(true); }

static __attribute__((unused)) void
grbl_test_wait_for_state(const char *state, uint32_t timeout_ms) {
  uint32_t started = mcu_millis();
  do {
    grbl_test_clear_output();
    grbl_test_inject("?");
  ...
  snprintf(message, sizeof(message),
           "Machine did not reach `%s`; last report: %s", state,
           grbl_test_transcript);
  TEST_FAIL_MESSAGE(message);
}
