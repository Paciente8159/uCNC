...
static void d3_start(void) {
  TEST_ASSERT_EQUAL_INT_MESSAGE(
      3, AXIS_COUNT, "Domain 3 requires the default three-axis build");
  grbl_test_start();
}

static void d3_stop(void) { grbl_test_stop(); }
...
static bool d3_final_line_matches(const char *text, const char *expected) {
  ...
  if (!strcmp(expected, "error:"))
    return actual >= 6U && !strncmp(start, "error:", 6U);
  size_t wanted = strlen(expected);
  while (wanted &&
         (expected[wanted - 1U] == '\r' || expected[wanted - 1U] == '\n'))
    --wanted;
  return actual == wanted && !strncmp(start, expected, wanted);
}

static bool d3_command(const char *command, const char *terminal,
                       uint32_t timeout_ms) {
  char line[192];
  ...
  if (!grbl_test_wait_for(wait, timeout_ms))
    return false;
  grbl_test_snapshot();
...
static void d3_expect_command(const char *command, const char *terminal) {
  if (!d3_command(command, terminal, D3_TIMEOUT_MS)) {
    ...
    snprintf(message, sizeof(message),
             "`%s` expected terminal `%s`, received `%s`", command, terminal,
...
static bool d3_status(char *destination, size_t capacity) {
  ...
  if (!mcu_unit_test_inject("?") || !grbl_test_wait_for(">", 500U))
    return false;
  mcu_unit_test_buffer_read_since(grbl_test_output_cursor, destination,
                                  capacity);
...
static bool d3_wait_state(const char *state, uint32_t timeout_ms) {
  ...
  char status[512];
  do {
    if (d3_status(status, sizeof(status)) && strstr(status, state))
      return true;
    sched_yield();
...
static void d3_expect_state(const char *state, uint32_t timeout_ms) {
  if (!d3_wait_state(state, timeout_ms)) {
    char message[768];
    snprintf(message, sizeof(message), "expected state `%s`, last status `%s`",
             state, grbl_test_transcript);
    TEST_FAIL_MESSAGE(message);
  }
}

static __attribute__((unused)) bool d3_wait_either_state(const char *first,
                                                         const char *second,
                                                         uint32_t timeout_ms) {
  uint32_t start = mcu_millis();
  char status[512];
  do {
    if (d3_status(status, sizeof(status)) &&
        (strstr(status, first) || strstr(status, second)))
      return true;
    sched_yield();
  } while ((uint32_t)(mcu_millis() - start) < timeout_ms);
  return false;
...
d3_expect_no_serial_response(uint8_t byte, uint32_t observation_ms) {
  grbl_test_clear_output();
  d3_realtime(byte);
  uint32_t start = mcu_millis();
  ...
  TEST_ASSERT_EQUAL_STRING_MESSAGE(
      "", grbl_test_transcript,
      "realtime command unexpectedly emitted a serial response");
}
