...
static bool d1_terminal_response_matches(const char *transcript,
                                         const char *expected) {
  ...
  while (expected_length && (expected[expected_length - 1U] == '\r' ||
                             expected[expected_length - 1U] == '\n')) {
    --expected_length;
  }
  return actual_length == expected_length &&
         !strncmp(start, expected, expected_length);
}
...
static bool d1_enter_check(void) {
  ...
  if (!mcu_unit_test_inject("?") || !grbl_test_wait_for(">\r\n", 1000U) ||
      !grbl_test_snapshot_contains("<Check")) {
    return false;
  }
  return true;
}
...
static __attribute__((unused)) void d1_run_current_case(void) {
  ...
  if (rejection_case) {
    TEST_ASSERT_TRUE_MESSAGE(d1_get_modal(modal_before, sizeof(modal_before)),
                             "could not capture initial modal state");
  }

  if (!d1_send_line(d1_current_case->command, d1_current_case->expected)) {
    char reason[GRBL_TEST_TRANSCRIPT_SIZE + 256U];
    snprintf(reason, sizeof(reason), "`%s` expected `%s`, received `%s`",
             d1_current_case->command, d1_current_case->expected,
             grbl_test_transcript);
    TEST_FAIL_MESSAGE(reason);
  }

  if (rejection_case) {
    TEST_ASSERT_TRUE_MESSAGE(d1_get_modal(modal_after, sizeof(modal_after)),
                             "could not capture final modal state");
    TEST_ASSERT_EQUAL_STRING_MESSAGE(
        modal_before, modal_after,
        "rejected command altered parser modal state");
  }
