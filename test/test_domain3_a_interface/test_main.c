...
static void test_reset_help_and_queries(void) {
  ...
  d3_expect_command("$$", "ok\r\n");
  TEST_ASSERT_TRUE(d3_contains("$0="));
  TEST_ASSERT_TRUE(d3_contains("$132="));
  d3_expect_command("$#", "ok\r\n");
  TEST_ASSERT_TRUE(d3_contains("[G54:"));
  TEST_ASSERT_TRUE(d3_contains("[PRB:"));
  d3_expect_command("$G", "ok\r\n");
  TEST_ASSERT_TRUE(d3_contains("[GC:"));
  d3_expect_command("$I", "ok\r\n");
  TEST_ASSERT_TRUE(d3_contains("[VER:"));
  TEST_ASSERT_TRUE(d3_contains("[OPT:"));
  d3_expect_command("$N", "ok\r\n");
  TEST_ASSERT_TRUE(d3_contains("$N0="));
  TEST_ASSERT_TRUE(d3_contains("$N1="));
...
static void test_realtime_status_has_no_ok(void) {
  ...
  grbl_test_clear_output();
  d3_realtime('?');
  grbl_test_assert_wait_for(">");
  grbl_test_snapshot();
  TEST_ASSERT_NULL(strstr(grbl_test_transcript, "ok\r\n"));
}

D3_UNITY_MAIN(RUN_TEST(test_reset_help_and_queries);
