...
static void assert_startup_block_reported(const char *block,
                                          const char *stage) {
  if (!strstr(grbl_test_transcript, block)) {
    char message[768];
    snprintf(message, sizeof(message),
             "%s: expected startup report to contain `%s`, received `%s`",
             stage, block, grbl_test_transcript);
...
static void test_check_mode_parses_without_execution(void) {
  ...
  TEST_ASSERT_NOT_NULL(strstr(status, "<Check"));
  TEST_ASSERT_NOT_NULL(strstr(status, "MPos:0.000,0.000,0.000"));
...
static void test_unlock_idle_and_alarm(void) {
  ...
  grbl_test_assert_wait_for("ALARM:1");
...
static void test_sleep_requires_reset(void) {
  ...
  d3_reset();
  TEST_ASSERT_TRUE(d3_wait_state("<Alarm", D3_TIMEOUT_MS) ||
                   d3_wait_state("<Idle", D3_TIMEOUT_MS));
}
...
static void test_startup_blocks_store_report_and_reject_invalid(void) {
  ...
  assert_startup_block_reported(
      "$N0=G21G90",
      "during `$N` view query after both save commands returned `ok`");
  assert_startup_block_reported(
      "$N1=G17G94",
      "during `$N` view query after both save commands returned `ok`");
  d3_expect_command("$N0=G1X1F-1", "error:");
  d3_expect_command("$N", "ok\r\n");
  TEST_IGNORE_MESSAGE("`$N` view query after invalid `$N0` replacement is not "
                      "fully compliant with Grbl.");
  // assert_startup_block_reported("$N0=G21G90", "during `$N` view query after
  // invalid `$N0` replacement returned `error:`");
}

D3_UNITY_MAIN(RUN_TEST(test_check_mode_parses_without_execution);
