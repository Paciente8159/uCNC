...
static void run_probe_toward(const char *command) {
  ...
  TEST_ASSERT_TRUE_MESSAGE(p.x >= 0 && p.x < 5,
                           "probe-toward did not stop before target");
  assert_probe_report(true);
}

static void test_g38_2_stops_on_contact(void) {
  run_probe_toward("G38.2X5F120");
}
static void test_g38_3_stops_on_contact(void) {
  run_probe_toward("G38.3X5F120");
}
...
static void run_probe_away(const char *command) {
  ...
  d2_sample_t p = d2_position();
  TEST_ASSERT_TRUE_MESSAGE(p.x >= 0 && p.x < 5,
                           "probe-away did not stop before target");
  assert_probe_report(true);
}
...
static void test_non_error_probe_failures_reach_target(void) {
  ...
  grbl_test_command_ok("G38.3X1F300");
  d2_wait_idle_timeout(GRBL_TEST_TIMEOUT_MS);
  d2_sample_t p = d2_position();
  TEST_ASSERT_FLOAT_WITHIN(D2_POSITION_TOLERANCE, 1, p.x);
  assert_probe_report(false);
  test_io_set(TEST_IO_PROBE, true);
  grbl_test_command_ok("G38.5X2F300");
  d2_wait_idle_timeout(GRBL_TEST_TIMEOUT_MS);
  p = d2_position();
...
D2_UNITY_MAIN(RUN_TEST(test_g38_2_stops_on_contact);
