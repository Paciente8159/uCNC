...
{
	grbl_test_command_ok("G53G0X4Y2Z1"); d2_wait_idle_timeout(3000U);
	grbl_test_command_ok("G28.1");
	grbl_test_command_ok("G53G0X0Y0Z0"); d2_wait_idle_timeout(3000U);
	grbl_test_command_ok("G28"); d2_wait_idle_timeout(5000U);
	d2_sample_t p = d2_position(); d2_assert_position(&p, 4, 2, 1, D2_POSITION_TOLERANCE);
}

static void test_g30_store_and_return(void)
{
	grbl_test_command_ok("G53G0X3Y1Z2"); d2_wait_idle_timeout(3000U);
	grbl_test_command_ok("G30.1");
	grbl_test_command_ok("G53G0X0Y0Z0"); d2_wait_idle_timeout(3000U);
	grbl_test_command_ok("G30"); d2_wait_idle_timeout(5000U);
	d2_sample_t p = d2_position(); d2_assert_position(&p, 3, 1, 2, D2_POSITION_TOLERANCE);
}

static void test_g28_intermediate_point_has_two_legs(void)
{
	grbl_test_command_ok("G53G0X4Y0"); d2_wait_idle_timeout(3000U);
	grbl_test_command_ok("G28.1");
	grbl_test_command_ok("G53G0X0Y0"); d2_wait_idle_timeout(3000U);
	d2_trace_t trace;
...
static void test_tool_length_offset_and_cancel(void)
{
	...
	grbl_test_command_ok("G1Z0F300"); d2_wait_idle_timeout(3000U);
