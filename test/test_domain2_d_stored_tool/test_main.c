...
static void test_g28_intermediate_point_has_two_legs(void) {
  ...
  for (size_t i = 0; i < trace.count; ++i)
    reached_intermediate |= fabsf(trace.samples[i].x - 2) < 0.2f &&
                            fabsf(trace.samples[i].y - 2) < 0.2f;
  TEST_ASSERT_TRUE_MESSAGE(reached_intermediate,
                           "G28 trace did not pass through intermediate point");
}
...
static void test_tool_length_offset_and_cancel(void) {
  ...
  d2_assert_position(&unchanged, before.x, before.y, before.z,
...
D2_UNITY_MAIN(RUN_TEST(test_g28_store_and_return);
