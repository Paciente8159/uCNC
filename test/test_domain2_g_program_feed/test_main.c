...
static void test_dwell_has_no_position_effect(void) {
  ...
  TEST_ASSERT_GREATER_OR_EQUAL_UINT32_MESSAGE(30U, elapsed,
                                              "G4 dwell completed too early");
}
...
static void test_feed_is_reported_and_endpoint_is_unchanged(void) {
  ...
  TEST_ASSERT_TRUE_MESSAGE(
      fast_max > slow_max,
      "higher programmed feed did not increase reported feed");
}
...
static void test_axis_max_rate_does_not_change_endpoint(void) {
  ...
  TEST_ASSERT_TRUE_MESSAGE(trace.saw_run,
                           "rate-limited move never entered Run");
}

D2_UNITY_MAIN(RUN_TEST(test_dwell_has_no_position_effect);
