...
static void test_feed_override_bytes_and_clamps(void) {
  d3_idle();
  send_override(0x90);
  TEST_ASSERT_EQUAL(100, status_override(0));
  send_override(0x91);
  TEST_ASSERT_EQUAL(110, status_override(0));
  send_override(0x92);
  TEST_ASSERT_EQUAL(100, status_override(0));
  send_override(0x93);
  TEST_ASSERT_EQUAL(101, status_override(0));
  send_override(0x94);
  TEST_ASSERT_EQUAL(100, status_override(0));
  for (int i = 0; i < 20; ++i)
    send_override(0x91);
...
static void test_rapid_override_bytes(void) {
  d3_idle();
  send_override(0x95);
  TEST_ASSERT_EQUAL(100, status_override(1));
  send_override(0x96);
  TEST_ASSERT_EQUAL(50, status_override(1));
  send_override(0x97);
  TEST_ASSERT_EQUAL(25, status_override(1));
  send_override(0x95);
  TEST_ASSERT_EQUAL(100, status_override(1));
}
...
static void test_spindle_override_bytes(void) {
  d3_idle();
  d3_expect_command("M3S500", "ok\r\n");
  send_override(0x99);
  TEST_ASSERT_EQUAL(100, status_override(2));
  send_override(0x9A);
  TEST_ASSERT_EQUAL(110, status_override(2));
  send_override(0x9B);
  TEST_ASSERT_EQUAL(100, status_override(2));
  send_override(0x9C);
  TEST_ASSERT_EQUAL(101, status_override(2));
  send_override(0x9D);
  TEST_ASSERT_EQUAL(100, status_override(2));
}
...
D3_UNITY_MAIN(RUN_TEST(test_feed_override_bytes_and_clamps);
