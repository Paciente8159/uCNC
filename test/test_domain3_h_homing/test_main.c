...
{
	d3_idle(); d3_expect_command("$22=0", "ok\r\n");
...
static void test_reset_during_homing_alarm_6(void)
{
	...
	grbl_test_clear_output(); TEST_ASSERT_TRUE(mcu_unit_test_inject("$H\n")); d3_expect_state("<Home", D3_TIMEOUT_MS);
	grbl_test_clear_output(); d3_realtime(0x18);
	grbl_test_assert_wait_for("Grbl "); grbl_test_assert_wait_for("ALARM:6");
...
static void test_door_during_homing_alarm_7(void)
{
	d3_idle(); d3_expect_command("$21=1", "ok\r\n"); d3_expect_command("$22=1", "ok\r\n");
	grbl_test_clear_output(); TEST_ASSERT_TRUE(mcu_unit_test_inject("$H\n")); d3_expect_state("<Home", D3_TIMEOUT_MS);
	grbl_test_clear_output(); d3_realtime(0x84);
	grbl_test_assert_wait_for("ALARM:7"); d3_expect_state("<Alarm", D3_TIMEOUT_MS);
}
...
static void test_homing_search_failure_alarm_9(void)
{
	...
	grbl_test_clear_output(); TEST_ASSERT_TRUE(mcu_unit_test_inject("$H\n"));
	grbl_test_assert_wait_for("ALARM:9"); d3_expect_state("<Alarm", D3_MOTION_TIMEOUT_MS);
}
