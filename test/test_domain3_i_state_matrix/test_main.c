...
static void test_run_acceptance_matrix(void) {
  ...
  d3_expect_command("G91G1X4F40", "ok\r\n");
...
static void test_hold_acceptance_matrix(void) {
  ...
  TEST_ASSERT_NOT_NULL(strstr(status, "<Hold"));

  /* A valid motion is accepted during hold, but its ok is deferred until
   * cycle-start. */
...
static void test_jog_acceptance_matrix(void) {
  ...
  d3_expect_command("$J=G91X10F60", "ok\r\n");
  d3_expect_state("<Jog", D3_TIMEOUT_MS);
  d3_realtime(0x85);
...
D3_UNITY_MAIN(RUN_TEST(test_idle_acceptance);
