...
static void test_hard_limits_idle_each_axis(void) {
  const test_io_id_t limits[] = {TEST_IO_LIMIT_X, TEST_IO_LIMIT_Y,
                                 TEST_IO_LIMIT_Z};
  for (size_t i = 0; i < 3; ++i) {
    d3_idle();
    d3_expect_command("$21=1", "ok\r\n");
    grbl_test_clear_output();
    test_io_set(limits[i], true);
...
static void test_probe_alarm_4_and_5(void) {
  ...
  grbl_test_clear_output();
  TEST_ASSERT_TRUE(mcu_unit_test_inject("G38.2X1F300\n"));
...
D3_UNITY_MAIN(RUN_TEST(test_hard_limits_idle_each_axis);
